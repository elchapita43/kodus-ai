# Plano — Sync de Cody Rules globais a partir de repositórios selecionados

## Objetivo
Permitir que o usuário selecione um ou mais repositórios conectados como **fontes de
Cody Rules globais**. As rules desses repos são importadas com o **mesmo mecanismo de
scan e manutenção** que já existe hoje para rules por-repositório, mas salvas no
**escopo global** (`repositoryId = "global"`).

Sem marcador `@cody-global` — a seleção do repo-fonte é o único gatilho.

## Como o sistema funciona hoje (baseline confirmado)
- Scan por padrões de path hardcoded em `libs/common/utils/cody-rules/file-patterns.ts`
  (`RULE_FILE_PATTERNS`): `.cursorrules`, `.cursor/rules/**`, `CLAUDE.md`, `AGENTS.md`,
  `.github/copilot-instructions.md`, `.cody/rules/**`, `rules/**/*.md`, etc.
- Orquestração: `libs/codyRules/infrastructure/adapters/services/codyRulesSync.service.ts`
...
- Escopo/"nível" é modelado por campos, não por enum: `repositoryId` (sentinel
  `"global"` para org-wide) + `directoryId`. `CodyRulesScope` só tem `FILE`/`PULL_REQUEST`.
...
- **`sourceRepositoryId`** na rule sincronizada (e no `CreateCodyRuleDto`).
...
- **Cron diário em background** (via `@nestjs/schedule` — já usado; ex. cron de cody-rules
  às 04:00 em `cody-rule-detector-sweep.service.ts`) varrendo todos os repos-fonte globais.
...
- **Short-circuit por SHA:** `getRepositoryAllFiles` já retorna o SHA de cada arquivo do
  ...
  `lastContentHash` em `codyRules.interface.ts:200`, hoje declarado e nunca usado) e no
...
- **PR merged no próprio repo-fonte** → mantém como caminho imediato (estender o listener
  `cody-rules-sync.listener.ts` para fan-out ao escopo global).
...
- `codyRulesSync.service.ts`: variante das rotinas de scan aceitando target global +
  `sourceRepositoryId`; lógica de upsert com nova chave.
- `libs/ee/codyRules/dtos/create-cody-rule.dto.ts`: campo `sourceRepositoryId`.
- `libs/codyRules/domain/interfaces/codyRules.interface.ts`: interface da rule.
- Mongo model em `libs/core/infrastructure/database/mongo/cody-rules/`.
- `cody-rules-sync.listener.ts`: fan-out para escopo global no `pull-request.closed`.
...
- Controller `apps/api/src/controllers/codyRules.controller.ts`: endpoints de
  get/save fontes globais + resync global.

## Frontend — arquivos a tocar
- Aba Configuration da tela de Cody Rules (escopo global):
  `apps/web/.../cody-rules/_components/_page.tsx` (TabsContent configuration,
  gate `isGlobalView`).
...
- Client + hooks em `apps/web/src/lib/services/codyRules/` (fetch.ts, hooks.ts, index.ts)
  para os novos endpoints.

## Skip de code review em repo-fonte
Repo selecionado só como fonte de Cody Rules globais é repo de config/dados, não
