# Cody Rules — resumo estruturado de rules longas (produtização)

## Contexto

Investigação Physitrack (2026-07): recall de cody rules limitado pela **detecção do judge**,
não pelos filtros (fase1==fase2 medido). Experimento validado no eval
(`evals/cody-rules/summarize-rules.js` + `rails-convention-cases-summarized.json`):
...
4. **Geração**: prompt verbatim do experimento (2 seções), `generateText` simples, async.
   - No write (create/update/import): hook fire-and-forget no choke point
     `libs/ee/codyRules/service/codyRules.service.ts` (todos os caminhos convergem ali).
...
| # | Arquivo | Mudança |
|---|---|---|
| 1 | `libs/codyRules/domain/interfaces/codyRules.interface.ts` | `ICodyRuleSummary` + campo `summary?` em `ICodyRule` |
| 2 | `libs/codyRules/infrastructure/adapters/repositories/schemas/codyRules.model.ts` | campo no schema Mongoose |
| 3 | `libs/codyRules/.../services/cody-rule-summary.service.ts` (novo) | `isLong`, `generate`, `resolveForReview`, `ensureSummaries` |
| 4 | `libs/ee/codyRules/service/codyRules.service.ts` | hook async pós-write + limpeza quando encurta |
