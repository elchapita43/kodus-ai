...
某些约定适用于所有地方。将它们作为记忆教给 Cody：

```
@cody remember: in this monorepo, all packages use ESM imports.
CommonJS require() is not allowed anywhere.
```

```
@cody remember: shared libs must maintain backward compatibility.
...
有关目录配置的更多信息，请参阅[目录级配置](/how_to_use/zh/code_review/configs/directory_level)。有关规则继承，请参阅[规则继承](/how_to_use/zh/code_review/configs/rules_inheritance)。
