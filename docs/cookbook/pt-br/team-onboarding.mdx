...
description: "Guia passo a passo para configurar o Codus para um novo time — desde a conexão de repositórios até o ensino das primeiras convenções."
icon: "users-gear"
---

## Visão geral

Colocar um novo time em funcionamento com o Codus leva cerca de 30 minutos. Este guia percorre a configuração completa para que seu time extraia valor desde o primeiro PR.
...
```yaml
# codus-config.yml — coloque este arquivo na raiz de cada repositório
reviewOptions:
  bug: true
  security: true
  performance: true
...
1. Vá para **Configurações de Code Review** → **Cody Rules**
2. O Codus detecta arquivos `.cursorrules`, `.github/copilot-instructions.md` e `.claude`
...
Conecte Jira, Linear, Notion ou ClickUp em **Configurações** → **Plugins** para habilitar a validação de lógica de negócio. Isso permite que o Codus verifique se os PRs implementam o que as tarefas descrevem.

## Passo 7 — Execute um PR piloto

Abra um PR de teste e verifique:

- [ ] Os comentários do Codus aparecem no PR
...
- Quer regras geradas automaticamente? → Clique em **Gerar Cody Rules** para obter sugestões do histórico de revisões

## Dicas

- Não sobrecarregue com regras no primeiro dia — comece pequeno e expanda
- Deixe o time ensinar o Cody naturalmente por meio de conversas antes de formalizar como regras
- Use `@cody start-review` nos primeiros PRs para que o time veja o Codus em ação
- Habilite a aprovação de memórias geradas por LLM se quiser revisar o que o Cody aprende
