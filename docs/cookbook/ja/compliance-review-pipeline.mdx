...
description: "監査トレイル、データ処理、アクセス制御要件を適用するためのCodyルールとMemoriesの設定方法。"
icon: "file-shield"
---
...
まず高レベルのコンプライアンス原則をCodyに教えます：

```
@cody remember: 私たちはSOC2準拠です。すべてのデータアクセスは、
誰が何を、いつ、どこからアクセスしたかを記録する必要があります。
```

```
@cody remember: PII（個人識別情報）はログ、エラーメッセージ、
またはAPIレスポンスに絶対に表示されてはなりません。PIIには以下が含まれます：
氏名、メールアドレス、電話番号、住所、SSN、生年月日、IPアドレス。
```

```
@cody remember: 保存されているすべてのデータは暗号化される必要があります。
PIIを含むデータベースフィールドはアプリケーションレベルの暗号化を使用する必要があります。
```

```
@cody remember: アクセス制御は最小権限の原則に従います。
...
ルールの設定の詳細については、[Codyルール](/how_to_use/ja/code_review/configs/cody_rules) を参照してください。
