...
description: "Convert your engineering handbook into living Cody Rules with optional MCP plugins."
icon: "shield-check"
---

## Why this guide matters

Every engineering team has a “best practices” doc somewhere. The problem is that IDE suggestions don’t read it, reviewers forget the nuances, and the document slowly loses authority. This guide shows how to turn your handbook into active guardrails so Cody enforces those rules automatically and cites the relevant section whenever code drifts.

We’ll focus on two building blocks:

- [Cody Rules](/how_to_use/en/code_review/configs/cody_rules) – encode each policy bullet as deterministic file-level or PR-level checks.
...
- You have permission to edit **Code Review Settings → Cody Rules**.
...
1. Create or reuse a markdown file such as `docs/engineering-standards.md`. The only requirement: Cody must be able to reference it via `@file:...`.
...
## Phase 3 – Translate sections into Cody Rules

Open **Code Review Settings → Cody Rules** and map each policy section to the appropriate scope.
...
<Info>
  Rules already understand variables (`fileDiff`, `pr_files_diff`, etc.), file references, and MCP invocations. See the full matrix in the [Cody Rules guide](/how_to_use/en/code_review/configs/cody_rules).
</Info>


## Phase 4 – Mirror external standards via MCP (optional)

Sometimes the handbook (or part of it) must stay in Notion, Confluence, Google Docs, or another system. You can still feed it into Cody:
...
Now Cody enforces the same standards regardless of where the document lives.

## Phase 5 – Test, observe, and keep it alive

<AccordionGroup>
  <Accordion title="Seed a verification PR">
    Create a throwaway PR that violates each section. Mention @cody and ensure the inline + PR-level comments cite the right bullets.
  </Accordion>
...
- [ ] File-level and PR-level Cody Rules cover your critical sections.
