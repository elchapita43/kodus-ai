...
description: "Step-by-step guide to set up Codus for a new team — from connecting repos to teaching your first conventions."
icon: "users-gear"
---

## Overview

Getting a new team up and running with Codus takes about 30 minutes. This guide walks through the complete setup so your team gets value from the first PR.
...
```yaml
# codus-config.yml — drop this in each repo root
reviewOptions:
  bug: true
  security: true
  performance: true
...
1. Go to **Code Review Settings** → **Cody Rules**
2. Codus detects `.cursorrules`, `.github/copilot-instructions.md`, and `.claude` files
...
Connect Jira, Linear, Notion, or ClickUp in **Settings** → **Plugins** to enable business logic validation. This lets Codus check if PRs implement what the tasks describe.

## Step 7 — Run a pilot PR

Open a test PR and verify:

- [ ] Codus comments appear on the PR
...
- Want auto-generated rules? → Click **Generate Cody Rules** to get suggestions from review history

## Tips

- Don't overload with rules on day one — start small and grow
- Let the team teach Cody naturally through conversation before formalizing as rules
- Use `@cody start-review` for the first few PRs so the team sees Codus in action
- Enable LLM-generated memories approval if you want to review what Cody learns
