...
description: "Configura Cody Rules y Memorias para aplicar requisitos de registro de auditoría, manejo de datos y control de acceso."
icon: "file-shield"
---
...
Empieza enseñando a Cody los principios de cumplimiento de alto nivel:

```
@cody remember: we are SOC2 compliant. All data access must be logged
with who accessed what, when, and from where.
```

```
@cody remember: PII (personally identifiable information) must never
appear in logs, error messages, or API responses. PII includes:
name, email, phone, address, SSN, date of birth, IP address.
```

```
@cody remember: all data at rest must be encrypted. Database fields
containing PII must use application-level encryption.
```

```
@cody remember: access control follows least privilege principle.
