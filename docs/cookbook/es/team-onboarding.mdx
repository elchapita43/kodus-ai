...
description: "Guía paso a paso para configurar Codus con un nuevo equipo, desde la conexión de repositorios hasta la enseñanza de las primeras convenciones."
icon: "users-gear"
---

## Descripción general

Poner en marcha un nuevo equipo con Codus lleva unos 30 minutos. Esta guía recorre la configuración completa para que tu equipo obtenga valor desde el primer PR.
...
```yaml
# codus-config.yml — coloca esto en la raíz de cada repositorio
reviewOptions:
  bug: true
  security: true
  performance: true
...
1. Ve a **Configuración de Code Review** → **Cody Rules**
2. Codus detecta archivos `.cursorrules`, `.github/copilot-instructions.md` y `.claude`
...
```
@cody remember: we use camelCase for variables and PascalCase for types.
```

```
@cody remember: all API responses use the standard envelope format
{ data, error, meta }.
```

```
@cody remember: we prefer composition over inheritance.
Use interfaces and dependency injection.
```

```
@cody remember: every public function must have JSDoc with
@param and @returns descriptions.
```

```
@cody remember: database queries go through the repository layer only.
...
Conecta Jira, Linear, Notion o ClickUp en **Configuración** → **Plugins** para habilitar la validación de lógica de negocio. Esto permite a Codus verificar si los PRs implementan lo que describen las tareas.

## Paso 7 — Ejecuta un PR piloto

Abre un PR de prueba y verifica:

- [ ] Los comentarios de Codus aparecen en el PR
...
- ¿Quieres reglas generadas automáticamente? → Haz clic en **Generar Cody Rules** para obtener sugerencias del historial de revisiones

## Consejos

- No sobrecargues con reglas el primer día — empieza pequeño y crece
- Deja que el equipo enseñe a Cody de forma natural mediante conversaciones antes de formalizarlo como reglas
- Usa `@cody start-review` durante los primeros PRs para que el equipo vea a Codus en acción
- Habilita la aprobación de memorias generadas por LLM si quieres revisar lo que aprende Cody
