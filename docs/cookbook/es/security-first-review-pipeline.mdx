...
description: "Configura Cody Rules enfocadas en el OWASP top 10, detección de secretos y patrones de codificación segura."
icon: "shield-halved"
---
...
Esto te da las verificaciones de seguridad integradas de Codus. Las reglas a continuación añaden las políticas de seguridad específicas de tu equipo por encima.
...
```
@cody remember: all user input must be validated and sanitized
before use. Never trust client-side data.
```

```
@cody remember: all API endpoints that handle sensitive data
must use HTTPS only and include rate limiting.
```

```
@cody remember: error messages must never expose stack traces,
...
1. Habilita **Solicitar cambios** en la configuración de flujo de trabajo de PR para que Cody solicite cambios cuando se encuentren problemas críticos
...
Para más información sobre la configuración de reglas, consulta [Cody Rules](/how_to_use/es/code_review/configs/cody_rules).
