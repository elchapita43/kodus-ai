---
title: "自托管配置"
description: "将 Codus CLI 连接到您的自托管 Codus 实例。"
---

如果您在自己的基础设施上运行 Codus，CLI 的使用方式完全相同 — 只需将其指向您的 API。

## 连接到您的实例

<Steps>
  <Step title="设置 API URL">
    告诉 CLI 您的 Codus API 地址。选择一种方式：
    ...
    <Tabs>
      ...
      <Tab title="配置文件">
        ...
        运行 `codus auth team-key` 时 CLI 会自动创建此文件，但您也可以手动创建。
      </Tab>
      <Tab title="内联（一次性）">
        ```bash
    ...
    **优先级：** `CODUS_API_URL` 环境变量 > `~/.codus/config.json` 中的 apiUrl > 默认值（`https://api.kodus.io`）。
  </Step>

  <Step title="认证">
    <Tabs>
      <Tab title="团队密钥（推荐）">
        从您的自托管 Codus 仪表板生成团队密钥并配置：
      ...
      <Tab title="个人登录">
        ...
        输入在自托管实例上注册的邮箱和密码。令牌以受限权限（600）存储在 `~/.codus/credentials.json` 中。
      </Tab>
      <Tab title="CI/CD 令牌">
        从个人账户生成令牌，然后在流水线中使用：
...
CLI 将配置存储在 `~/.codus/config.json` 中：
...
| 字段 | 必需 | 描述 |
|------|------|------|
| `apiUrl` | 否 | 自托管 API URL。可被 `CODUS_API_URL` 环境变量覆盖。 |
| `teamKey` | 否 | 团队 API 密钥（以 `codus_` 开头）。可被 `CODUS_TEAM_KEY` 环境变量覆盖。 |
| `teamName` | 否 | 团队显示名称。 |
...
<Info>
  配置文件以 `0600` 权限创建（仅所有者可读写）。目录 `~/.codus/` 以 `0700` 权限创建。
</Info>
...
| 变量 | 描述 |
|------|------|
| `CODUS_API_URL` | 自托管 API 端点（需要 HTTPS，localhost 除外） |
| `CODUS_TEAM_KEY` | 团队共享访问密钥 |
| `CODUS_TOKEN` | CI/CD 流水线令牌 |
| `CF_ACCESS_CLIENT_ID` | Cloudflare Access 客户端 ID |
| `CF_ACCESS_CLIENT_SECRET` | Cloudflare Access 客户端密钥 |
| `CODUS_VERBOSE` | 设置为 `true` 以查看解析的 API URL 和请求详情 |
...
<AccordionGroup>
  ...
  <Accordion title="连接被拒绝或超时">
    - 验证 URL 可达：`curl -I https://codus-api.yourcompany.com`
    - 检查防火墙或 VPN 是否阻止了连接
