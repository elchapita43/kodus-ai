---
title: "AI 代理"
description: "将 Codus CLI 与 AI 编码代理集成，实现自主审查-修复循环。"
---
...
任何能够执行 shell 命令并解析结构化文本的代理，都能直接使用 `codus review --prompt-only`。
...
1. 全局安装 `@codus/cli` 可执行文件。
2. 将团队密钥配置到 `~/.codus/config.json`。
3. 将 Codus 技能部署到每个检测到的代理（Claude Code、Cursor、Codex、Windsurf）。
...
<Steps>
  ...
  <Step title="代理提交并推送">
    审查通过后，代理提交并推送。作为额外保障，可以安装 pre-push 钩子（`codus hook install`），为手动推送提供双保险。
  </Step>
</Steps>
...
codus review --prompt-only --fail-on error    # 通用审查
codus pr business-validation --task-id KC-1441 --dry-run   # 对照任务检查
codus review --prompt-only --fix               # 自动应用可修复问题
git commit -am "fix: KC-1441 address review findings"
```

使用技能安装器后，Claude Code 的 Codus 技能知道何时调用这些命令 — 您无需做 prompt 工程。
