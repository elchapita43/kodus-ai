---
title: "セルフホストセットアップ"
description: "Codus CLI を自己ホスト型 Codus インスタンスに接続します。"
---

独自のインフラで Codus を実行している場合、CLI は同じように動作します — API に向けるだけです。

## インスタンスへの接続

<Steps>
  <Step title="API URL を設定">
    Codus API の場所を CLI に伝えます。方法を1つ選択してください:
    ...
    <Tabs>
      ...
      <Tab title="設定ファイル">
        ...
        CLI は `codus auth team-key` を実行したときにこのファイルを自動的に作成しますが、手動で作成することもできます。
      </Tab>
    ...
    **優先順位:** `CODUS_API_URL` 環境変数 > `~/.codus/config.json` の apiUrl > デフォルト（`https://api.kodus.io`）。
  </Step>

  <Step title="認証">
    <Tabs>
      <Tab title="チームキー（推奨）">
        セルフホスト型 Codus ダッシュボードからチームキーを生成して設定:
      ...
      <Tab title="個人ログイン">
        ...
        セルフホスト型インスタンスに登録されたメールとパスワードを入力します。トークンは制限されたパーミッション（600）で `~/.codus/credentials.json` にローカル保存されます。
      </Tab>
...
CLI は `~/.codus/config.json` に設定を保存します:
...
| フィールド | 必須 | 説明 |
|-----------|------|------|
| `apiUrl` | いいえ | セルフホスト型 API URL。`CODUS_API_URL` 環境変数で上書き。 |
| `teamKey` | いいえ | チーム APIキー（`codus_` で始まる）。`CODUS_TEAM_KEY` 環境変数で上書き。 |
...
<Info>
  設定ファイルは `0600` パーミッション（所有者の読み取り/書き込みのみ）で作成されます。ディレクトリ `~/.codus/` は `0700` パーミッションで作成されます。
</Info>
...
| 変数 | 説明 |
|------|------|
| `CODUS_API_URL` | セルフホスト型 API エンドポイント（HTTPS 必須、localhost 除く） |
| `CODUS_TEAM_KEY` | 共有アクセス用チームキー |
| `CODUS_TOKEN` | パイプライン用 CI/CD トークン |
| `CF_ACCESS_CLIENT_ID` | Cloudflare Access クライアント ID |
| `CF_ACCESS_CLIENT_SECRET` | Cloudflare Access クライアントシークレット |
| `CODUS_VERBOSE` | 解決された API URL とリクエスト詳細を表示するには `true` に設定 |
...
<AccordionGroup>
  ...
  <Accordion title="接続拒否またはタイムアウト">
    - URL に到達できることを確認: `curl -I https://codus-api.yourcompany.com`
  ...
  <Accordion title="認証失敗（401）">
    ...
    - `CODUS_API_URL` が正しいインスタンスを向いていることを確認
  </Accordion>
