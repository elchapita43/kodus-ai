---
title: "Overview"
description: "Cody Rules are customizable guidelines your team sets up to automatically enforce code quality, consistency, security, and maintainability."
---
...
## Two Types in the Cody Rules UI

Cody Rules are organized into two categories, managed through a tabbed interface in your settings:
...
<Tabs>
  ...
  <Tab title="Memories">
    Persistent contextual instructions that are injected across all prompts and conversations. Memories allow Cody to learn and retain how your codebase works so future reviews and suggestions are more relevant.

    - Injected as **high-priority context** in code reviews, cross-file analysis, and conversations
    - Created via **conversation with Cody** or **manually in the UI**
...
- Cody identifies file references automatically when you save a rule
...
- File contents are injected into the rule context, allowing Cody to compare, validate, or enforce patterns
...
- Navigate to Discovery Rules in your Codus dashboard.
...
Memories are persistent contextual instructions that Cody learns from your team's conversations and coding practices. Unlike Review Rules that run during code review, Memories are injected across all prompts and conversations to provide continuous, high-priority context and improve future suggestions.
...
- **Better future suggestions**: Memories help Cody avoid repeating the same mistaken assumptions about your repo
- **Intelligent deduplication**: When a new memory is created, Cody uses an LLM-based resolution mechanism to determine whether to create it, skip it (if duplicate), or update an existing memory
...
The most natural way to create memories is through conversation with Cody in your PR comments. Cody detects both explicit and implicit intents to save conventions:

**Explicit** — directly asking Cody to remember:
...
**Implicit** — stating a preference or convention that Cody captures:
...
```
@cody in this service, tests usually live next to implementation files rather than in a central test folder.
```
...
<Info>
  Cody will NOT create memories for transient instructions (e.g., "fix this now"), debugging chatter, questions, vague statements, or requests explicitly scoped to a single task or PR.
</Info>

When a memory is created or updated, Cody responds with a confirmation and provides a **direct link** to view the memory in the UI.

#### Via the UI

You can also create memories manually:

1. Go to **Code Review Settings** → **Repository** → **Cody Rules**
...
1. Go to **Code Review Settings** → **Repository** → **Cody Rules** → **Memories** tab
2. Enable **Cody knowledge approval**

When enabled:
- AI-generated rules and memories enter a **pending state** and are not active until approved
- A notification badge appears showing the pending count
- Click **Pending** to review, approve, discard, or convert pending items
- Both new creations and updates to existing rules/memories go through the approval flow

You can also set this in `codus-config.yml`:
...
<CardGroup cols={3}>
  ...
  <Card title="AI Generation" icon="sparkles" href="/how_to_use/en/code_review/learning/cody_rules_generation">
