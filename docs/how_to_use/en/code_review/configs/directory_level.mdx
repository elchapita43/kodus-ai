...
Cody selects a single config per PR: **Directory** → **Repository** → **Global**
...
<Note>
  Cody uses one config per PR to keep reviews predictable. Mixing configs causes conflicts.
</Note>
...
If you also use `codus-config.yml`, it overrides web settings (see Config Priority in General settings).
