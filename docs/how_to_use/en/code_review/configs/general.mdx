---
title: "General"
description: "The core review settings — cadence, PR workflow, skip conditions, and analysis types — plus the full codus-config.yml reference."
---

## Core Settings

<Note>
  Codus has three config levels: Global, Repository, and Directory. Learn how they work in <a href="/how_to_use/en/code_review/configs/inheritance_overrides">Config Inheritance & Overrides</a>.
</Note>

### Config Priority

You can keep your review configuration in a `codus-config.yml` file at the root of your
repository instead of the web UI.

<Warning>
  **The file is ignored until you opt in.** By default `codusConfigFileOverridesWebPreferences`
  is `false`, which means Cody never reads `codus-config.yml` — committing the file alone does
  nothing.

  Turn it on first in **Settings → Code Review → the repository → General**, or set
  `codusConfigFileOverridesWebPreferences: true` at the repository (or directory) level in the
...
<Accordion title="Sample codus-config.yml">
```yaml
version: 2.0
...
Cody can review PRs automatically when opened, or only when you comment `@cody start-review`.
...
How Cody handles reviews on subsequent commits:
...
<Tabs>
  ...
  <Tab title="Manual">
    Only run follow-up reviews when you comment `@cody start-review`.
  </Tab>
</Tabs>
...
#### Auto-approve PRs
Cody approves PRs when no issues are found.

**Platforms:** GitHub, GitLab, Bitbucket

#### Request Changes
Cody requests changes when finding critical issues.
...
<Info>
  **Empty (the default) means every base branch is reviewed** — there is no built-in
  ...
  list is non-empty, Cody reviews only PRs targeting those branches (your repository's default
...
Cody reviews across four categories. Enable or disable each one under **Analysis Types**, or via
`reviewOptions` in `codus-config.yml`.
...
<Note>
  Cody Rules are **not** one of these categories — they always run, independently of
  `reviewOptions`. See [Cody Rules](/how_to_use/en/code_review/configs/cody_rules).
</Note>
