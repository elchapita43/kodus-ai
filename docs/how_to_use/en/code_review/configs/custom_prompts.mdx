...
description: "Customize Cody's review prompts for each category and how suggestions are written to match your team's needs"
---

Custom Prompts give you complete control over how Cody analyzes your code and communicates findings. Tailor what the AI looks for in each review category and how it presents suggestions to match your team's standards and communication style.
...
<CardGroup cols={2}>
  <Card title="Team-Specific Standards" icon="users">
    Align Cody's reviews with your team's unique coding standards, architectural patterns, and best practices
  </Card>
  ...
  <Card title="Focus Control" icon="bullseye">
    Direct Cody's attention to issues that matter most to your project and reduce noise
  </Card>
  <Card title="Communication Style" icon="comments">
    Customize how Cody presents suggestions - detailed for critical issues or concise for minor ones
  </Card>
</CardGroup>
...
Define what Cody looks for in each review category:
...
Control how Cody communicates each individual suggestion. This customizes the message format and tone that appears in PR comments, like:
...
Your custom prompt instructs Cody on how to write suggestion messages. The prompt receives context about the severity level, problem description, suggested code, and whether it's a Cody Rules violation. Cody uses this context along with your prompt to generate appropriate messages for each suggestion.
...
## Setting them in `codus-config.yml`

The same four fields are available under the `v2PromptOverrides` key:
...
These four are what the web UI exposes, and each is capped at 2000 characters. There is no
prompt override for the Business Logic category.

The config file accepts one more group that the UI doesn't surface — the severity rubric Cody
uses to classify a finding:
...
v2PromptOverrides:
    severity:
        flags:
            critical: |
                What counts as critical (crash, data loss, security breach...).
            high: |
            medium: |
            low: |
```
...
<Warning>
  This changes how Cody *classifies* findings, not just how it words them — a bad rubric can
  quietly mute or inflate severities across every review. Start from the shipped defaults in
  [`default-codus-config.yml`](https://github.com/elchapita43/codus-ai/blob/main/default-codus-config.yml)
  rather than writing a rubric from scratch.
</Warning>

<Note>
  The shipped `default-codus-config.yml` also contains a `v2PromptOverrides.level` group. It is
  still accepted for backward compatibility but **the engine no longer reads it** — setting it
  changes nothing. Only `categories.descriptions`, `severity.flags` and `generation.main` reach
  the review.
</Note>

The file is only read once `codusConfigFileOverridesWebPreferences` is enabled (see
[General](/how_to_use/en/code_review/configs/general)).

## External References

All custom prompts—Category, Suggestion, and PR Summary—support referencing real files from the codebases Cody can access. When you save an update, Cody automatically detects external references and resolves them for future reviews.

- Use `@file:path/to/file.ts` to point at files in the repository you're editing; Cody will try to locate the path locally first.
...
- After saving, Cody processes the references in the background. Check the status indicator beside the prompt editor to confirm when resolution completes.
...
<Tabs>
  <Tab title="Good: Meaningful Difference">
    ```
    For Cody Rules violations, use a stricter tone: 
  ...
  <Tab title="Bad: Superficial Label">
    ```
    For Cody Rules, add "[RULE]" prefix before the problem.
...
<AccordionGroup>
  <Accordion title="Industry-Specific Requirements">
    ...
    **Benefit:** Cody catches compliance-specific issues that generic security reviews might miss.
  </Accordion>
  ...
  <Accordion title="Different Tone for Rules vs Suggestions">
    **Scenario:** Cody Rules are strict requirements, suggestions are recommendations
...
<AccordionGroup>
  ...
  <Accordion title="Missing Expected Issues">
    **Symptoms:** Cody doesn't catch issues it should
...
<AccordionGroup>
  ...
  <Accordion title="What happens to existing PRs when I change prompts?">
    Changes only affect new reviews. Re-run with `@cody start-review` to apply new prompts to existing PRs.
  </Accordion>

  <Accordion title="Can I customize prompts with codus-config.yml?">
    No. Custom prompts are web-only to ensure intentional changes. Other settings (ignored paths, branches) can use `codus-config.yml`.
  </Accordion>
  ...
  <Accordion title="What's the difference between Category prompts and Suggestion prompts?">
    **Category prompts** control what Cody analyzes in each review category. **Suggestion prompts** control how Cody presents findings to your team. You can customize both independently.
  </Accordion>
  ...
  <Accordion title="What happens if my suggestion template has syntax errors?">
    Cody will fall back to the default suggestion format and notify you of the error in settings.
  </Accordion>

  <Accordion title="Do suggestion prompts work for both Cody Rules and standard suggestions?">
    Yes. Use the `isCodyRule` variable to create different messages for rules vs suggestions in the same prompt.
  </Accordion>
</AccordionGroup>
