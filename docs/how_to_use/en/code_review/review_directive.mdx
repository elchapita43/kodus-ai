...
description: "Direct Cody's attention to a specific area of a pull request with a one-off review directive, via PR comment or CLI"
---

A **review directive** lets you tell Cody where to focus its deepest analysis for a single review. Instead of changing any configuration, you attach a short instruction when you trigger the review — for example, "focus on the auth and session logic" — and Cody spends its hardest analysis on the changed code that matches.

<Note>
  A directive is **ephemeral**: it applies to that one review run only. It is not saved to your settings and does not affect future reviews. To change Cody's behavior permanently, use [Custom Prompts](/how_to_use/en/code_review/configs/custom_prompts) or [Cody Rules](/how_to_use/en/code_review/configs/cody_rules) instead.
</Note>

## When to Use It

- A PR touches many files but you care most about one risky area (auth, payments, a migration).
- You want Cody to trace the callers and callees of a specific change and challenge it harder.
...
<Warning>
  A directive sets **priority, not a filter**. Cody prioritizes the focus area, but it still reports any concrete bug, security, or performance issue it notices **elsewhere** in the diff. It never suppresses findings outside the focus, and it never approves the rest of the PR unseen.
</Warning>
...
<Tabs>
  <Tab title="PR / MR Comment">
    ...
    ```text
    @cody review focus on the auth and session logic
    ```

    You can also use `start-review`, and combine it with `--force`:

    ```text
    @cody start-review focus on the new database migration
    @cody review --force focus on error handling in the webhook parser
    ```

    <Note>
      `--force` re-runs the review even when Cody would normally skip it ("no new commits since the last review"). Use it — with or without a focus directive — to get a fresh pass after fixing whatever made the previous run fail, or to apply updated settings to an already-reviewed PR.
    </Note>
  ...
  <Tab title="CLI">
    ...
    ```bash
    codus review --focus "the auth and session logic"
    ```
...
A focus directive tells Cody *where* to look hardest. `--heavy` tells it to look *longer*: the
...
```text
@cody review --heavy
@cody review --heavy focus on the payment reconciliation logic
codus review --heavy
```

`--heavy` can sit anywhere on the command line — before or after your focus text — and it is
stripped from the directive either way, so `@cody review focus on the retry logic --heavy`
focuses on "the retry logic".

<Warning>
  **To combine it with `--force`, put `--force` first.** `--force` is only recognized
  immediately after the command word, so `@cody review --force --heavy` runs a forced heavy
  review, while `@cody review --heavy --force` runs a heavy review that is **not** forced.
</Warning>

A dash is required: a bare `heavy` is read as directive text, so `@cody review heavy checkout
...
<AccordionGroup>
  <Accordion title="Good examples">
    - `@cody review focus on the authentication and token-refresh flow`
    - `@cody review focus on the new Stripe webhook handling`
    - `codus review --focus "race conditions in the background worker"`
  </Accordion>
  <Accordion title="Weak examples">
    - `@cody review please be thorough` — no area to prioritize.
    - `@cody review only comment on file X` — a directive is not a filter; use [ignore paths](/how_to_use/en/code_review/configs/general) to scope files instead.
  </Accordion>
</AccordionGroup>
...
- **Untrusted by design:** because anyone who can comment on the PR can supply a directive, the text is sanitized before use (control characters and angle brackets are stripped). This does not change your focus — it only prevents the text from tampering with Cody's internal prompt.
