...
description: "How a Cody review runs end to end: when it triggers, when it skips, what you get back, and what each status emoji means."
---

## Overview

Cody reviews your PRs automatically or on demand, adds clear suggestions, and helps your team decide when to merge. Below is the simple, user-facing view of how it works.

Reference: <a href="https://www.figma.com/board/ZZJcIJMJWGuyzRGMh3w5di/Codus-Code-Review-Flow?node-id=4-274" target="_blank">Figma board</a>

## When Cody Runs

- PR opened
- New commit pushed to the PR (depending on cadence)
- Manual trigger via comment: `@cody start-review`
...
<Note>
  To override a skip and force a fresh pass — for example after "no new commits" — comment `@cody review --force`.
</Note>
...
Cody uses emoji reactions to show the live status of your code review directly in your Git provider. No guessing whether she’s working, done, or ran into a problem. She always replaces the previous emoji with the latest status (for example: 🚀 → 🎉, 👀, 👎 or 😕).

<Note>
  **Native reactions** work on GitHub, GitLab, and Forgejo.

  Azure DevOps and Bitbucket have no reaction API, so Cody falls back to posting the status
  emoji as a **comment** instead — you'll see 👀 (skipped), 😕 (error), and 👎 (no license)
  there, but not the 🚀 / 🎉 progress pair.
</Note>

### What each emoji means

- 🚀 Processing — Cody is working on your code:
  - Analyzing files
...
- 👎 No license — The user is not licensed:
  - Cody cannot run without a valid license
...
- Manual review (comment `@cody start-review`): reaction on your comment where you invoked Cody
...
- GitHub / GitLab / Forgejo: ✅ native reactions
- Azure DevOps / Bitbucket: ❌ no reaction API — look for the status emoji posted as a comment instead

**Why did the emoji change?**

- Normal—Cody replaces previous status (🚀 → 🎉/👀/😕)

**I saw 👀 but wanted a review**

- Likely skipped: no new commits, merge-only, ignored files, or file limit
- Fix: comment `@cody review --force` — a plain `start-review` will skip again if there are no new commits
...
- Retry with `@cody start-review`. If persists, check [Troubleshooting](/how_to_use/en/code_review/troubleshooting) or [Discord](https://discord.gg/TFZBRk9fT6)
...
- Review filters: minimum severity and whether they apply to Cody Rules
...
- Re-run with `@cody start-review` after addressing feedback
