...
description: "Generate the Bitbucket Cloud API Token Cody needs for code review, with the exact permissions to grant."
---

## Introduction

To enable Cody to perform automatic code reviews on Bitbucket Cloud, you need to generate an **API Token**. This token allows Cody to access your repositories, pull requests, webhooks, issues, pipelines etc., with the specific permissions listed below.

---

## Generating the API Token in Bitbucket

1. Open the [Atlassian API tokens page](https://id.atlassian.com/manage-profile/security/api-tokens) directly (or, from your Atlassian profile, navigate to **Security → API tokens**).
2. Click **Create API token with scopes**.
3. Give the token a name (for example `cody_code_review`) and set an expiration date.
4. If prompted, select **Bitbucket** as the application.
5. Select all the required scopes (see below).
6. Review your choices, then click **Create token**.
7. **Copy** the token immediately and store it securely — after creation you will not be able to view it again.

---

## Required Scopes for Cody

The token must include **all** of the following scopes for full functionality with Cody:
...
- Once the token is generated with all the required scopes, paste it into the **Cody integration / automation setup screen** when enabling the Bitbucket integration.
- Make sure the token has `admin:repository:bitbucket` (or equivalent) if Cody needs to configure webhooks or listen for pull request events.
