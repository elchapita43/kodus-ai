---
title: "Codus Review"
description: "Measure how effective Codus's reviews are — and act on it"
---

## What is the Codus Review tab

The **Codus Review** tab answers one question: *is your team acting on what Codus says?*

Unlike the Productivity tab (which measures general delivery metrics like deploy frequency and PR cycle time), Codus Review is about Codus itself — how many suggestions get implemented, which categories and rules land or get ignored, and where the team pushes back.
...
| **Suggestions sent** | How many suggestions Codus delivered (as PR comments) in the period |
...
- Only **delivered** suggestions count (the ones actually posted as PR comments). Drafts Codus filtered out before commenting are never counted.
...
<Warning>
  **The "All / Codus only" toggle on the severity chart matters.** A Cody Rule carries the severity *you* set on the rule, not a risk assessment by Codus. Mixing the two distorts the calibration read — for example, a pile of medium Cody Rules with high adoption can make Codus look like it under-rates medium. Switch to **Codus only** to see Codus's own severity calibration. Bars built on very few suggestions are faded and marked with `*` — a 0% or 100% from a handful of suggestions is not a real signal.
</Warning>

## Negative feedback

Feedback comes from 👍 / 👎 reactions on Codus's suggestion comments.
...
A per-repository table: PRs reviewed, suggestions sent, implementation rate, 👍/👎, and the **weakest category** (the category with the lowest implementation rate in that repo, given a minimum sample). It shows where Codus is landing versus being ignored.

Clicking a row focuses the whole cockpit on that repository (same as picking it in the repository filter).

## Cody Rules — health

How each Cody Rule is performing in the period: triggers, implementation rate, 👍/👎, and a status. Only active rules appear — deleted or inactive ones are excluded since you can't act on them.
...
Filters: repository, category, severity, implementation status, Cody Rule, and free-text search. Each row expands to show the existing vs. suggested code and a link to the PR comment.
