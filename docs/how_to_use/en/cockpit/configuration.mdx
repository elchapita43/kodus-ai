...
| Tab | What it holds |
| --- | --- |
| **Codus Review** | How effective Codus's reviews are — implementation rate, severity calibration, negative feedback, Cody Rules health. |
...
- New metrics ship **visible by default**. If Codus adds a chart, it appears for everyone until
...
- Hiding a metric only affects display. Codus keeps ingesting the underlying data, so turning it
