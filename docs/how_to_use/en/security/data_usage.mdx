...
There is no single fixed model. **Codus runs on Bring Your Own Key (BYOK) by default on every
...
| **Trial / no key connected** | Codus, for a limited number of reviews. | A coding-specialized model we pick, with a smaller open-weights model as fallback. |
| **Enterprise with a Codus-managed key** | Codus, under your Enterprise agreement. | Frontier general-purpose models, selected by us. |

### Supported provider classes

Under BYOK you can point Codus at:
...
<Note>
  **A BYOK review never silently falls back to a Codus-managed model.** If your configured model
...
## What Codus does with your code
...
- Nothing is retained for training by Codus.
...
contract with Codus — it's governed by yours with the provider.**
...
- Retention windows (for abuse monitoring, for example) are set by the provider, not by Codus.
...
<Warning>
  Because you choose the provider, **Codus cannot make data-handling promises on their behalf.**
  If your compliance posture depends on specific guarantees, confirm them with your provider
  before pointing Codus at it.
</Warning>

## Security measures on the Codus side
...
- **Self-hosting**: run the whole stack in your own infrastructure if you don't want code
  leaving your network at all. See [Deploy Codus](/how_to_deploy/en/deploy_codus/generic_vm).

## Self-hosted telemetry

If you run Codus self-hosted, each instance sends one anonymous daily
...
or disable it with `CODUS_TELEMETRY_DISABLED=true`. Full schema, retention
policy, and source code links are documented in
[Anonymous Telemetry](/how_to_deploy/en/deploy_codus/telemetry).
