---
title: "Concepts"
description: "Mental models to get the most out of the Codus CLI."
---
...
- **Personal login** (`codus auth login`) — an individual account. Tokens refresh automatically.
- **Team key** (`codus auth team-key --key codus_xxxxx`) — one shared key for a team, generated in the dashboard. Recommended for AI agents and anywhere you don't want interactive logins.
- **CI/CD token** (`codus auth token`) — a long-lived token for pipelines. Generated from a logged-in machine, used via `CODUS_TOKEN`.

See [Getting Started](getting_started) for the full decision tree.

## Cody Rules

A **Cody Rule** is a structured rule the Codus reviewer applies during every review, scoped to a repository or global to your team.
...
Rules layer on top of the general Codus review — the AI uses them as additional criteria, not replacements.

Manage rules from the dashboard or from the CLI with `codus rules create|update|view`. The CLI is useful when you want rules in version control (generate them from a script, check into a repo, re-apply on provisioning).

## Repository Settings

Every repository in Codus has settings that govern how reviews behave: ignored files, base branch patterns, title filters, feature toggles. These live in the Codus backend and are mirrored anywhere the repo is reviewed (web PRs, CLI reviews, agent loops).

From the CLI:

```bash
codus config repo list                 # See your configured repos
codus config repo show                 # See settings for the current repo
codus config repo setup                # Guided wizard
codus config repo set . reviewEnabled true
codus config repo add-ignore-file . "**/*.generated.ts"
codus config repo open                 # Jump to the web dashboard
```

`codus config repo` and `codus config remote` are aliases for the same command group.

## Centralized Config

Normally, each repository's settings live in Codus. **Centralized Config** lets a team store those settings in a single source-of-truth git repository — every change becomes a pull request against that repo, giving you review and version history on your review configuration itself.
...
Use Centralized Config when your team wants auditable, versioned control over review settings. Keep the default (per-repo settings in Codus) when that ceremony is overkill.

## Business Validation

Where `codus review` asks "is this code good?", `codus pr business-validation` asks "does this diff actually do what the task said to do?". You point it at a Linear/Jira/URL-based task and a diff source (working tree, staged, branch, commit) and it checks the implementation against the task's acceptance criteria.
