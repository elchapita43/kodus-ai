---
title: "AI Agents"
description: "Integrate the Codus CLI with AI coding agents for autonomous review-fix loops."
---
...
For any other agent that can run shell commands and parse structured text, `codus review --prompt-only` works out of the box.
...
1. Installs the `@codus/cli` binary globally.
2. Configures the team key in `~/.codus/config.json`.
3. Deploys Codus skills to every detected agent (Claude Code, Cursor, Codex, Windsurf).
...
<Steps>
  ...
  <Step title="Agent commits and pushes">
    Once the review is clean, the agent commits and pushes. Optionally, install the pre-push hook (`codus hook install`) for an additional safety net on manual pushes.
  </Step>
</Steps>
...
codus review --prompt-only --fail-on error    # general review
codus pr business-validation --task-id KC-1441 --dry-run   # check against task
codus review --prompt-only --fix               # auto-apply fixable issues
git commit -am "fix: KC-1441 address review findings"
```

With the skill installer, Claude Code's Codus skill knows when to invoke each of these — no prompt engineering required on your side.
