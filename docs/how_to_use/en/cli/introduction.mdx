...
Codus CLI reviews your code changes directly from the terminal — catching bugs, security issues, and style violations before they reach a pull request. It's the same Codus engine that runs on your PRs, but pointed at your working tree, staged diff, branch, or commit.
...
Running `codus review` against a working tree change produces a structured report: files affected, issues grouped by severity, suggested fixes you can preview and apply, and an exit code safe for scripting.

<img src="/images/codus_cli_review.jpg" alt="AI code review in your CLI"/>
...
<CardGroup cols={2}>
  ...
  <Card title="Pre-push Hook" icon="shield-check">
    Block pushes on issues above a severity threshold with `codus hook install`.
  </Card>
  <Card title="PR Suggestions" icon="code-pull-request">
    Fetch Cody's suggestions on an open pull request without leaving your terminal.
  </Card>
...
Trial limits reset daily. Run `codus auth status` to check your current usage.

## CLI vs Codus Web

The CLI and the Codus web dashboard share the same review engine, rules, and repository settings. Use the CLI when you want reviews **before the PR exists** — on a local branch, a staged diff, or inside an agent loop. Use the web dashboard for PR reviews, metrics, and team-level configuration.

Changes you make via `codus config repo` and `codus rules` apply to both surfaces.
...
<CardGroup cols={2}>
  ...
  <Card title="Self-Hosted" icon="server" href="self_hosted">
    Connect the CLI to your own Codus instance.
  </Card>
