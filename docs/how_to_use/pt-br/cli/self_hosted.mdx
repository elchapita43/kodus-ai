...
description: "Conecte o Codus CLI à sua instância self-hosted do Codus."
---

Se você estiver executando o Codus na sua própria infraestrutura, o CLI funciona da mesma forma — você só precisa apontá-lo para a sua API.

## Conectar à Sua Instância

<Steps>
  <Step title="Definir a URL da API">
    Informe ao CLI onde sua API do Codus está. Escolha um método:
    ...
    <Tabs>
      <Tab title="Variável de Ambiente (Recomendado)">
        ...
        ```bash
        echo 'export CODUS_API_URL=https://codus-api.suaempresa.com' >> ~/.zshrc
        source ~/.zshrc
        ```
      </Tab>
      <Tab title="Arquivo de Configuração">
        Crie ou edite `~/.codus/config.json`:

        ```json
        {
          "apiUrl": "https://codus-api.suaempresa.com",
          "teamKey": "codus_xxxxx",
          "teamName": "Sua Equipe",
        ...
        O CLI cria esse arquivo automaticamente quando você executa `codus auth team-key`, mas você também pode criá-lo manualmente.
      </Tab>
      <Tab title="Inline (Uso Único)">
        ```bash
        CODUS_API_URL=https://codus-api.suaempresa.com codus review
        ```
      </Tab>
    </Tabs>
    ...
    **Ordem de prioridade:** Variável de ambiente `CODUS_API_URL` > apiUrl em `~/.codus/config.json` > padrão (`https://api.kodus.io`).
  </Step>

  <Step title="Autenticar">
    <Tabs>
      <Tab title="Chave de Equipe (Recomendado)">
        Gere uma chave de equipe no painel do Codus self-hosted e configure-a:
      ...
      <Tab title="Login Pessoal">
        ...
        Insira o e-mail e a senha registrados na sua instância self-hosted. Os tokens são armazenados localmente em `~/.codus/credentials.json` com permissões restritas (600).
      </Tab>
...
O CLI armazena a configuração em `~/.codus/config.json`:
...
|-------|-------------|-----------|
| `apiUrl` | Não | Sua URL de API self-hosted. Substituída pela variável de ambiente `CODUS_API_URL`. |
| `teamKey` | Não | Chave de API da equipe (começa com `codus_`). Substituída pela variável de ambiente `CODUS_TEAM_KEY`. |
...
<Info>
  O arquivo de configuração é criado com permissões `0600` (apenas leitura/escrita pelo proprietário). O diretório `~/.codus/` é criado com permissões `0700`.
</Info>
...
| Variável | Descrição |
|----------|-----------|
| `CODUS_API_URL` | Endpoint da API self-hosted (HTTPS obrigatório, exceto localhost) |
| `CODUS_TEAM_KEY` | Chave de equipe para acesso compartilhado |
| `CODUS_TOKEN` | Token CI/CD para pipelines |
| `CF_ACCESS_CLIENT_ID` | ID do cliente Cloudflare Access |
| `CF_ACCESS_CLIENT_SECRET` | Segredo do cliente Cloudflare Access |
| `CODUS_VERBOSE` | Defina como `true` para ver a URL de API resolvida e detalhes das requisições |
...
```bash
CODUS_API_URL=https://codus-api.suaempresa.com \
  curl -fsSL https://review-skill.com/install | bash -s -- --team-key codus_xxxxx
```
...
<AccordionGroup>
  ...
  <Accordion title="Conexão recusada ou timeout">
    - Verifique se a URL está acessível: `curl -I https://codus-api.suaempresa.com`
  ...
  <Accordion title="Falha de autenticação (401)">
    - Execute `codus auth status` para verificar seu estado de autenticação atual
    ...
    - Certifique-se de que `CODUS_API_URL` aponta para a instância correta
  </Accordion>
