...
Não existe um modelo fixo. **O Codus roda em BYOK (Bring Your Own Key) por padrão em todos os
...
| **Trial / sem chave conectada** | Codus, por um número limitado de revisões. | Um modelo especializado em código escolhido por nós, com um modelo open-weights menor como fallback. |
| **Enterprise com chave gerenciada pela Codus** | Codus, sob o seu contrato Enterprise. | Modelos frontier de propósito geral, selecionados por nós. |

### Classes de provedor suportadas

No BYOK você pode apontar o Codus para:
...
<Note>
  **Uma revisão BYOK nunca cai silenciosamente em um modelo gerenciado pela Codus.** Se o modelo
...
## O que o Codus faz com seu código
...
- Nada é retido pelo Codus para treinamento.
...
a Codus — é regido pelo seu contrato com o provedor.**
...
<Warning>
  Como quem escolhe o provedor é você, **o Codus não pode fazer promessas de tratamento de dados
  em nome dele.** Se sua postura de compliance depende de garantias específicas, confirme com o
  provedor antes de apontar o Codus para ele.
</Warning>

## Medidas de segurança do lado do Codus
...
- **Self-hosting**: rode todo o stack na sua própria infraestrutura se não quiser que o código
  saia da sua rede. Veja [Deploy do Codus](/how_to_deploy/pt-br/deploy_codus/generic_vm).

## Telemetria self-hosted

Se você roda o Codus self-hosted, cada instância envia um heartbeat anônimo diário
...
com `CODUS_TELEMETRY_DISABLED=true`. Schema completo, política de retenção e links
para o código-fonte estão em
[Telemetria Anônima](/how_to_deploy/pt-br/deploy_codus/telemetry).
