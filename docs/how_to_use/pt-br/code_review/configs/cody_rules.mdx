---
title: "Visão Geral"
description: "Cody Rules são diretrizes personalizáveis que sua equipe configura para aplicar automaticamente qualidade de código, consistência, segurança e manutenibilidade."
---
...
## Dois tipos na interface de Cody Rules

As Cody Rules são organizadas em duas categorias, gerenciadas por meio de uma interface com abas nas suas configurações:
...
<Tabs>
  ...
  <Tab title="Memórias">
    Instruções contextuais persistentes que são injetadas em todos os prompts e conversas. As Memórias permitem que Cody aprenda e retenha como seu codebase funciona para que futuras revisões e sugestões sejam mais relevantes.

    - Injetadas como **contexto de alta prioridade** em revisões de código, análise entre arquivos e conversas
    - Criadas via **conversa com Cody** ou **manualmente na interface**
...
- Cody identifica referências a arquivos automaticamente quando você salva uma regra
...
- O conteúdo dos arquivos é injetado no contexto da regra, permitindo que Cody compare, valide ou aplique padrões
...
- Navegue até Descoberta de Regras no seu painel Codus.
...
- **Melhores sugestões futuras**: As Memórias ajudam o Cody a parar de repetir as mesmas suposições erradas sobre seu repositório
- **Deduplicação inteligente**: Quando uma nova memória é criada, Cody usa um mecanismo de resolução baseado em LLM para determinar se deve criá-la, ignorá-la (se duplicada) ou atualizar uma memória existente
...
A forma mais natural de criar memórias é por meio de conversa com Cody nos comentários do PR. Cody detecta intenções explícitas e implícitas de salvar convenções:

**Explícito** — pedindo diretamente para Cody lembrar:
...
**Implícito** — declarando uma preferência ou convenção que Cody captura:
...
<Info>
  Cody NÃO criará memórias para instruções transitórias (ex.: "corrija isso agora"), conversas de debug, perguntas, declarações vagas ou solicitações explicitamente limitadas a uma única tarefa ou PR.
</Info>

Quando uma memória é criada ou atualizada, Cody responde com uma confirmação e fornece um **link direto** para visualizar a memória na interface.

#### Via Interface

Você também pode criar memórias manualmente:

1. Acesse **Configurações de Code Review** → **Repositório** → **Cody Rules**
...
1. Acesse **Configurações de Code Review** → **Repositório** → **Cody Rules** → aba **Memórias**
...
Você também pode definir isso em `codus-config.yml`:
...
<CardGroup cols={3}>
  ...
  <Card title="Geração por IA" icon="sparkles" href="/how_to_use/pt-br/code_review/learning/cody_rules_generation">
