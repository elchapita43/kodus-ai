...
description: "Como uma revisão do Cody roda de ponta a ponta: quando dispara, quando pula, o que você recebe de volta e o que cada emoji de status significa."
---

## Visão Geral

Cody revisa seus PRs automaticamente ou sob demanda, adiciona sugestões claras e ajuda sua equipe a decidir quando fazer o merge. Abaixo está a visão simples e orientada ao usuário de como funciona.

Referência: <a href="https://www.figma.com/board/ZZJcIJMJWGuyzRGMh3w5di/Codus-Code-Review-Flow?node-id=4-274" target="_blank">Quadro no Figma</a>

## Quando Cody Executa

- PR aberto
- Novo commit enviado ao PR (dependendo da cadência)
- Acionamento manual via comentário: `@cody start-review`
...
<Note>
  Para ignorar um skip e forçar uma nova análise — por exemplo após "sem novos commits" — comente `@cody review --force`.
</Note>
...
Cody usa reações com emoji para mostrar o status em tempo real do seu code review diretamente no seu provedor Git. Sem precisar adivinhar se ela está trabalhando, concluída ou encontrou um problema. Ela sempre substitui o emoji anterior pelo status mais recente (por exemplo: 🚀 → 🎉, 👀, 👎 ou 😕).

<Note>
  **Reações nativas** funcionam no GitHub, GitLab e Forgejo.

  Azure DevOps e Bitbucket não têm API de reações, então o Cody publica o emoji de status
  como **comentário** — lá você vê 👀 (pulado), 😕 (erro) e 👎 (sem licença), mas não o par
  de progresso 🚀 / 🎉.
</Note>

### O que cada emoji significa

- 🚀 Processando — Cody está trabalhando no seu código:
  - Analisando arquivos
...
- 👎 Sem licença — O usuário não está licenciado:
  - Cody não pode executar sem uma licença válida
...
- Revisão manual (comentário `@cody start-review`): reação no seu comentário onde você invocou Cody
...
- GitHub / GitLab / Forgejo: ✅ reações nativas
- Azure DevOps / Bitbucket: ❌ sem API de reações — procure o emoji de status publicado como comentário

**Por que o emoji mudou?**

- Normal — Cody substitui o status anterior (🚀 → 🎉/👀/😕)

**Vi 👀 mas queria uma revisão**

- Provavelmente pulado: sem novos commits, apenas merge, arquivos ignorados ou limite de arquivos
- Solução: comente `@cody review --force` — um `start-review` simples vai pular de novo se não houver commits novos
...
- Tente novamente com `@cody start-review`. Se persistir, verifique [Solução de Problemas](/how_to_use/pt-br/code_review/troubleshooting) ou o [Discord](https://discord.gg/TFZBRk9fT6)
...
- Filtros de revisão: severidade mínima e se valem para as Cody Rules
...
- Execute novamente com `@cody start-review` após abordar o feedback
