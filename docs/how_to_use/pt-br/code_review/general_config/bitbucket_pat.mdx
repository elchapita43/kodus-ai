...
description: "Gere o API Token do Bitbucket Cloud que o Cody precisa para o code review, com as permissões exatas a conceder."
---

## Introdução

Para habilitar Cody a realizar code reviews automáticos no Bitbucket Cloud, você precisa gerar um **Token de API**. Esse token permite que Cody acesse seus repositórios, pull requests, webhooks, issues, pipelines etc., com as permissões específicas listadas abaixo.
...
3. Dê um nome ao token (por exemplo `cody_code_review`) e defina uma data de expiração.
...
## Escopos Necessários para Cody

O token deve incluir **todos** os seguintes escopos para funcionalidade completa com Cody:
...
- Uma vez que o token for gerado com todos os escopos necessários, cole-o na **tela de configuração de integração/automação do Cody** ao habilitar a integração com Bitbucket.
- Certifique-se de que o token tem `admin:repository:bitbucket` (ou equivalente) se Cody precisar configurar webhooks ou ouvir eventos de pull request.
