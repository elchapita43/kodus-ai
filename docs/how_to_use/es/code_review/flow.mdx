...
description: "Cómo se ejecuta una revisión de Cody de principio a fin: cuándo se dispara, cuándo se omite, qué recibes y qué significa cada emoji de estado."
---

## Descripción General

Cody revisa tus PRs automáticamente o bajo demanda, agrega sugerencias claras y ayuda a tu equipo a decidir cuándo hacer el merge. A continuación se muestra la vista simple orientada al usuario sobre cómo funciona.

Referencia: <a href="https://www.figma.com/board/ZZJcIJMJWGuyzRGMh3w5di/Codus-Code-Review-Flow?node-id=4-274" target="_blank">Tablero en Figma</a>

## Cuándo Ejecuta Cody

- PR abierto
- Nuevo commit subido al PR (dependiendo de la cadencia)
- Activación manual vía comentario: `@cody start-review`
...
<Note>
  Para anular una omisión y forzar un nuevo análisis — por ejemplo tras "sin nuevos commits" — comenta `@cody review --force`.
</Note>
...
Cody usa reacciones con emojis para mostrar el estado en vivo de tu revisión de código directamente en tu proveedor de Git. Sin necesidad de adivinar si está trabajando, terminó o tuvo un problema. Siempre reemplaza el emoji anterior con el estado más reciente (por ejemplo: 🚀 → 🎉, 👀, 👎 o 😕).

<Note>
  **Las reacciones nativas** funcionan en GitHub, GitLab y Forgejo.

  Azure DevOps y Bitbucket no tienen API de reacciones, así que Cody publica el emoji de estado
  como **comentario** — ahí verás 👀 (omitido), 😕 (error) y 👎 (sin licencia), pero no el par
  de progreso 🚀 / 🎉.
</Note>

### Qué significa cada emoji

- 🚀 Procesando — Cody está trabajando en tu código:
  - Analizando archivos
...
- 👎 Sin licencia — El usuario no tiene licencia:
  - Cody no puede ejecutarse sin una licencia válida
...
- Revisión manual (comentario `@cody start-review`): reacción en tu comentario donde invocaste a Cody
...
- GitHub / GitLab / Forgejo: ✅ reacciones nativas
- Azure DevOps / Bitbucket: ❌ sin API de reacciones — busca el emoji de estado publicado como comentario

**¿Por qué cambió el emoji?**

- Normal — Cody reemplaza el estado anterior (🚀 → 🎉/👀/😕)

**Vi 👀 pero quería una revisión**

- Probablemente omitido: sin nuevos commits, solo merge, archivos ignorados o límite de archivos
- Solución: comenta `@cody review --force` — un `start-review` simple volverá a omitirse si no hay commits nuevos
...
- Reintenta con `@cody start-review`. Si persiste, consulta [Solución de Problemas](/how_to_use/es/code_review/troubleshooting) o [Discord](https://discord.gg/TFZBRk9fT6)
...
- Filtros de revisión: severidad mínima y si se aplican a las Cody Rules
...
- Vuelve a ejecutar con `@cody start-review` después de abordar el feedback
