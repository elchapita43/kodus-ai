...
description: "Genera el API Token de Bitbucket Cloud que Cody necesita para la revisión de código, con los permisos exactos a otorgar."
---

## Introducción

Para permitir que Cody realice revisiones de código automáticas en Bitbucket Cloud, necesitas generar un **Token API**. Este token permite a Cody acceder a tus repositorios, pull requests, webhooks, issues, pipelines, etc., con los permisos específicos listados a continuación.
...
3. Dale un nombre al token (por ejemplo `cody_code_review`) y establece una fecha de vencimiento.
...
## Alcances Requeridos para Cody

El token debe incluir **todos** los siguientes alcances para la funcionalidad completa con Cody:
...
- Una vez que el token esté generado con todos los alcances requeridos, pégalo en la **pantalla de configuración de integración/automatización de Cody** al habilitar la integración con Bitbucket.
- Asegúrate de que el token tenga `admin:repository:bitbucket` (o equivalente) si Cody necesita configurar webhooks o escuchar eventos de pull request.
