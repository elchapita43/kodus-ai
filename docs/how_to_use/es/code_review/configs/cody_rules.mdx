...
description: "Las Cody Rules son pautas personalizables que tu equipo configura para aplicar automáticamente calidad de código, consistencia, seguridad y mantenibilidad."
---
...
## Dos tipos en la interfaz de Cody Rules

Las Cody Rules están organizadas en dos categorías, gestionadas a través de una interfaz con pestañas en tu configuración:
...
<Tabs>
  ...
  <Tab title="Memorias">
    Instrucciones contextuales persistentes que se inyectan en todos los prompts y conversaciones. Las Memorias permiten a Cody aprender y retener cómo funciona tu código para que las futuras revisiones y sugerencias sean más relevantes.

    - Se inyectan como **contexto de alta prioridad** en revisiones de código, análisis entre archivos y conversaciones
    - Se crean vía **conversación con Cody** o **manualmente en la UI**
...
- Cody identifica las referencias a archivos automáticamente cuando guardas una regla
...
- El contenido de los archivos se inyecta en el contexto de la regla, permitiendo a Cody comparar, validar o aplicar patrones
...
- Navega a Reglas de Descubrimiento en tu panel de Codus.
...
- **Mejores sugerencias futuras**: Las Memorias ayudan a Cody a no repetir las mismas suposiciones equivocadas sobre tu repositorio
- **Deduplicación inteligente**: Cuando se crea una nueva memoria, Cody usa un mecanismo de resolución basado en LLM para determinar si crearla, omitirla (si es duplicada) o actualizar una memoria existente
...
La forma más natural de crear memorias es a través de la conversación con Cody en los comentarios del PR. Cody detecta tanto intenciones explícitas como implícitas de guardar convenciones:

**Explícita** — pedir directamente a Cody que recuerde:

```
@cody remember: este repositorio refleja una API de terceros, así que algunos campos del payload se mantienen intencionalmente en snake_case.
```

```
@cody please remember: este servicio sigue arquitectura hexagonal y deja los adapters en los bordes.
```

**Implícita** — expresar una preferencia o convención que Cody captura:

```
@cody este repositorio refleja una API de terceros, así que algunos campos del payload se mantienen intencionalmente en snake_case.
```

```
@cody en este servicio, las pruebas suelen vivir junto a los archivos de implementación y no en una carpeta central de tests.
```

```
@cody el módulo de facturación está a mitad de una migración, así que convienen cambios incrementales y no refactors amplios.
```

<Info>
  Cody NO creará memorias para instrucciones transitorias (p. ej., "arregla esto ahora"), conversaciones de depuración, preguntas, declaraciones vagas o solicitudes explícitamente limitadas a una sola tarea o PR.
</Info>

Cuando se crea o actualiza una memoria, Cody responde con una confirmación y proporciona un **enlace directo** para ver la memoria en la UI.

#### Vía la UI

También puedes crear memorias manualmente:

1. Ve a **Configuración de Code Review** → **Repositorio** → **Cody Rules**
...
1. Ve a **Configuración de Code Review** → **Repositorio** → **Cody Rules** → pestaña **Memorias**
...
También puedes configurar esto en `codus-config.yml`:
...
<CardGroup cols={3}>
  ...
  <Card title="Generación con IA" icon="sparkles" href="/how_to_use/es/code_review/learning/cody_rules_generation">
