...
Con el backend vinculado, Cody comprueba el otro lado de ese contrato y comenta en el PR:
...
<Info>
- Solo se pueden vincular repositorios **ya conectados a la misma organización de Codus** (Cody reutiliza tu integración de Git existente — sin tokens adicionales).
- Hasta **3 repositorios vinculados** por repositorio.
- El acceso es de solo lectura. Cody nunca hace push, comenta ni modifica un repositorio vinculado.
...
Controla qué rama ("ref") del repositorio vinculado lee Cody. **La mayoría de los equipos lo deja vacío**: Cody entonces lee la rama con el mismo nombre que la de tu PR cuando existe (para que los cambios coordinados entre repositorios se vean antes del merge), y recurre a la rama por defecto en caso contrario.
