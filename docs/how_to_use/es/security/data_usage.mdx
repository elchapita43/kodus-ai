...
No hay un modelo fijo. **Codus funciona con BYOK (Bring Your Own Key) por defecto en todos los
...
| **Trial / sin clave conectada** | Codus, por un número limitado de revisiones. | Un modelo especializado en código elegido por nosotros, con un modelo open-weights más pequeño como fallback. |
| **Enterprise con clave gestionada por Codus** | Codus, bajo tu acuerdo Enterprise. | Modelos frontier de propósito general, seleccionados por nosotros. |

### Clases de proveedor soportadas

Con BYOK puedes apuntar Codus a:
...
<Note>
  **Una revisión BYOK nunca recurre en silencio a un modelo gestionado por Codus.** Si el modelo
...
- Codus no retiene nada para entrenamiento.
...
contrato con Codus — se rige por el tuyo con el proveedor.**
...
- Las ventanas de retención (para monitoreo de abuso, por ejemplo) las define el proveedor, no Codus.
...
<Warning>
  Como tú eliges el proveedor, **Codus no puede hacer promesas de tratamiento de datos en su
  nombre.** Si tu postura de cumplimiento depende de garantías específicas, confírmalas con tu
  proveedor antes de apuntar Codus hacia él.
</Warning>

## Medidas de seguridad del lado de Codus
...
- **Self-hosting**: ejecuta todo el stack en tu propia infraestructura si no quieres que el
  código salga de tu red. Consulta [Desplegar Codus](/how_to_deploy/es/deploy_codus/generic_vm).

## Telemetría self-hosted

Si ejecutas Codus self-hosted, cada instancia envía un heartbeat anónimo diario a
...
`CODUS_TELEMETRY_DISABLED=true`. El esquema completo, la política de retención y los
