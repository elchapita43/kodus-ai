...
在审查选项中设置 `business_logic: true`（默认启用），当连接了任务管理插件时，Codus 在每次 PR 审查期间自动验证业务规则。
...
通过[插件](/how_to_use/zh/code_review/plugins)连接以自动获取任务上下文：
...
- 具有明确验收标准的任务获得最全面的验证
- 修复问题后，重新运行命令以确认合规
- 如果 PR 范围与任务不匹配，Cody 会标记范围不匹配而不是推测
