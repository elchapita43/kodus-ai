...
## How it works with Codus

Codus lets you define **Cody Rules** — custom instructions that are checked automatically during every code review. There are two types:

- **Review Rules** run during code review and analyze file diffs or the entire PR against your standards
- **Memories** are persistent conventions that Cody applies across all interactions — code reviews, conversations, and suggestions
...
- **Repository rules** — define rules in markdown files directly in your repo (`codus/rules/`)
- **Auto-generating rules** — let Cody analyze 3 months of review history and suggest rules based on your team's patterns
...
4. Cody will enforce it on every future PR

For more details, see the [Cody Rules documentation](/how_to_use/en/code_review/configs/cody_rules).
