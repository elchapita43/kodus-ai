...
description: "Learn how to connect Synthetic.new's subscription-based API — bundling Kimi, DeepSeek, Qwen Coder, GLM and more — to Codus."
icon: "infinity"
---
...
Codus reaches Synthetic through the OpenAI-compatible endpoint, so there are no adapter changes — just BYOK credentials.
...
## Configure Synthetic in Codus

The primary flow is BYOK on Codus Cloud. Self-hosted installs can use environment variables as a secondary option.

### Option 1 — BYOK on Codus Cloud (recommended)

1. In the Codus web UI, open **Settings → BYOK** ([app.kodus.io/organization/byok](https://app.kodus.io/organization/byok)).
...
4. Click **Test & save**. Codus probes the endpoint and persists the config on success. 401 means the key is wrong; 404 usually means the base URL or model path is wrong.
...
<Note>
  Configure Synthetic as **Main** and keep an OpenAI or Anthropic key as **Fallback** — when the 5-hour window fills up, Codus automatically fails over and reviews keep flowing.
</Note>

### Option 2 — Self-hosted (environment variables)

If you run Codus in Fixed Mode (single global provider, no per-org BYOK), configure Synthetic in the `.env` of your API + worker containers:
...
<Note>
  This path is only needed for self-hosted Codus installs that deliberately disable BYOK. If BYOK is enabled on your self-hosted instance, prefer **Option 1** — the UI-based flow is the same as on Cloud.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Long cold-start on first call">
    ...
    - If first-call latency is blocking Codus health checks, warm the model by issuing a tiny test request before opening PRs.
  </Accordion>
