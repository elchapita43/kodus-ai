...
A separate Node process running the **same image as `worker`** (`codus-ai-worker`),
...
```yaml
worker-analytics:
    image: ghcr.io/elchapita43/codus-ai-worker:latest
    platform: linux/amd64
    container_name: codus-worker-analytics
    environment:
        - ENV=production
        - NODE_ENV=production
...
ANALYTICS_PG_DB_DATABASE=codus_analytics
ANALYTICS_PG_DB_SCHEMA=analytics
...
the analytics warehouse migrations (`pnpm run analytics:migration:run:prod`)
