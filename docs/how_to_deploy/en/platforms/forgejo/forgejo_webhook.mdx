...
description: "Manually create the Forgejo webhook when automatic registration didn't happen, and select the events Codus consumes."
icon: webhook
---
...
<Note>
  Codus consumes pull request, push, pull request review comment, and issue comment events.
  Push events matter on Forgejo specifically: they are the only event carrying both the previous
  and current branch heads, which is how Codus detects force-pushes and maps a branch back to its
