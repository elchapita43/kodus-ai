...
description: "Manually register the Azure Repos webhook when Cody couldn't create the service hook for you, including the signed token the URL requires."
icon: webhook
---

Cody usually creates the webhook automatically when you connect an Azure DevOps project. This page covers the **manual setup** path you take when automatic registration didn't happen — typically on self-hosted installs or in Azure projects where Cody can't create service hooks on your behalf.
...
```
https://codus-api.yourdomain.com/azure-repos/webhook?token=<generated>
```
...
<Note>
  Cody generates this token automatically when it creates the webhook for you via the UI-driven integration flow. You only need to generate it by hand when you're registering the webhook manually in Azure DevOps.
</Note>

## Generating the webhook token

Pick the option that matches your environment. Both use the env vars that your Codus stack already has configured — you do **not** need to clone the repository.

### Option A — inside the running Codus container (recommended)
...
If your Codus stack is not running or you want to generate the token on a different machine, export the two env vars and run:
...
<Warning>
  Both env var values must match **exactly** the ones your Codus API container is using. A key mismatch produces a valid-looking token that Codus will reject with `403 Unauthorized`.
</Warning>
...
5. Configure the webhook:
   ...
   - **URL:** your Codus Azure Repos webhook URL with the token appended:
...
Create one subscription per event type. Codus currently reacts to:
...
<AccordionGroup>
  <Accordion title="Azure sends the webhook but Codus returns 403">
