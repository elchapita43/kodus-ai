...
description: "Cómo Codus parsea tu repo (codus-graph), dónde se ejecuta (modos de sandbox), y cuándo elegir cada modo."
icon: "box"
---

Para revisar un PR con el contexto correcto, Codus parsea el repositorio
...
Codus usa [`@codus/codus-graph`](https://www.npmjs.com/package/@codus/codus-graph),
...
imagen del worker** (`codus-ai-worker`) — en modo `local` no hay setup
adicional.

Ciclo de vida:

1. **Cuando seleccionas un repositorio** (en el setup, o al agregar un
   repo después en la UI) — Codus encola un job `AstGraphBuild` que
   ejecuta `codus-graph parse --all` en el branch por defecto, persiste
...
3. **En cada review de PR** — Codus carga el graph cacheado, parsea
...
|---|---|---|
| **`local`** _(default del installer)_ | Ejecuta `codus-graph` dentro del propio contenedor `worker`. Bun + el CLI vienen preinstalados. | Sin dependencia externa. El worker necesita más RAM en repos grandes. |
...
```bash
# Desde el directorio codus-installer, con el stack corriendo:
...
puedes seguir usando Codus mientras terminan.
...
flowchart LR
    ...
    worker -->|modo local| sandbox_local[CLI codus-graph<br/>dentro del contenedor worker]
    worker -.->|modo e2b| sandbox_e2b[Sandbox remoto E2B<br/>codus-graph corre allí]
...
El sandbox `local` ejecuta `git clone` y `codus-graph parse` dentro del
