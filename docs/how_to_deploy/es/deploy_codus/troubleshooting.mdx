...
description: "Resuelve los problemas más comunes con Codus."
icon: "headset"
---
...
```bash
docker-compose ps codus-web
```

2. Verifica que los puertos estén correctamente mapeados:

```bash
docker-compose port codus-web 3000
```

3. Revisa los logs del contenedor web:

```bash
docker-compose logs codus-web
```

### "Errores de Edge Runtime en los logs de `codus-web`"
...
1. Actualiza a la imagen más reciente de `codus-web` (vuelve a ejecutar el instalador o ejecuta `docker-compose pull codus-web` y `docker-compose up -d`).
...
```bash
docker-compose ps db_codus_postgres
docker-compose ps db_codus_mongodb
```

2. Revisa los logs de la base de datos:

```bash
docker-compose logs db_codus_postgres
docker-compose logs db_codus_mongodb
```

3. Prueba las conexiones a la base de datos:

```bash
docker-compose exec db_codus_postgres psql -U $API_PG_DB_USERNAME -d $API_PG_DB_DATABASE
docker-compose exec db_codus_mongodb mongosh -u $API_MG_DB_USERNAME -p $API_MG_DB_PASSWORD
```
...
- [Issues en GitHub](https://github.com/elchapita43/codus-installer/issues)
