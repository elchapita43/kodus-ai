...
description: "公式 Helm チャートで Codus を Kubernetes または OpenShift にデプロイします。"
icon: "dharmachakra"
---

単一の VM よりもクラスターを使いたい場合は、Codus が
[codus-installer](https://github.com/elchapita43/codus-installer) リポジトリで
...
`imageTag` はスタック全体の Codus バージョンを固定します（Docker Compose の `IMAGE_TAG`
...
helm install codus . -f values.yaml -f values-openshift.yaml \
  -n codus --create-namespace --set imageTag=<バージョン>
```
...
- **ヘルスチェック** — `../../scripts/doctor-k8s.sh -n codus` がワークロード、設定、実際の
  ヘルスエンドポイントを検査します。

<Card title="Helm の完全なリファレンス" icon="github" href="https://github.com/elchapita43/codus-installer/blob/main/charts/README.md">
