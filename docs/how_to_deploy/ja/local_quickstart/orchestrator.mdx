---
title: "ローカルクイックスタート"
description: "ローカルマシンでCodusを起動する — API、Worker、Webhooks、Web UI、およびすべての依存関係を1つのコマンドで。"
icon: "laptop"
---

<Note>
  このガイドはローカル開発を目的としています。本番環境へのデプロイについては、
  [Codusのデプロイ](/how_to_deploy/ja/deploy_codus/generic_vm)ガイドを参照してください。
</Note>
...
<Note>
  ...
  `CODUS_TELEMETRY_DISABLED=true` を設定してください。送信される内容と
  確認方法は [匿名テレメトリ](/how_to_deploy/ja/deploy_codus/telemetry)
  を参照してください。
</Note>
...
- ✅ **サービス**: Codus API、Worker、Webhooks、Web、PostgreSQL、MongoDB、RabbitMQ
...
- Codusを起動する前にこれらのポートを使用している他のサービスを停止
