---
title: GitHub Appの作成
description: "Cody がリポジトリを読み、プルリクエストにコメントし、Webhook を受け取るための GitHub App を作成する手順。付与すべき権限も明記します。"
icon: github
---

CodyをGitHub Appで実行する場合は、アプリを作成する必要があります。このアプリにより、Codyはリポジトリとやり取りし、プルリクエストを管理し、Webhookを受信できるようになります。
...
- **WEBHOOK_DOMAIN**（オプション）: Webhook用のパブリックドメイン（例：`https://codus-webhooks.yourdomain.com`）。APIドメインで`/.../webhook`をルーティングする場合は`API_DOMAIN`を使用できます。
...
- オプション**Redirect on update**を有効にする
  これにより、アプリのインストール後にユーザーがCodusにリダイレクトされてセットアップを完了できるようになります。

## ステップ3：Webhookの設定

WebhookはCodusにプルリクエストのイベントを通知します。
...
GitHubが生成した認証情報を取得し、Codusの環境変数に追加する必要があります。
...
GitHubで**Save Changes**をクリックし、Codusアプリケーションを再起動して変更を適用してください。
