---
title: "本地快速开始"
description: "在本地计算机上运行 Codus — API、Worker、Webhooks、Web UI 和所有依赖项，一条命令全部启动。"
icon: "laptop"
---

<Note>
  本指南用于本地开发目的。对于生产部署，请参阅[部署 Codus](/how_to_deploy/zh/deploy_codus/generic_vm) 指南。
</Note>

<Note>
  本地开发使用 `API_CLOUD_MODE=false` 运行 — 与自托管实例相同的模式 —
  这意味着到 `telemetry.kodus.io` 的每日匿名心跳默认启用。如果您不希望
  本地实验出现在集群中,请在 `.env` 中设置 `CODUS_TELEMETRY_DISABLED=true`。
  请参阅 [匿名遥测](/how_to_deploy/zh/deploy_codus/telemetry) 了解发送
  内容和检查方式。
</Note>
...
5. **健康检查**：`pnpm run dev:health-check`
...
此综合健康检查验证：
- ✅ **服务**：Codus API、Worker、Webhooks、Web、PostgreSQL、MongoDB、RabbitMQ
