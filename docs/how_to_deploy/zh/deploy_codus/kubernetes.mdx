...
description: "使用官方 Helm charts 在 Kubernetes 或 OpenShift 上部署 Codus。"
icon: "dharmachakra"
---

相比单台虚拟机更倾向于使用集群？Codus 在
[codus-installer](https://github.com/elchapita43/codus-installer) 仓库中提供了适用于
...
`imageTag` 为整个技术栈固定 Codus 版本（类似 Docker Compose 的 `IMAGE_TAG`）；生产环境
...
helm install codus . -f values.yaml -f values-openshift.yaml \
  -n codus --create-namespace --set imageTag=<版本>
```
...
- **健康检查** — `../../scripts/doctor-k8s.sh -n codus` 会检查工作负载、配置以及真实的健康端点。

<Card title="完整 Helm 参考" icon="github" href="https://github.com/elchapita43/codus-installer/blob/main/charts/README.md">
