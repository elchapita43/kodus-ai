...
```
CLI (codus review) → POST /cli/review → ExecuteCliReviewUseCase → Pipeline com 5 stages:
...
- prompt: sim, plz. Vc sbae como é pra rodar local? CODUS_API_URL se eu não me engano é a variavel.
...
```bash
CODUS_API_URL=http://localhost:3001 codus review -b main
```

Depois de rodar, vê os logs com:

```bash
docker logs codus_worker --tail 200 2>&1 | grep "\[TIMING\]"
```

Ou se o safeguard rodar no container da API:

```bash
docker logs codus_api --t...
...
```bash
CODUS_API_URL=http://localhost:3001 codus review -b main libs/code-review/infrastructure/adapters/services/safeguardPipeline.service.ts
```
...
```bash
codus review libs/code-review/infrastructure/adapters/services/safeguardPipeline.service.ts libs/code-review/infrastructure/adapters/services/safeguardTriage.service.ts libs/common/utils/langchainCommon/prompts/code...
...
```bash
CODUS_API_URL=http://localhost:3001 codus review libs/code-review/infrastructure/adapters/services/safeguardPipeline.service.ts libs/code-review/infrastructure/adapters/services/safeguardTriage.service.ts libs/common/utils/langchainCommon/prompts/codeReviewSafeguardVerif...
...
<output-file>/private/tmp/claude-501/-Users-gabrielmalinosqui-dev-codus-codus-ai/tasks/bvev6cmhy.output</output-file>
<status>completed</status>
<summary>Background command "Run codus review pointing to local API" completed (exit code 0)</summary>
</task-notification>
Read the output file to retrieve the result: /private/tmp/claude-501/-Users-gabrielmalinosqui-dev-codus-codus-ai/tasks/bvev6cmhy.output
...
### [deferral] Quer tentar rodar sem filtrar arquivos (`codus review` puro pra pegar todo o diff)
- **Rationale:** Quer tentar rodar sem filtrar arquivos (`codus review` puro pra pegar todo o diff)
...
<output-file>/private/tmp/claude-501/-Users-gabrielmalinosqui-dev-codus-codus-ai/tasks/b68joy6zv.output</output-file>
<status>completed</status>
<summary>Background command "Check if local backend is running" completed (exit code 0)</summary>
</task-notification>
Read the output file to retrieve the result: /private/tmp/claude-501/-Users-gabrielmalinosqui-dev-codus-codus-ai/tasks/b68joy6zv.output
...
- `memories` — cody rules (regras de memória do time)
