/**
 * Export cody-rules from a Mongo instance into the gitignored prod-rules.json
 * that backfill-shipped.js / prod-rules-judge-eval.js read.
 *
 * RUN IT YOURSELF (you hold the prod credential; this is customer data):
 *   MONGO_URI='mongodb+srv://…' node evals/cody-rules/pull-prod-rules.js [--limit=1000] [--collection=codyRules] [--db=codus]
 *
...
const COLLECTION = args.collection || 'codyRules';
...
(async () => {
    ...
    try {
        ...
        console.log(
            `\nprod-rules.json is gitignored — do NOT commit it. Now run:\n  node evals/cody-rules/backfill-shipped.js --model=gpt-5.4-mini`,
        );
    } finally {
        await client.close();
    }
