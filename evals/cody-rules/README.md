# Cody-rules adherence eval

Does the review correctly enforce team Cody Rules? The cody-rules agent is a
separate agent from the bug/security finder, so this eval is its own thing.

It runs the **production engine** — the real `CodyRulesAgentProvider.execute()`,
...
```bash
yarn eval:cody-rules                 # real engine over the 14 frozen GitHub PRs
yarn eval:cody-rules:gate            # same, but exit non-zero if below baseline (CI)
yarn eval:cody-rules:bench           # regenerate + run the benchmark-diff tier
yarn eval:cody-rules:harvest         # refresh github-cases.json from live GitHub
```

Flags: `--model=` (`gpt-5.4` | `gpt-5.4-mini` | DO models), `--runs=`,
`--gate` with `--occ-min=70 --spec-min=95`. Needs the model key in
`~/.codus-dev/config` (or `.env`). Loads `.ts` via an **esbuild** require-hook,
...
(`cody-rules-agent.provider.ts`, "report EVERY occurrence") was validated on the
