// Cody-rules adherence eval — REAL ENGINE (not a prototype).
//
// Runs the production `CodyRulesAgentProvider.execute()` end-to-end: the same
...
//
//   node evals/cody-rules/real-agent.js [--runs=3] [--model=gpt-5.4-mini]
//
...
// cody-targets.json (occMin = observed − ~2·binomial-SE). --occ-min=/--spec-min=
...
const CODY_TARGETS = (() => {
    ...
    try {
        return require('./cody-targets.json').models || {};
    } catch (err) {
...
const MODEL_TARGET = CODY_TARGETS[MODELKEY] || {};
...
// PRODUCTION, where formatCodyRules puts every applicable rule in ONE agentic
...
async function runCase(provider, c, changedFiles, replay) {
    ...
    const out = await provider.execute({
        ...
        prBody: c.body || '',
        codyRules: rulesForRun.map((r) => ({ ...r, type: 'standard', status: 'active', scope: r.scope || 'file' })),
    });
...
async function main() {
    ...
    for (const c of cases) {
        ...
        for (let r = 0; r < RUNS; r++) {
            ...
            if (targets) {
                ...
                // rules via brokenCodyRulesIds (mapAgentFindings re-homes the
                ...
                for (const t of targets) {
                    const tSugg = sugg.filter((s) =>
                        (s.brokenCodyRulesIds || []).includes(t.uuid) || s.ruleUuid === t.uuid);
            ...
            if (ALL_RULES) {
                ...
                sugg = sugg.filter((s) =>
                    (s.brokenCodyRulesIds || []).includes(c.rule.uuid) ||
    ...
    console.log(`\n════ cody-rules adherence — REAL ENGINE (${MODELKEY}, ${RUNS} runs/case) ════`);
