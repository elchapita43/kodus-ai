// FULL-PIPELINE runner: drives the REAL CodyRulesAgentProvider.execute() —
...
// PRODUCT path (CodyRuleSummaryService) is fully usage-span wrapped.
//   node evals/cody-rules/run-full-pipeline.js --model=glm-5.2 \
...
if (process.env.HOME) dotenv.config({ path: path.join(process.env.HOME, '.codus-dev/config'), override: true });
...
function buildProvider(byokConfig) {
    const { CodyRulesAgentProvider } = require(path.join(__dirname, '../../libs/code-review/infrastructure/agents/providers/cody-rules-agent.provider.ts'));
    ...
    return new CodyRulesAgentProvider({}, permissionValidationService, observabilityService);
}
...
async function main() {
    ...
    // --summary: exercise the PRODUCTIZED CodyRuleSummaryService (the same
    ...
    if (args.atoms) {
        ...
        const { CodyRuleSummaryService } = require(path.join(__dirname, '../../libs/codyRules/infrastructure/adapters/services/cody-rule-summary.service.ts'));
        ...
        const summaryService = new CodyRuleSummaryService(permStub, repoStub, obsStub);
        prepareRules = async (rules) => {
    ...
    for (const c of cases) {
        ...
        if (args.dump) {
            fs.appendFileSync(args.dump, JSON.stringify({
                ...
                flags: sugg.map((s) => ({ file: s.relevantFile, line: s.relevantLinesStart, uuids: s.brokenCodyRulesIds || (s.ruleUuid ? [s.ruleUuid] : []) })),
            }) + '\n');
        }
        ...
        const prCited = [...new Set(sugg.flatMap((s) => s.brokenCodyRulesIds || (s.ruleUuid ? [s.ruleUuid] : [])).filter((u) => prScopeUuids.has(u)))];
        ...
        const flags = sugg.map((s) => ({ file: normalizePath(s.relevantFile), line: s.relevantLinesStart, uuids: s.brokenCodyRulesIds || (s.ruleUuid ? [s.ruleUuid] : []) }))
