/**
 * BEHAVIORAL test of the SHIPPED sharded cody-rules path (#1449).
 *
 * Unlike sharded-experiment.js (which used a standalone prompt), this drives
 * the ACTUAL `judgeCodyRulesSharded` + its real SHARD prompts from the shipped
 ...
 *
 *   node evals/cody-rules/behavioral-shipped.js [--model=gpt-5.4-mini] [--conc=4] [--temp=0]
 */
const fs = require('fs');
...
const { judgeCodyRulesSharded } = require(
    '../../libs/code-review/infrastructure/agents/collaborators/cody-rules-sharded.judge.ts',
);
...
async function main() {
    ...
    console.log(
        `\n════ SHIPPED judgeCodyRulesSharded — behavioral (${MODELKEY}) ════`,
    );
    console.log(
