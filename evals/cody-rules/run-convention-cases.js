...
// judgeCodyRulesSharded + runJudge closure, so a number here is what ships.
//
//   node evals/cody-rules/run-convention-cases.js --model=kimi-k2.7-code --temp=none
//   node evals/cody-rules/run-convention-cases.js --model=gpt-5.4-mini --temp=0
const fs = require('fs');
...
const { judgeCodyRulesSharded } = require('../../libs/code-review/infrastructure/agents/collaborators/cody-rules-sharded.judge.ts');
...
async function main() {
    ...
    for (const c of cases) {
        ...
        for (const t of targets) {
            const tSugg = violations.filter((v) => (v.brokenCodyRulesIds || []).includes(t.uuid) || v.ruleUuid === t.uuid);
