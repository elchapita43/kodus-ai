# Cody Rules System - Complete Flow Documentation

## Overview

The Cody Rules system is a sophisticated code analysis engine that combines standard code review suggestions with organization-specific coding rules. This document explains how the entire system works, from initial analysis to final suggestion delivery, including the recent updates for enhanced suggestion processing.
...
- **Classifier Chain**: Identifies which Cody Rules apply to the code changes
- **Update Chain**: Enhances standard suggestions with Cody Rules context
- **Generation Chain**: Creates new Cody Rules-specific suggestions
- **Guardian Chain**: Validates and filters final suggestions

### 2. Cody Rules Processing Flow

#### Step 1: Initial Setup

```typescript
// Prepare analysis context with applicable Cody Rules
const codyRulesFiltered = getCodyRulesForFile(
...
const [classifiedRulesResult, updateStandardSuggestionsResult] =
    await Promise.all([
        classifiedCodyRulesChain.invoke(baseContext),
        hasCodeSuggestions
...
Determines which Cody Rules are relevant to the current changes:

- **Input**: All applicable Cody Rules + code changes
...
Enhances existing standard suggestions with Cody Rules awareness:
...
interface CodyRulesUpdateResponse {
    codeSuggestions: Array<{
        // ... standard fields
        violatedCodyRulesIds?: string[]; // Silent fixes
        brokenCodyRulesIds?: string[]; // Promote to Cody Rules
    }>;
}
```
...
**Path 2: Broken Rules (Promote to Cody Rules)**

```typescript
if (hasBroken) {
    processedSuggestions.push({
        ...normalizedSuggestion,
        label: 'cody_rules', // Fixed label
...
#### Step 5: Cody Rules Suggestions Generation
...
let finalOutput = {
    codeSuggestions: [
        ...generatedCodyRulesSuggestions?.codeSuggestions, // New Cody Rules
...
Previously, standard suggestions could inadvertently violate Cody Rules or miss opportunities to become rule-specific suggestions.
...
2. **Promotion**: Important violations become Cody Rules suggestions
...
```typescript
const result = await codyRulesAnalysisService.analyzeCodeWithAI(
...
The Cody Rules system provides a comprehensive, intelligent code analysis solution that combines automated rule enforcement with flexible suggestion enhancement. The recent updates enable more nuanced handling of different violation types while maintaining backward compatibility and system performance.
