import { CodyRulesPrLevelAnalysisService } from './codyRulesPrLevelAnalysis.service';

describe('CodyRulesPrLevelAnalysisService — Cody Rule link generation', () => {
    let service: CodyRulesPrLevelAnalysisService;
    let codyRulesService: { findById: jest.Mock };

    const orgData: any = { organizationId: 'org-1', teamId: 'team-1' };
    const PR_NUMBER = 42;

    const buildLinks = (foundIds: string[], content: string) =>
        (service as any).buildCodyRuleLinkAndRepalceIds(
            foundIds,
            content,
            orgData,
            PR_NUMBER,
    ...
    beforeEach(() => {
        ...
        codyRulesService = { findById: jest.fn() };

        service = new CodyRulesPrLevelAnalysisService(
    ...
    it('returns content unchanged when there are no rule IDs', async () => {
        ...
        expect(codyRulesService.findById).not.toHaveBeenCalled();
    });
    ...
    it('replaces a global-scoped rule ID with a markdown link to the global settings page', async () => {
        ...
        codyRulesService.findById.mockResolvedValue({
            uuid: ruleId,
        ...
        const expectedLink = `[Avoid console log](https://app.kodus.io/settings/code-review/global/cody-rules/${ruleId}?teamId=team-1)`;
    ...
    it('uses the repository-scoped URL when the rule has a non-global repositoryId', async () => {
        ...
        expect(result).toContain(
            `(https://app.kodus.io/settings/code-review/repo-uuid-xyz/cody-rules/${ruleId}?teamId=team-1)`,
        );
    ...
    it('renders 3 distinct links for 3 distinct rule IDs', async () => {
        ...
        codyRulesService.findById.mockImplementation(async (id: string) => ({
            uuid: id,
        ...
        for (const id of ids) {
            expect(result).toContain(
                `/settings/code-review/global/cody-rules/${id}?teamId=team-1)`,
            );
        }
    ...
    it('unwraps single backticks around the ID before linking', async () => {
        ...
        expect(result).toBe(
            `See [Backtick rule](https://app.kodus.io/settings/code-review/global/cody-rules/${ruleId}?teamId=team-1) for details.`,
        );
    ...
    it('leaves content unchanged when the rule is not found in the database', async () => {
        ...
        codyRulesService.findById.mockResolvedValue(null);
