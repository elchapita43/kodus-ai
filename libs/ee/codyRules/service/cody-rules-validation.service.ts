...
import { splitRulePathGlobs } from '@libs/common/utils/cody-rules/file-patterns';
...
/**
 * Service for validating and ordering Cody Rules in cloud mode
 */
@Injectable()
export class CodyRulesValidationService {
    public readonly MAX_CODY_RULES = 10;
    ...
    async validateRulesLimit(
        ...
        const limited =
            await this.permissionValidationService.shouldLimitResources(
                ...
                CodyRulesValidationService.name,
            );
        ...
        return totalRules <= this.MAX_CODY_RULES;
    }
    ...
    /**
     * Filters and orders Cody Rules.
     * It selects directory-specific, repository-specific and global active rules, removes duplicates,
     * orders them by createdAt (oldest first), and if not in cloud mode, limits the result to MAX_CODY_RULES.
     *
     * @param rules Array of CodyRules.
     * @param repositoryId Repository identifier.
     * @param directoryId Optional directory identifier.
     * @returns Array of filtered, ordered, and possibly limited CodyRules.
     */
    filterCodyRules(
        ...
        standardRules: Partial<ICodyRule>[];
        ...
        const directoryRules: Partial<ICodyRule>[] = [];
        const globalRules: Partial<ICodyRule>[] = [];
        ...
        const mergedRulesWithoutDuplicates =
            this.extractUniqueCodyRules(mergedRules);

        const limit = limited ? this.MAX_CODY_RULES : 0;
        ...
        const [standardRules, memoryRules] = orderedRules.reduce(
            ...
            [[], []] as [Partial<ICodyRule>[], Partial<ICodyRule>[]],
        );
    ...
    /**
     * Removes duplicate Cody Rules based on the 'rule' property.
     * @param codyRules Array of CodyRules.
     * @returns Array of unique CodyRules.
     */
    private extractUniqueCodyRules(
        codyRules: Partial<ICodyRule>[],
    ): Partial<ICodyRule>[] {
        const seenRules = new Set<string>();
        const uniqueCodyRules: Partial<ICodyRule>[] = [];

        codyRules.forEach((codyRule) => {
            if (codyRule?.rule && !seenRules.has(codyRule.rule)) {
                seenRules.add(codyRule.rule);
                uniqueCodyRules.push(codyRule);
            }
        });

        return uniqueCodyRules;
    }

    /**
     * Retrieves the specific Cody rules for a *file* based on glob patterns.
     * This method only matches rules whose glob patterns directly match the file.
     * @param fileName Name of the file to be checked.
     * @param codyRules Array of objects containing the pattern and Cody rules.
     * @param filters Filtering options for repository and directory.
     * @returns Array of Cody rules applicable to the file.
     */
    getCodyRulesForFile(
    ...
    /**
     * Retrieves the specific Cody rules for a *folder* based on glob patterns.
     * This matches rules that apply to the folder itself or are recursive (e.g., ** /*).
     * @param folderName Name of the folder to be checked.
     * @param codyRules Array of objects containing the pattern and Cody rules.
     * @param filters Filtering options for repository and directory.
     * @returns Array of Cody rules applicable to the folder.
     */
    getCodyRulesForFolder(
    ...
    getMemoryRulesForContext(
        ...
        const activeMemoryRules = codyRules.filter(
            (rule) =>
        ...
        return this.getCodyRulesForFolder(
            path,
            activeMemoryRules,
            normalizedFilters,
        );
    ...
    private getCodyRules(
        ...
        const isPathMatch = (rule: Partial<ICodyRule>): boolean => {
            return pathMatcher(rule, normalizedPath);
        };

        // Check if the rule matches the repository (global or specific)
        const isRepositoryMatch = (rule: Partial<ICodyRule>): boolean => {
        ...
        const isInheritanceMatch = (rule: Partial<ICodyRule>): boolean => {
