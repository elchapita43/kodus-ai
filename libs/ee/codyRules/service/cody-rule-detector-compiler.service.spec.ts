import { CodyRuleDetectorCompilerService } from './cody-rule-detector-compiler.service';
...
const make = (compilerOutput: any) => {
    ...
    const svc = new CodyRuleDetectorCompilerService(
        permissionValidationService,
        {} as any, // observabilityService (unused — runStructuredReviewCall mocked)
        codyRulesService,
    );
    return { svc, codyRulesService };
};
...
describe('CodyRuleDetectorCompilerService.compileAndSave (#1449 T0)', () => {
    it('persists a detector when the model compiles a passing regex', async () => {
        const { svc, codyRulesService } = make({
            mechanical: true,
            pattern: 'console\\.(log|warn|error)\\(',
        });
        await svc.compileAndSave(org, 'r1', mechanicalRule);
        expect(codyRulesService.updateRuleDetector).toHaveBeenCalledTimes(1);
        const [, ruleId, detector] =
            codyRulesService.updateRuleDetector.mock.calls[0];
    ...
    it('does NOT persist when the model declines (rule stays semantic)', async () => {
        ...
        await svc.compileAndSave(org, 'r1', mechanicalRule);
        expect(codyRulesService.updateRuleDetector).not.toHaveBeenCalled();
    });
    ...
    it('clears a stale detector when an edited rule no longer compiles', async () => {
        const { svc, codyRulesService } = make({ mechanical: false });
        await svc.compileAndSave(org, 'r1', {
            ...mechanicalRule,
            detector: { type: 'regex', pattern: 'old' }, // had one before
        });
        expect(codyRulesService.updateRuleDetector).toHaveBeenCalledWith(
            org.organizationId,
            'r1',
            null,
        );
    ...
    it('never throws / never persists when the LLM call errors', async () => {
        const { svc, codyRulesService } = make(null);
        mockRun.mockReset();
