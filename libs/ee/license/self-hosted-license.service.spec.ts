...
describe('SelfHostedLicenseService.getLicenseKey', () => {
    ...
    it('reads the customer-facing CODUS_LICENSE_KEY from env', async () => {
        process.env.CODUS_LICENSE_KEY = 'customer-license-jwt';
        expect(await getKey(makeService())).toBe('customer-license-jwt');
    });

    it('prefers the DB-stored key over env', async () => {
        process.env.CODUS_LICENSE_KEY = 'env-loses';
...
/**
 * Self-hosted has no Codus-funded managed trial credits, so credit
