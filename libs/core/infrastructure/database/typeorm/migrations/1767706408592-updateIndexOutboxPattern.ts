...
export class UpdateIndexOutboxPattern1767706408592 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`
            ...
            ON "codus_workflow"."inbox_messages" ("consumerId", "status", "lockedAt")
        `);
        ...
        await queryRunner.query(`
            ...
            ON "codus_workflow"."outbox_messages" ("status", "createdAt")
        `);

        // OUTBOX: Index for reaper queries (stuck messages in PROCESSING)
        await queryRunner.query(`
            CREATE INDEX IF NOT EXISTS "IDX_outbox_messages_locked_at"
            ON "codus_workflow"."outbox_messages" ("lockedAt")
        `);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`
            DROP INDEX IF EXISTS "codus_workflow"."IDX_outbox_messages_locked_at"
        `);
        await queryRunner.query(`
            DROP INDEX IF EXISTS "codus_workflow"."IDX_outbox_messages_status_created"
        `);
        await queryRunner.query(`
            DROP INDEX IF EXISTS "codus_workflow"."IDX_inbox_messages_locked_at"
        `);
        await queryRunner.query(`
            DROP INDEX IF EXISTS "codus_workflow"."IDX_inbox_messages_consumer_status_locked"
        `);
    }
}
