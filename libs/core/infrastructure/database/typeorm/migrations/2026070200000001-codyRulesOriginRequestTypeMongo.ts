...
import { migrateCodyRulesOriginRequestType } from '../../mongo/cody-rules/migrate-origin-request-type';
 ...
 */
export class CodyRulesOriginRequestTypeMongo2026070200000001
    implements MigrationInterface
{
    name = 'CodyRulesOriginRequestTypeMongo2026070200000001';
    transaction = false;

    public async up(_queryRunner: QueryRunner): Promise<void> {
        if (mongoMigrationsSkipped()) {
            console.log(
                '[CodyRulesOriginRequestTypeMongo] skipped (SKIP_MONGO_MIGRATIONS=true)',
            );
            return;
        }
        const { db, close } = await mongoMigrationClient();
        try {
            await migrateCodyRulesOriginRequestType(db, {
                log: (m) => console.log(m),
            });
        } finally {
            await close();
