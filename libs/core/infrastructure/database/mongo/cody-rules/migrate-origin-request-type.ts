/**
 * One-shot backfill for the widened Cody Rules `origin` enum and the
...
// libs/common/utils/cody-rules/file-patterns.ts. Inlined so the module stays
// dependency-free.
...
export type MigrateCodyRulesOptions = {
    dryRun?: boolean;
    log?: (msg: string) => void;
};

export type MigrateCodyRulesResult = {
    docsScanned: number;
    docsUpdated: number;
    rulesMigrated: number;
};

export async function migrateCodyRulesOriginRequestType(
    db: Db,
    opts: MigrateCodyRulesOptions = {},
): Promise<MigrateCodyRulesResult> {
    ...
    log(
        `[migrate-cody-rules]${dryRun ? ' [DRY RUN]' : ''} scanned ${docsScanned} org doc(s); ` +
