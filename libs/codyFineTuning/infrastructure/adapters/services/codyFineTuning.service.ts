...
} from '@libs/codyFineTuning/domain/suggestionEmbedded/contracts/suggestionEmbedded.service.contract';
import { IClusterizedSuggestion } from '@libs/codyFineTuning/domain/interfaces/codyFineTuning.interface';
import { LabelType } from '@libs/common/utils/codeManagement/labels';
import { ISuggestionEmbedded } from '@libs/codyFineTuning/domain/suggestionEmbedded/interfaces/suggestionEmbedded.interface';
import { FeedbackType } from '@libs/codyFineTuning/domain/enums/feedbackType.enum';
import { FineTuningType } from '@libs/codyFineTuning/domain/enums/fineTuningType.enum';
import { FineTuningDecision } from '@libs/codyFineTuning/domain/enums/fineTuningDecision.enum';
import { SeverityLevel } from '@libs/common/utils/enums/severityLevel.enum';

@Injectable()
export class CodyFineTuningService {
    private readonly logger = createLogger(CodyFineTuningService.name);
    ...
    public async startAnalysis(
        ...
        try {
            ...
            this.logger.error({
                ...
                error,
                context: CodyFineTuningService.name,
    ...
    async getDataForEmbedSuggestions(
        ...
        const suggestionsWithFeedbackFilteredLabels =
            refinedSuggestions.uniqueSuggestionsWithFeedback.filter(
                (suggestion) =>
                    suggestion.label !== LabelType.CODY_RULES &&
    ...
    private async analyzeWithClusterization(
        ...
        for (const newSuggestion of suggestionsToAnalyze) {
            if (
                newSuggestion?.label === LabelType.CODY_RULES ||
    ...
    private async defineFineTuningThresholds(): Promise<{
        ...
        const globalParameters = await this.globalParametersService.findByKey(
            GlobalParametersKey.CODY_FINE_TUNING_CONFIG,
        );
