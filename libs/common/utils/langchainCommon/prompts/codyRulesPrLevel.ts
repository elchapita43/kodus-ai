...
export type CodyRulesPrLevelPayload = {
    pr_title: string;
...
function formatMemoriesSection(
    memories: CodyRulesPrLevelPayload['memories'],
): string {
...
function formatExternalReferencesMapSection(
    externalReferencesMap: CodyRulesPrLevelPayload['externalReferencesMap'],
): string {
...
function formatMcpResultsSection(
    mcpResultsMap: CodyRulesPrLevelPayload['mcpResultsMap'],
): string {
...
export const prompt_codyrules_prlevel_analyzer = (
    payload: CodyRulesPrLevelPayload,
) => {
    ...
    "violations": [
      {
        ...
        "suggestionContent": "Detailed explanation of the violation and specific steps to fix it. Always end with: Cody Rule violation: rule-id-here"
      }
    ]
  }
]
  ...
  "violations": [
    {
      ...
      "suggestionContent": "The new route /api/users was added in the controller but routes.json was not updated. Please add an entry for this route in the routes.json file following the existing format. Cody Rule violation: route-documentation"
    }
  ]
}
\`\`\`
    ...
    "violations": [
      {
        ...
        "suggestionContent": "Pull request description is empty but is required for all PRs. Cody Rule violation: pr-description-required"
      }
    ]
  }
]
...
- **Always include rule reference** - end suggestionContent with "Cody Rule violation: [rule-id]"
...
export const prompt_codyrules_prlevel_group_rules = (payload: any) => {
...
- Maintain the existing "Cody Rule violation:" reference at the end if present

## Example Consolidation
**Input Violations**:
1. "O arquivo de serviço AcompanhamentoNutricionalService.cs foi adicionado, mas o arquivo de teste correspondente não foi encontrado. Cody Rule violation: service-test-required"
2. "O arquivo de serviço QuestionarioService.cs foi adicionado, mas o arquivo de teste correspondente não foi encontrado. Cody Rule violation: service-test-required"

**Expected Output**:
"Os arquivos de serviço AcompanhamentoNutricionalService.cs e QuestionarioService.cs foram adicionados, mas os arquivos de teste correspondentes não foram encontrados no Pull Request. É necessário adicionar testes para as novas funcionalidades de serviço para garantir a qualidade e o funcionamento correto. Cody Rule violation: service-test-required"
