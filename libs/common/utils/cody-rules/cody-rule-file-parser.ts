...
/**
 * Deterministic parser for `.cody/rules/**` markdown files (the structured
 ...
 * `CodyRulesSyncService.convertFileToCodyRules`) remains the fallback for
 * free-form sources (CLAUDE.md, .cursorrules, ...) and for `.cody` files
...
export interface ParsedCodyRuleFile {
    uuid?: string;
    title: string;
...
export function extractExamplesFromBody(
    body: string,
): ParsedCodyRuleFile['examples'] {
    const examples: ParsedCodyRuleFile['examples'] = [];
...
export function parseCodyRuleFile(content: string): ParsedCodyRuleFile | null {
...
/**
 * Whether `filePath` is a structured Cody rule template file (the only
 * sources parsed verbatim): anything under `.cody/rules/`, or a `.md`
 ...
 */
export function isCodyRuleTemplateFile(
    filePath: string | null | undefined,
): boolean {
    if (!filePath) return false;
    const lower = filePath.replace(/\\/g, '/').toLowerCase();

    const codyEnd = hasDirSegment(lower, '.cody/rules/');
    if (codyEnd >= 0 && lower.length > codyEnd) return true;
