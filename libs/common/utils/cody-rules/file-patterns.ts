...
export const RULE_FILE_PATTERNS = [
    ...
    '.rules/**/*',
    '.cody/rules/**',
    // Documented alongside `.cody/rules/**` on the "Repository Rules"
 ...
 * IDE-synced rules — e.g. the toggle-off purge in `CodyRulesSyncService` —
 ...
 *
 * Cody Rules support OR-joined globs in a single `path` string (the
...
export function pathMatchesIdeRuleDir(
    ...
    // Comma-separated list (CodyRules supports OR-joined globs)
