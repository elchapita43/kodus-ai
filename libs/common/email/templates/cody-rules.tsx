...
export type CodyRulesEmailProps = {
    userName: string;
...
export function codyRulesEmailMeta({
    organizationName,
}: {
    organizationName: string;
}) {
    return {
        ...
        subject: `New Cody Rules generated for ${organizationName}`,
    };
}
...
function CodyRulesEmail({
    ...
    return (
        <BrandLayout
            preview={`${rulesCount} new Cody Rule${
                rulesCount === 1 ? '' : 's'
            } for ${organizationName}`}
        >
            <Heading style={baseHeading}>New Cody Rules are ready</Heading>
            ...
            {remaining > 0 ? (
                <Text style={mutedText}>
                    + {remaining} more — review the full list in Codus.
                </Text>
            ) : null}
...
CodyRulesEmail.PreviewProps = {
    ...
    rulesCount: 4,
    rulesLink: 'https://app.kodus.io/library/cody-rules',
} satisfies CodyRulesEmailProps;

export default CodyRulesEmail;
