...
export class CheckSyncStatusUseCase {
    ...
    async execute(
        ...
        const syncStatusFlags = {
            ...
            codyRulesGeneratorEnabledFirstTime: true,
        };
        ...
        try {
            ...
            if (!ideRulesSyncEnabled) {
                const ideRules = rules?.find((rule) =>
                    rule?.rules?.find((r: ICodyRule) => r.sourcePath),
                );
            ...
            const hasPastReviewRules = rules?.some((rule) =>
                rule?.rules?.some(
                    (r: ICodyRule) =>
                        r.origin === CodyRulesOrigin.PAST_REVIEWS,
                ),
            );

            syncStatusFlags.codyRulesGeneratorEnabledFirstTime =
                !hasPastReviewRules;
