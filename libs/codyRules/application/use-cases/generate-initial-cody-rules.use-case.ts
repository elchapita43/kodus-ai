...
/**
 * Seed a repository's Cody Rules from its last 3 months of PR review comments,
 ...
 */
@Injectable()
export class GenerateInitialCodyRulesUseCase {
    private readonly logger = createLogger(
        GenerateInitialCodyRulesUseCase.name,
    );
    ...
    static initialGenerationLockKey(
        ...
        return `CODY_RULES:INITIAL_GEN:${organizationId}:${repositoryId}`;
    }
    ...
    async execute(params: {
        ...
        try {
            ...
            if (!lock) {
                this.logger.log({
                    message:
                        'Initial Cody Rules generation skipped — already running for this repository',
                    context: GenerateInitialCodyRulesUseCase.name,
            ...
            if (await this.hasPastReviewRules(organizationId, repositoryId)) {
                this.logger.log({
                    message:
                        'Initial Cody Rules generation skipped — repository already has past-review rules',
                    context: GenerateInitialCodyRulesUseCase.name,
                    metadata: { organizationId, teamId, repositoryId },
                });
                return;
            ...
            this.logger.log({
                message:
                    'Starting initial 3-month Cody Rules generation for newly-enabled repository',
                context: GenerateInitialCodyRulesUseCase.name,
                metadata: { organizationId, teamId, repositoryId },
            });
            ...
            this.logger.error({
                message: 'Initial Cody Rules generation failed',
                context: GenerateInitialCodyRulesUseCase.name,
                error:
    ...
    async hasPastReviewRulesForRepos(
        ...
        const documents = await this.codyRulesService.find({
            organizationId,
        ...
        for (const document of documents ?? []) {
            for (const rule of document?.rules ?? []) {
                if (
                    rule?.origin === CodyRulesOrigin.PAST_REVIEWS &&
                    rule?.repositoryId &&
