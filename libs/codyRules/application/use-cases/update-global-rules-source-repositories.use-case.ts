...
export class UpdateGlobalRulesSourceRepositoriesUseCase {
    ...
    async execute(params: {
        ...
        // the actual import in CodyRulesSyncService.syncRepositoryGlobal.
        const tier =
        ...
        if (tier === 'free') {
            throw new ForbiddenException(
                'Importing global Cody Rules is not available on the Free plan. Upgrade to enable it.',
            );
        }
        ...
        for (const repo of removed) {
            await this.codyRulesSyncService.purgeGlobalRulesForSourceRepository({
                organizationAndTeamData,
        ...
        this.logger.log({
            message: `[cody-rules-global-sync] source list updated: +${added.length} added, -${removed.length} removed, ${nextRepositories.length} total`,
            context: UpdateGlobalRulesSourceRepositoriesUseCase.name,
            metadata: {
                organizationAndTeamData,
