...
@Injectable()
export class SyncSelectedRepositoriesCodyRulesUseCase {
    private readonly logger = createLogger(
        SyncSelectedRepositoriesCodyRulesUseCase.name,
    );
    constructor(
    ...
    async execute(params: {
        ...
        try {
            ...
            for (const repo of filtered) {
                await this.codyRulesSyncService.syncRepositoryMain({
                    organizationAndTeamData,
                    repository: {
                        id: String(repo.id),
                        name: repo.name,
            ...
            this.logger.error({
                message: 'Failed to sync selected repositories Cody Rules',
                context: SyncSelectedRepositoriesCodyRulesUseCase.name,
                error,
                metadata: {
                    organizationAndTeamData,
                    params,
