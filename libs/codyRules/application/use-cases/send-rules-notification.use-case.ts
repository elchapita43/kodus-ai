...
export class SendRulesNotificationUseCase {
    ...
    async execute(organizationId: string, rules: string[]): Promise<void> {
        try {
            this.logger.log({
                message: 'Starting Cody Rules notification process',
            ...
            this.logger.log({
                message: 'Emitting Cody Rules notification',
                context: SendRulesNotificationUseCase.name,
                metadata: {
                    organizationId,
            ...
            this.logger.log({
                message: 'Cody Rules notification emitted successfully',
                context: SendRulesNotificationUseCase.name,
                metadata: { organizationId },
            });
        } catch (error) {
            this.logger.error({
                message: 'Error in Cody Rules notification process',
                context: SendRulesNotificationUseCase.name,
                error,
                metadata: {
                    organizationId,
