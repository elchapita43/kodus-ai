 ...
 * `POST /cody-rules/imported/manage` so the UI banner ("Auto-sync OFF, X
 * rules still active") can offer pause/resume/delete in one place.
 */
export type ManageImportedRulesAction = 'pause' | 'resume' | 'delete';

@Injectable()
export class ManageImportedCodyRulesUseCase {
    private readonly logger = createLogger(
        ManageImportedCodyRulesUseCase.name,
    );

    constructor(
        private readonly syncService: CodyRulesSyncService,
    ...
    async execute(params: {
        ...
        // The endpoint guard only checks Update on cody_rules at the type
