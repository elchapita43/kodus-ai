...
import { GenerateCodyRulesDTO } from '@libs/core/domain/dtos/generate-cody-rules.dto';

import {
    CommentAnalysisService,
    CodyRulesModelSelection,
} from '@libs/code-review/infrastructure/adapters/services/commentAnalysis.service';
import { PermissionValidationService } from '@libs/ee/shared/services/permissionValidation.service';
import { resolveCodyRulesModelPolicy } from '@libs/codyRules/application/services/cody-rules-model-policy';
...
// onboarding into elchapita43/tiny-url (38+ historical PRs). Cut to 2 so
...
@Injectable()
export class GenerateCodyRulesUseCase {
    private readonly logger = createLogger(GenerateCodyRulesUseCase.name);
    constructor(
    ...
    private async requiresPastReviewApproval(
        ...
        try {
            ...
            this.logger.warn({
                message:
                    'Could not resolve codyKnowledgeApproval for generated rules; defaulting to active',
                context: GenerateCodyRulesUseCase.name,
                error:
    ...
    async execute(body: GenerateCodyRulesDTO, organizationId: string) {
        ...
        try {
            ...
            // fetching or status transition, and leave codyLearningStatus as-is
            // (ENABLED) so the UI shows the feature idle rather than stuck.
            const modelPolicy = await resolveCodyRulesModelPolicy(
            ...
            if (!modelPolicy.generate) {
                this.logger.warn({
                    message: `Skipping Cody Rules generation — ${modelPolicy.skipReason}`,
                    context: GenerateCodyRulesUseCase.name,
            ...
            await this.createOrUpdateParametersUseCase.execute(
                ParametersKey.PLATFORM_CONFIGS,
                {
                    ...
                    // crash leaves it incremented — the CodyLearning cron
                    // reads it to stop retrying a run that keeps dying.
                    codyLearningStuckRetries:
                        (platformConfig.configValue.codyLearningStuckRetries ??
                            0) + 1,
                },
                organizationAndTeamData,
            );
            ...
            for (const repository of filteredRepositories) {
                ...
                if (excludedForRepo && excludedForRepo.size > 0) {
                    this.logger.log({
                        message: `Excluding ${excludedForRepo.size} reviewer(s) from Cody Rules learning`,
                        context: GenerateCodyRulesUseCase.name,
                        metadata: {
                            organizationAndTeamData,
                ...
                const rules =
                    await this.commentAnalysisService.generateCodyRules({
                ...
                for (const rule of rules) {
                    const dto: CreateCodyRuleDto = {
                        ...
                        path: '',
                        status: targetStatus,
                        severity: rule.severity as CodyRuleSeverity,
                    };

                    const userInfo = {
                        userId: 'cody-system-rules-generator',
                    ...
                    try {
                        ...
                        if (!createdRule) {
                            throw new Error(
                                'Failed to persist generated Cody rule',
                            );
                        }
                        ...
                        this.logger.error({
                            message:
                                'Failed to persist a generated Cody rule; keeping the others',
                            context: GenerateCodyRulesUseCase.name,
                            error:
            ...
            // resets codyLearningStatus to ENABLED) instead of a misleading
            ...
            this.logger.log({
                message: 'Cody rules generated successfully',
                context: GenerateCodyRulesUseCase.name,
                metadata: { body, organizationAndTeamData },
            });
            ...
            if (createdRules.length > 0) {
                this.logger.log({
                    message: 'Sending email notification for new Cody rules',
                    context: GenerateCodyRulesUseCase.name,
                    metadata: {
                        organizationId,
                ...
                // readable, and the same text the Cody Rules screen shows.
                ...
                this.sendRulesNotificationUseCase
                    ...
                    .catch((error) => {
                        this.logger.error({
                            message:
                                'Error sending email notification for Cody rules',
                            context: GenerateCodyRulesUseCase.name,
                            error,
                            metadata: {
                                organizationId,
    ...
    private async resolveExcludedReviewersByRepo(
        ...
        try {
            ...
            const globalExcluded = (resolvedGlobal as any)
                ?.codyLearningExcludedReviewers as string[] | undefined;
            ...
            for (const repo of codeReviewConfig?.configValue?.repositories ??
                ...
                byRepo.set(
                    repo.id,
                    (resolvedRepo as any)?.codyLearningExcludedReviewers as
                        | string[]
                        | undefined,
                );
            }
