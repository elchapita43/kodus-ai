...
@Injectable()
export class FindRulesInOrganizationByRuleFilterCodyRulesUseCase implements IUseCase {
    private readonly logger = createLogger(
        FindRulesInOrganizationByRuleFilterCodyRulesUseCase.name,
    );
    ...
    async execute(
        ...
        try {
            ...
            const ruleFilters: Partial<ICodyRule>[] = [];
            ...
            const existingRules = await this.codyRulesService.find({
                organizationId,
            ...
            this.logger.error({
                message:
                    'Error finding Cody Rules in organization by rule filter',
                context:
                    FindRulesInOrganizationByRuleFilterCodyRulesUseCase.name,
                error: error,
                metadata: {
