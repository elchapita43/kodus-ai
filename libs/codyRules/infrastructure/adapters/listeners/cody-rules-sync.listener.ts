...
import { GlobalRulesSourceConfig } from '@libs/codyRules/domain/interfaces/global-rules-source.interface';
...
@Injectable()
export class CodyRulesSyncListener {
    private readonly logger = createLogger(CodyRulesSyncListener.name);

    constructor(
        private readonly codyRulesSyncService: CodyRulesSyncService,
    ...
    private async isGlobalSourceRepo(event: PullRequestClosedEvent): Promise<boolean> {
        try {
            ...
            this.logger.error({
                ...
                context: CodyRulesSyncListener.name,
                error,
                metadata: {
    ...
    private async claimSyncRun(claimKey: string): Promise<boolean> {
        try {
            ...
            await this.dataSource
                .query(
                    `CREATE TABLE IF NOT EXISTS codus_event_claims (
            ...
            await this.dataSource.query(
                `DELETE FROM codus_event_claims WHERE created_at < now() - interval '7 days'`,
            );
            const rows: unknown[] = await this.dataSource.query(
                `INSERT INTO codus_event_claims (claim_key) VALUES ($1)
    ...
    async handlePullRequestClosedEvent(event: PullRequestClosedEvent) {
        if (!event.repository || !event.repository.id) {
            this.logger.warn({
                message:
                    'Received pull-request.closed event without repository information, skipping Cody rules sync',
                context: CodyRulesSyncListener.name,
                metadata: {
        ...
        if (!event.merged) {
            this.logger.log({
                message:
                    'Received non-merged pull-request.closed event, skipping Cody rules sync',
                context: CodyRulesSyncListener.name,
                metadata: {
                    pullRequestNumber: event.pullRequestNumber,
                    repositoryId: event.repository.id,
        ...
        if (await this.isCentralizedConfigRepo(event)) {
            this.logger.log({
                message:
                    'Pull request closed in centralized config repository, skipping Cody rules sync',
                context: CodyRulesSyncListener.name,
                metadata: {
                    pullRequestNumber: event.pullRequestNumber,
                    repositoryId: event.repository.id,
        ...
        this.logger.log({
            message: 'Handling pull-request.closed event for Cody Rules Sync',
            context: CodyRulesSyncListener.name,
            metadata: {
        ...
        const claimKey = `cody-rules-sync:${event.organizationAndTeamData?.organizationId}:${event.repository.id}:${event.pullRequestNumber}`;
        ...
        try {
            await this.codyRulesSyncService.syncFromChangedFiles({
                organizationAndTeamData: event.organizationAndTeamData,
            ...
            if (await this.isGlobalSourceRepo(event)) {
                await this.codyRulesSyncService.syncRepositoryGlobal({
                    organizationAndTeamData: event.organizationAndTeamData,
                    repository: event.repository,
                });
            }
            ...
            await this.dataSource
                .query(`DELETE FROM codus_event_claims WHERE claim_key = $1`, [
                    claimKey,
                ])
                .catch(() => undefined);
            throw error;
    ...
    async handleIdeRulesSyncDisabled(
        ...
        switch (action) {
            case 'keep':
                ...
                return;
            case 'pause':
                await this.codyRulesSyncService.pauseAllIdeSyncRulesForRepository(
                    {
                        organizationAndTeamData: event.organizationAndTeamData,
                        repositoryId: event.repositoryId,
                    },
                ...
                return;
            case 'delete':
                await this.codyRulesSyncService.purgeAllIdeSyncRulesForRepository(
                    {
                        organizationAndTeamData: event.organizationAndTeamData,
                        repositoryId: event.repositoryId,
                    },
    ...
    private async isCentralizedConfigRepo(
        ...
        try {
            ...
            this.logger.warn({
                message:
                    'Failed to determine centralized config status for Cody rules listener',
                context: CodyRulesSyncListener.name,
                metadata: {
                    organizationAndTeamData: event.organizationAndTeamData,
                    repositoryId: event.repository?.id,
