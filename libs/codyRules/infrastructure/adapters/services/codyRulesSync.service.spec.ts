...
import { CodyRulesSyncService } from './codyRulesSync.service';
 ...
 */
describe('CodyRulesSyncService.convertFileToCodyRules — post-trial BYOK gate', () => {
    const ORG = {
    ...
    const makeService = (opts: {
        ...
        const service = new (CodyRulesSyncService as any)(...deps);
        return { service, permissionValidationService };
    };

    const params = {
        // NOT a `.cody/rules/**` template → does not import verbatim, falls
    ...
    it.each(POST_TRIAL)(
        ...
        async (status) => {
            ...
            const result = await (service as any).convertFileToCodyRules(
                params,
                options,
            );
    ...
    it('trial (not ended) + no BYOK → managed model IS allowed (gate does not fire)', async () => {
        ...
        await (service as any)
            .convertFileToCodyRules(params, options)
