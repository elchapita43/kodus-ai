...
} from '@libs/common/utils/cody-rules/file-patterns';
import {
    isCodyRuleTemplateFile,
    parseCodyRuleFile,
} from '@libs/common/utils/cody-rules/cody-rule-file-parser';
...
import { GLOBAL_RULES_TRIAL_IMPORT_LIMIT } from '@libs/codyRules/domain/interfaces/global-rules-source.interface';
...
import {
    codyRulesIDEGeneratorSchema,
    codyRulesIDEGeneratorSchemaOnboarding,
    codyRulesManifestGeneratorSchemaOnboarding,
...
@Injectable()
export class CodyRulesSyncService {
    private readonly systemUserInfo: UserInfo = {
        userId: 'cody-rules-sync',
        userEmail: 'cody@kodus.io',
    };

    private readonly logger = createLogger(CodyRulesSyncService.name);
    constructor(
    ...
    private async resolveDirectoryForFile(params: {
        ...
        try {
            ...
            this.logger.warn({
                ...
                context: CodyRulesSyncService.name,
                error,
                metadata: params,
            });
            return null;
    ...
    private async findRuleBySourcePath(params: {
        ...
        repositoryId: string;
        sourcePath: string;
    }): Promise<Partial<{ uuid: string; status: CodyRulesStatus }> | null> {
        try {
            ...
            const found =
                newestFirst.find(
                    (r) => r?.status !== CodyRulesStatus.DELETED,
                ) ?? newestFirst[0];
    ...
    private async deleteRuleBySourcePath(params: {
        ...
        try {
            const { organizationAndTeamData, repositoryId, sourcePath } =
                params;
            const entity = await this.codyRulesService.findByOrganizationId(
                organizationAndTeamData.organizationId,
            );
            if (!entity) return;
            ...
            // reappears (or the @cody-ignore marker is removed).
            await this.codyRulesService.createOrUpdate(
                organizationAndTeamData,
                { ...toDelete, status: CodyRulesStatus.DELETED } as any,
                this.systemUserInfo,
            );
        } catch (error) {
            this.logger.error({
     ...
     * the source file still exists but no longer carries `@cody-sync`
    ...
    async syncFromChangedFiles(params: {
        ...
        try {
            ...
            // If the sync is disabled, we need to force sync the files that have @cody-sync
            const forceSyncFiles: string[] = [];
            // Cache decoded file content from the @cody-sync scan so the
            ...
            if (!syncEnabled) {
                ...
                // Now we need to check which files have @cody-sync in the content
                for (const f of ruleChanges) {
                    ...
                    if (content) {
                        ...
                        if (this.shouldForceSync(content)) {
                            ...
                            this.logger.log({
                                message:
                                    'File marked for force sync with @cody-sync',
                                context: CodyRulesSyncService.name,
                                metadata: {
                                    filename: f.filename,
                ...
                // `@cody-sync` marker or were deleted outright. Without
                ...
                if (forceSyncFiles.length === 0) {
                    this.logger.log({
                        message:
                            'IDE rules sync disabled and no files marked with @cody-sync',
                        context: CodyRulesSyncService.name,
                        metadata: {
                            repositoryId: repository.id,
                            organizationAndTeamData,
            ...
            for (const f of ruleChanges) {
                ...
                // Reuse cached content from the @cody-sync scan when
                ...
                //Verify if the file should be ignored due to the @cody-ignore marker
                if (this.shouldIgnoreFile(decoded)) {
                    this.logger.log({
                        message:
                            'File ignored due to @cody-ignore marker - removing existing rules',
                        context: CodyRulesSyncService.name,
                        metadata: {
                            file: f.filename,
                            repositoryId: repository.id,
                ...
                const rules = await this.convertFileToCodyRules({
                    filePath: f.filename,
                    repositoryId: repository.id,
                    content: decoded,
                    organizationAndTeamData,
                ...
                const dto: CreateCodyRuleDto = {
                    ...
                    severity:
                        ...
                        )?.toLowerCase?.() as CodyRuleSeverity) ||
                    ...
                    scope:
                        (oneRule.scope as CodyRulesScope) ||
                        CodyRulesScope.FILE,
                ...
                // @cody-sync is an EXPLICIT repo-is-source-of-truth marker:
                ...
                if (
                    ...
                    existing?.status === CodyRulesStatus.REJECTED
                ) {
                    this.logger.log({
                        message:
                            '[cody-rules-sync] force-synced file over a rejected rule — reactivating',
                        context: CodyRulesSyncService.name,
                        metadata: {
                            file: f.filename,
                    ...
                    (dto as any).status = CodyRulesStatus.ACTIVE;
                }
            ...
            this.logger.error({
                message: 'Failed to sync Cody Rules from changed files',
                context: CodyRulesSyncService.name,
                error,
                metadata: params,
            });
    ...
    async syncRepositoryMain(params: SyncTarget): Promise<void> {
        ...
        try {
            ...
            // Se o sync não estiver habilitado, verificar quais arquivos têm @cody-sync
            ...
            if (!syncEnabled) {
                ...
                // `@cody-sync`). For each pinned rule:
            ...
            // Grep-able per-run summary ("[cody-rules-sync] summary"):
            // the single record self-hosted operators need to answer
            // "did my file sync, and if not why".
            this.logger.log({
                message: `[cody-rules-sync] summary: ${syncOutcome.imported.length} imported, ${syncOutcome.skipped.length} skipped, ${syncOutcome.removed.length} removed (of ${filesToSync.length} candidate file(s))`,
                context: CodyRulesSyncService.name,
                metadata: {
                    organizationAndTeamData,
                    repositoryId: repository.id,
            ...
            this.logger.error({
                message: 'Failed to sync Cody Rules from main',
                context: CodyRulesSyncService.name,
                error,
                metadata: params,
            });
    ...
    private async syncSingleFileFromMain(params: {
        ...
        if (!syncEnabled && !this.shouldForceSync(content)) {
            // File exists but no longer carries `@cody-sync` — depin so the
            ...
            this.logger.log({
                message:
                    'Requested file is not marked with @cody-sync while IDE rules sync is disabled',
                context: CodyRulesSyncService.name,
                metadata: {
                    repositoryId: repository.id,
                    filePath,
        ...
        if (!syncEnabled) {
            this.logger.log({
                message: 'File marked for force sync with @cody-sync',
                context: CodyRulesSyncService.name,
                metadata: {
                    filename: filePath,
                    repositoryId: repository.id,
        ...
        const dto: CreateCodyRuleDto = {
            ...
            origin: CodyRulesOrigin.REPO_FILE_SYNC,
            status: oneRule.status as any,
            scope: (oneRule.scope as CodyRulesScope) || CodyRulesScope.FILE,
            examples: Array.isArray(oneRule.examples)
                ? (oneRule.examples as any)
                : [],
    ...
    async syncRepositoryMainFast(
        ...
        rules: Array<Partial<CreateCodyRuleDto>>;
        ...
        const response = {
            rules: [] as Array<Partial<CreateCodyRuleDto>>,
        ...
        try {
                    ...
                    while (true) {
                        ...
                        try {
                            ...
                            if (this.shouldIgnoreFile(content)) {
                                response.skippedFiles.push({
                                    file: file.path,
                                    reason: 'ignored via @cody-ignore',
                                });
                                continue;
                            }
            ...
            // Structured `.cody/rules/**` templates never go through the
            // batch LLM — parse them verbatim and only send the free-form
            // remainder to the model (same policy as the main sync path).
            const templateRules: Array<Partial<CreateCodyRuleDto>> = [];
            const llmCandidates: typeof candidates = [];
            for (const candidate of candidates) {
                const parsed = isCodyRuleTemplateFile(candidate.path)
                    ? parseCodyRuleFile(candidate.content)
                    : null;
                if (parsed) {
                    ...
                    templateRules.push({
                        ...
                        severity: parsed.severity as CodyRuleSeverity,
                        scope: parsed.scope as CodyRulesScope,
            ...
            const llmRules = !llmCandidates.length
                ? []
                : manifestMode
                  ? await this.convertManifestsToCodyRulesFastBatch({
                        files: llmCandidates,
                        repositoryId: repository.id,
                        organizationAndTeamData,
                    })
                  : await this.convertFilesToCodyRulesFastBatch({
                        files: llmCandidates,
                        repositoryId: repository.id,
                        organizationAndTeamData,
                    });
            ...
            if (Array.isArray(rules)) {
                for (const rule of rules) {
                    ...
                    if (validated.reason !== 'accepted-as-is') {
                        this.logger.log({
                            message: `[cody-rules-fast] path validation: ${validated.reason}`,
                            context: CodyRulesSyncService.name,
                            metadata: {
                                sourceFilePath: sourcePath,
                    ...
                    const dto: CreateCodyRuleDto = {
                        ...
                        severity:
                            ...
                            CodyRuleSeverity.MEDIUM,
                        scope:
                            (rule.scope as CodyRulesScope) ||
                            CodyRulesScope.FILE,
                        origin: CodyRulesOrigin.REPO_FILE_SYNC,
                        status: (rule.status as any) || CodyRulesStatus.PENDING,
    ...
    private async resolveSyncDefaultStatus(
        ...
        try {
            ...
            this.logger.warn({
                message:
                    'Could not resolve codyKnowledgeApproval for IDE-synced rule; defaulting to active',
                context: CodyRulesSyncService.name,
                error:
                    error instanceof Error ? error : new Error(String(error)),
                metadata: { organizationAndTeamData, repositoryId },
            });
            return CodyRulesStatus.ACTIVE;
        }
    }
     ...
     * a file extension. Markers without an extension (`@cody-sync`) and
    ...
    private async inlineAtFileReferences(params: {
        ...
        for (const match of params.content.matchAll(
            CodyRulesSyncService.AT_FILE_REF_RE,
        )) {
            refs.add(match[1]);
        ...
        for (const ref of refs) {
            ...
            if (resolvedPath && refContent) {
                ...
                this.logger.warn({
                    message:
                        '[cody-rules-sync] could not resolve @file reference while importing rule file',
                    context: CodyRulesSyncService.name,
                    metadata: {
                        reference: `@${ref}`,
        ...
        this.logger.log({
            message: `[cody-rules-sync] inlined ${sections.length} @file reference(s) before rule extraction`,
            context: CodyRulesSyncService.name,
            metadata: {
     ...
     * rules. Verbatim `.cody/rules` templates are exempt (returned before
     * inlining) — user-authored bodies are never mutated.
     */
    private async convertFileToCodyRules(
        params: {
            filePath: string;
            repositoryId: string;
            content: string;
        ...
        options?: {
            ...
            runName?: string;
            defaultStatus?: CodyRulesStatus;
        },
    ): Promise<Array<Partial<CreateCodyRuleDto>>> {
        const validationResult =
            await this.permissionValidationService.validateBasicLicense(
                params.organizationAndTeamData,
                CodyRulesSyncService.name,
            );
        ...
        // Structured `.cody/rules/**` templates are imported VERBATIM —
        ...
        // stripped identifiers). Non-template `.cody` files (no/invalid
        // frontmatter) fall through to the LLM path below.
        if (isCodyRuleTemplateFile(params.filePath)) {
            const parsed = parseCodyRuleFile(params.content);
            if (parsed) {
                if (!parsed.enabled) {
                    this.logger.log({
                        message:
                            '[cody-rules-sync] template file disabled via frontmatter, skipping import',
                        context: CodyRulesSyncService.name,
                        metadata: {
                            filePath: params.filePath,
                ...
                this.logger.log({
                    message:
                        '[cody-rules-sync] imported .cody/rules template verbatim (no LLM)',
                    context: CodyRulesSyncService.name,
                    metadata: {
                        filePath: params.filePath,
                        repositoryId: params.repositoryId,
        ...
        // `.cody/rules` templates (returned above) use no LLM and are never
        ...
        if (
            ...
            this.logger.log({
                message:
                    '[cody-rules-sync] skipping LLM file conversion: trial ended and no BYOK configured',
                context: CodyRulesSyncService.name,
                metadata: {
                    filePath: params.filePath,
                    repositoryId: params.repositoryId,
        ...
        const mainRun = options?.runName ?? 'codyRulesFileToRules';

        try {
            const result = await runStructuredReviewCall({
                byokConfig: byokConfigValue ?? undefined,
                schema: codyRulesIDEGeneratorSchema,
                organizationId:
                    params.organizationAndTeamData?.organizationId,
                runName: `${CodyRulesSyncService.name}::${mainRun}`,
                attrs: {
                    repositoryId: params.repositoryId,
                    filePath: params.filePath,
                    fallback: false,
                ...
                system: [
                                'Convert repository rule files (Cursor, Claude, GitHub rules, coding standards, etc.) into a JSON array of Cody Rules. IMPORTANT: Enforce exactly one rule per file. If multiple candidate rules exist, merge them COMPREHENSIVELY into one unified rule that preserves all essential details.',
                                ...
                                '       source "apps/api/.cody/rules/security.md" → "apps/api/**"',
                                ...
                                'CRITICAL — NEVER set path to a glob that would match the rule source files themselves: do NOT emit ".cursor/rules/**", ".cody/rules/**", "CLAUDE.md", ".cursorrules", ".github/instructions/**", or any other IDE-rule directory. Those host the rule, not the code it lints. If you find yourself wanting to do that, fall back to (3) or (4).',
            ...
            const normalizeRule = (rule: any): Partial<CreateCodyRuleDto> => {
                ...
                if (validated.reason !== 'accepted-as-is') {
                    ...
                    this.logger.log({
                        message: `[cody-rules-sync] path validation: ${validated.reason}`,
                        context: CodyRulesSyncService.name,
                        metadata: {
                            sourceFilePath: sourcePath,
                ...
                return {
                    ...
                    severity:
                        ...
                        CodyRuleSeverity.MEDIUM,
                    scope: (rule?.scope as any) || CodyRulesScope.FILE,
                    path: validated.path,
                    sourcePath,
            ...
            try {
                ...
                const { text: raw } =
                    await this.observabilityService.runAiSdkLLMInSpan<any>({
                        spanName: `${CodyRulesSyncService.name}::${fbRun}`,
                        runName: fbRun,
    ...
    private async convertFilesToCodyRulesFastBatch(params: {
        ...
        const mainRun = 'codyRulesFilesToRulesFastBatch';
        ...
        try {
            const { result } = await this.observabilityService.runLLMInSpan({
                spanName: `${CodyRulesSyncService.name}::${mainRun}`,
                runName: mainRun,
                attrs: {
                    repositoryId: params.repositoryId,
                ...
                exec: async (callbacks) => {
                    return await promptRunner
                        ...
                        .addPrompt({
                            ...
                            prompt: [
                                'You will receive multiple repository rule files. Return ONLY a JSON object { "rules": [...] } (no code fences) with up to 3 MOST IMPORTANT Cody Rules across all files (prioritize critical/high impact, security/compliance, or broad applicability). If none, return { "rules": [] }.',
            ...
            return (result.rules as Array<Partial<CreateCodyRuleDto>>)
                .slice(0, 3)
                .map((rule) => ({
                    ...rule,
                    repositoryId:
            ...
            try {
                ...
                return (parsed as Array<Partial<CreateCodyRuleDto>>)
                    .slice(0, 3)
                    .map((rule) => ({
                        ...rule,
                        repositoryId:
    ...
    private async convertManifestsToCodyRulesFastBatch(params: {
        files: Array<{ path: string; content: string }>;
        repositoryId: string;
        organizationAndTeamData: OrganizationAndTeamData;
    }): Promise<Array<Partial<CreateCodyRuleDto>>> {
        ...
        const mainProvider = LLMModelProvider.CEREBRAS_GLM_47;
        const mainFallback = LLMModelProvider.GROQ_MOONSHOTAI_KIMI_K2_;
        const mainRun = 'codyRulesManifestsToRulesFastBatch';
                    ...
                    return await promptRunner
                        ...
                        .addPrompt({
                            ...
                            prompt: [
                                ...
                                'Produce up to 3 HIGH-IMPACT Cody Rules tailored to this stack. Prioritize security/auth, secrets handling, logging/observability, testing/linting/type-check, dependency hygiene. Avoid generic style nits.',
    ...
    private async processContextReferences(params: {
        ...
        try {
            ...
            this.logger.log({
                message: 'Processed context references for synced cody rule',
                context: CodyRulesSyncService.name,
                metadata: {
                    ruleId,
                    repositoryId,
            ...
            this.logger.error({
                message:
                    'Failed to detect or persist context references for cody rule',
                context: CodyRulesSyncService.name,
                error,
                metadata: {
                    ruleId,
    ...
    /**
     * Verifica se um arquivo deve ser sincronizado forçadamente baseado na marcação @cody-sync
    ...
    private shouldForceSync(content: string): boolean {
        ...
        // Padrão para detectar @cody-sync (case insensitive, com word boundary)
        // Deve ter uma quebra de palavra antes do @ E depois de "sync" para evitar falsos positivos
        const syncPattern = /(?:^|[^a-zA-Z0-9._-])@cody-sync(?![a-zA-Z0-9_-])/i;
    ...
    /**
     * Verifica se um arquivo deve ser ignorado baseado na marcação @cody-ignore
     * A marcação pode estar no início ou final do arquivo
     */
    private shouldIgnoreFile(content: string): boolean {
        ...
        // Padrão para detectar @cody-ignore (case insensitive, com possíveis comentários)
        const ignorePattern = /@cody-ignore\b/i;
     ...
     * an `@cody-sync` marker (`pinnedSync === true`). The bulk pause/delete
    ...
    private async transitionIdeSyncRulesStatus(params: {
        organizationAndTeamData: OrganizationAndTeamData;
        repositoryId: string;
        targetStatus: CodyRulesStatus;
        onlyFromStatus?: CodyRulesStatus[];
        ...
        for (const rule of ideSyncRules) {
            ...
            if (targetStatus === CodyRulesStatus.DELETED) {
    ...
    async purgeAllIdeSyncRulesForRepository(params: {
        ...
        try {
            await this.transitionIdeSyncRulesStatus({
                ...params,
                targetStatus: CodyRulesStatus.DELETED,
            });
        } catch (error) {
            this.logger.error({
    ...
    async pauseAllIdeSyncRulesForRepository(params: {
        ...
        try {
            await this.transitionIdeSyncRulesStatus({
                ...params,
                targetStatus: CodyRulesStatus.PAUSED,
                onlyFromStatus: [CodyRulesStatus.ACTIVE],
            });
        } catch (error) {
            this.logger.error({
    ...
    async resumeAllIdeSyncRulesForRepository(params: {
        ...
        try {
            await this.transitionIdeSyncRulesStatus({
                ...params,
                targetStatus: CodyRulesStatus.ACTIVE,
                onlyFromStatus: [CodyRulesStatus.PAUSED],
            });
        } catch (error) {
            this.logger.error({
     ...
     * `@cody-sync` — those won't be touched by pause/delete bulk actions
    ...
    async countIdeSyncRulesForRepository(params: {
        ...
        for (const r of entity.rules as any[]) {
            ...
            if (r?.status === CodyRulesStatus.ACTIVE) counts.active += 1;
            else if (r?.status === CodyRulesStatus.PAUSED) counts.paused += 1;
            else if (r?.status === CodyRulesStatus.DELETED) {
                counts.deleted += 1;
            }
            if (
                r?.pinnedSync === true &&
                (r?.status === CodyRulesStatus.ACTIVE ||
                    r?.status === CodyRulesStatus.PAUSED)
            ) {
                counts.pinned += 1;
            }
        }
    ...
    private isGlobalSyncedRule(rule: any, sourceRepositoryId?: string): boolean {
        ...
        if (rule?.origin !== CodyRulesOrigin.GLOBAL_REPO_FILE_SYNC) return false;
        if (
    ...
    private selectGlobalRuleFromList(
        ...
        const found =
            newestFirst.find(
                (r: any) => r?.status !== CodyRulesStatus.DELETED,
            ) ?? newestFirst[0];
    ...
    async syncRepositoryGlobal(params: SyncTarget): Promise<void> {
        ...
        try {
            ...
            if (tier === 'free') {
                this.logger.log({
                    message:
                        '[cody-rules-global-sync] skipped: global rules import is not available on the Free plan',
                    context: CodyRulesSyncService.name,
                    metadata: {
                        organizationAndTeamData,
            ...
            for (const file of allFiles) {
                ...
                if (
                    existing &&
                    existing.status !== CodyRulesStatus.DELETED &&
                    currentSha &&
                ...
                // @cody-ignore still applies: remove any existing global rule
                // for this file.
                if (this.shouldIgnoreFile(decoded)) {
                    if (existing?.uuid) {
                ...
                const consumesBudget =
                    !existing?.uuid ||
                    existing.status !== CodyRulesStatus.ACTIVE;
                ...
                const dto: CreateCodyRuleDto = {
                    ...
                    origin: CodyRulesOrigin.GLOBAL_REPO_FILE_SYNC,
                    status: oneRule.status as any,
                    scope:
                        (oneRule.scope as CodyRulesScope) ||
                        CodyRulesScope.FILE,
                ...
                if (existing?.status === CodyRulesStatus.REJECTED) {
                    (dto as any).status = CodyRulesStatus.ACTIVE;
                }
            ...
            for (const rule of orgRules) {
                ...
                if (rule?.status === CodyRulesStatus.DELETED) continue;
            ...
            this.logger.log({
                message: `[cody-rules-global-sync] summary: ${syncOutcome.imported.length} imported, ${syncOutcome.unchanged.length} unchanged, ${syncOutcome.skipped.length} skipped, ${syncOutcome.removed.length} removed, ${syncOutcome.limitReached.length} over ${tier} limit (of ${allFiles.length} candidate file(s))`,
                context: CodyRulesSyncService.name,
                metadata: {
                    organizationAndTeamData,
                    sourceRepositoryId: repository.id,
    ...
    private async deleteGlobalRuleByUuid(params: {
        ...
        await this.deleteRuleInOrganizationByIdCodyRulesUseCase.execute(uuid, {
            source: 'web',
            organizationId: organizationAndTeamData.organizationId,
            teamId: organizationAndTeamData.teamId,
            userId: this.systemUserInfo.userId,
    ...
    async purgeGlobalRulesForSourceRepository(params: {
        ...
        try {
            ...
            this.logger.log({
                message: `[cody-rules-global-sync] purged ${removed} global rule(s) for removed source repository`,
                context: CodyRulesSyncService.name,
