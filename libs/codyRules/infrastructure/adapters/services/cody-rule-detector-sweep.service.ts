...
} from '../../../domain/contracts/codyRules.service.contract';
 ...
 */
@Injectable()
export class CodyRuleDetectorSweepService {
    private readonly logger = createLogger(CodyRuleDetectorSweepService.name);
    ...
    constructor(
        ...
        // Default ON; set CODY_RULES_DETECTOR_SWEEP_ENABLED=false to disable.
        this.enabled =
            process.env.CODY_RULES_DETECTOR_SWEEP_ENABLED !== 'false';
        this.maxRulesPerRun = Number(
            process.env.CODY_RULES_DETECTOR_SWEEP_MAX_PER_RUN ?? 2000,
        );
    }
    ...
    private async acquireLock(): Promise<DistributedLock | null> {
        try {
            return await this.distributedLockService.acquire(
                'CRON:CODY_RULES:DETECTOR_SWEEP',
