...
/**
 * Structured validation summaries for LONG cody rules.
 *
 ...
 * cody-rules-summary-productization.md — including the variants that were
...
/**
 * Verbatim from the validated eval (evals/cody-rules/decompose-rules.js).
...
/**
 * Verbatim from the validated experiment (evals/cody-rules/summarize-rules.js,
 ...
 * (the review then uses the full rule text). Mirrors the cody-rules-sync file
 * conversion gate.
 */
...
@Injectable()
export class CodyRuleSummaryService {
    private readonly logger = createLogger(CodyRuleSummaryService.name);
    ...
    hasValidSummary(rule: Partial<ICodyRule>): boolean {
        return (
     ...
     */
    resolveForReview(rule: Partial<ICodyRule>): Partial<ICodyRule> {
        ...
        if (rule.summary.sourceHash !== this.hashOf(rule.rule)) {
            this.logger.warn({
                message:
                    '[cody-rule-summary] stale summary (sourceHash mismatch) — using full rule text; a write path missed regeneration',
                context: CodyRuleSummaryService.name,
                metadata: {
                    ruleUuid: rule.uuid,
    ...
    async generate(
        ...
        try {
            ...
            if (
                ...
                this.logger.log({
                    message:
                        '[cody-rule-summary] skipping generation: trial ended and no BYOK configured',
                    context: CodyRuleSummaryService.name,
                    metadata: {
            ...
            const runName = 'cody-rules.summary-generation';
            ...
            if (
                ...
                this.logger.warn({
                    message:
                        '[cody-rule-summary] generated text missing required sections — discarding',
                    context: CodyRuleSummaryService.name,
                    metadata: {
                        organizationId: organizationAndTeamData.organizationId,
                        ruleUuid: rule.uuid,
            ...
            this.logger.warn({
                message:
                    '[cody-rule-summary] generation failed — review will use the full rule text',
                context: CodyRuleSummaryService.name,
                metadata: {
                    organizationId: organizationAndTeamData.organizationId,
                    ruleUuid: rule.uuid,
    ...
    async ensureSummaries(
        ...
        try {
            const doc = await this.codyRulesRepository.findByOrganizationId(
                organizationAndTeamData.organizationId,
            );
            ...
            this.logger.warn({
                message:
                    '[cody-rule-summary] could not resolve codyRules doc for persistence — summaries will be used in-memory only',
                context: CodyRuleSummaryService.name,
                metadata: {
                    organizationId: organizationAndTeamData.organizationId,
                    error:
        ...
        const generated = new Map<string, ICodyRuleSummary>();
        await mapLimit(pending, SUMMARY_CONCURRENCY, async (rule) => {
            ...
            if (docUuid) {
                try {
                    await this.codyRulesRepository.updateRule(
                        docUuid,
                        rule.uuid!,
                        { summary },
                    );
                    ...
                    this.logger.warn({
                        message:
                            '[cody-rule-summary] persist failed — summary used in-memory only for this review',
                        context: CodyRuleSummaryService.name,
                        metadata: {
                            organizationId:
                                organizationAndTeamData.organizationId,
        ...
        if (generated.size > 0) {
            this.logger.log({
                message: `[cody-rule-summary] backfilled ${generated.size}/${pending.length} long-rule summaries`,
                context: CodyRuleSummaryService.name,
                metadata: {
                    organizationId: organizationAndTeamData.organizationId,
     ...
     */
    atomsHashOf(rule: Partial<ICodyRule>): string {
        return this.hashOf(
            `${rule.rule ?? ''} ${JSON.stringify(rule.examples ?? [])}`,
        );
    }

    hasValidAtoms(rule: Partial<ICodyRule>): boolean {
        return (
    ...
    async generateAtoms(
        ...
        try {
            ...
            if (
                ...
                this.logger.log({
                    message:
                        '[cody-rule-atoms] skipping decomposition: trial ended and no BYOK configured',
                    context: CodyRuleSummaryService.name,
                    metadata: {
                        organizationId: organizationAndTeamData.organizationId,
                        ruleUuid: rule.uuid,
            ...
            const decomposed = (await runStructuredReviewCall({
                ...
                runName: 'cody-rules.atom-decomposition',
                organizationId: organizationAndTeamData.organizationId,
            ...
            if (raw.length === 0) {
                this.logger.warn({
                    message:
                        '[cody-rule-atoms] decomposition produced no atoms — rule stays on summary/full-text path',
                    context: CodyRuleSummaryService.name,
                    metadata: {
                        organizationId: organizationAndTeamData.organizationId,
                        ruleUuid: rule.uuid,
            ...
            const runCompiler = makeLLMRunCompiler(async ({ system, user }) => {
                const parsed = await runStructuredReviewCall({
                    ...
                    system,
                    user,
                    runName: 'cody-rules.atom-detector-compiler',
                    organizationId: organizationAndTeamData.organizationId,
                    observabilityService: this.observabilityService,
                });
            ...
            const items: ICodyRuleAtom[] = [];
            for (let i = 0; i < raw.length; i++) {
                const a = raw[i];
                const atom: ICodyRuleAtom = {
            ...
            this.logger.log({
                message: `[cody-rule-atoms] decomposed rule into ${items.length} atoms (${items.filter((a) => a.detector).length} T0)`,
                context: CodyRuleSummaryService.name,
                metadata: {
                    organizationId: organizationAndTeamData.organizationId,
                    ruleUuid: rule.uuid,
            ...
            this.logger.warn({
                message:
                    '[cody-rule-atoms] decomposition failed — review will fall back to summary/full text',
                context: CodyRuleSummaryService.name,
                metadata: {
                    organizationId: organizationAndTeamData.organizationId,
                    ruleUuid: rule.uuid,
    ...
    async ensureAtoms(
        ...
        if (allPending.length > pending.length) {
            this.logger.log({
                message: `[cody-rule-atoms] backfill budget: decomposing ${pending.length}/${allPending.length} pending rules this review — the rest fall back to summary/full text and are picked up next review`,
                context: CodyRuleSummaryService.name,
                metadata: {
                    organizationId: organizationAndTeamData.organizationId,
        ...
        try {
            ...
            this.logger.warn({
                message:
                    '[cody-rule-atoms] could not resolve codyRules doc for persistence — atoms used in-memory only',
                context: CodyRuleSummaryService.name,
                metadata: {
                    organizationId: organizationAndTeamData.organizationId,
                    error:
        ...
        const generated = new Map<string, ICodyRuleAtoms>();
        await mapLimit(pending, SUMMARY_CONCURRENCY, async (rule) => {
            ...
            if (docUuid) {
                try {
                    ...
                    this.logger.warn({
                        message:
                            '[cody-rule-atoms] persist failed — atoms used in-memory only for this review',
                        context: CodyRuleSummaryService.name,
                        metadata: {
                            organizationId:
                                organizationAndTeamData.organizationId,
     ...
     */
    expandForReview(rule: Partial<ICodyRule>): Partial<ICodyRule>[] {
        ...
        if (rule.atoms.sourceHash !== this.atomsHashOf(rule)) {
            this.logger.warn({
                message:
                    '[cody-rule-atoms] stale atoms (sourceHash mismatch) — falling back to summary/full text',
                context: CodyRuleSummaryService.name,
                metadata: {
                    ruleUuid: rule.uuid,
                    ruleTitle: rule.title,
