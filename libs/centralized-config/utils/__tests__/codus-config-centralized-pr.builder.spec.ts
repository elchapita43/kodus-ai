...
import { buildCodusConfigCentralizedMutationRequest } from '../codus-config-centralized-pr.builder';
...
describe('buildCodusConfigCentralizedMutationRequest — directory group flow', () => {
    it('emits a single upsert at the encoded folder when paths are unchanged', () => {
        const service = buildServiceMock();
        const req = buildCodusConfigCentralizedMutationRequest({
            ...
            description: 'd',
            commitMessage: 'c',
            sourceBranchPrefix: 'codus-test',
        });

        const files = (req.files as any)({ repositoryFolder: 'repo-1-name' });
        expect(files).toHaveLength(1);
        expect(files[0]).toMatchObject({
            path: 'repo-1-name/app%2Fapi&app%2Fweb/codus-config.yml',
            operation: 'upsert',
        });
    ...
    it('emits upsert(new) + delete(old config + old rules) when paths change', () => {
        ...
        expect(paths).toEqual([
            'delete repo-1-name/app%2Fapi/.cody-rules/memories/style-guide.yml',
            'delete repo-1-name/app%2Fapi/.cody-rules/review/naming.yml',
            'delete repo-1-name/app%2Fapi/.cody-rules/review/no-console.yml',
            'delete repo-1-name/app%2Fapi/codus-config.yml',
            'upsert repo-1-name/app%2Fapi&app%2Fweb/codus-config.yml',
        ]);
    });
    ...
    it('moves rule files to the new folder (upsert+delete) when content is provided on a rename', () => {
        ...
        const paths = files.map((f: any) => `${f.operation} ${f.path}`).sort();
        expect(paths).toEqual([
            'delete repo-1-name/app%2Fapi/.cody-rules/memories/style.yml',
            'delete repo-1-name/app%2Fapi/.cody-rules/review/no-console.yml',
            'delete repo-1-name/app%2Fapi/codus-config.yml',
            'upsert repo-1-name/app%2Fapi&app%2Fweb/.cody-rules/memories/style.yml',
            'upsert repo-1-name/app%2Fapi&app%2Fweb/.cody-rules/review/no-console.yml',
        ]);
    ...
    it('deletes the current folder when content is cleared and paths are unchanged', () => {
        ...
        expect(files).toEqual([
            {
                path: 'repo-1-name/app%2Fapi/codus-config.yml',
                operation: 'delete',
            },
        ]);
    });
    ...
    it('uses a single-segment folder name for a single-path group', () => {
        ...
        const files = (req.files as any)({ repositoryFolder: 'repo-1-name' });
        expect(files).toHaveLength(1);
        expect(files[0].path).toBe('repo-1-name/app%2Fapi/codus-config.yml');
    ...
    it('routes non-group (legacy directoryPath) requests through the flat path branch', () => {
        ...
        const files = (req.files as any)({ repositoryFolder: 'repo-1-name' });
        expect(files).toHaveLength(1);
        expect(files[0].path).toBe('repo-1-name/apps/web/codus-config.yml');
