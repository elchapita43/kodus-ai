...
interface BuildCodusConfigCentralizedMutationRequestParams {
    ...
    configFileContent?: Partial<CodusConfigFile> | null;
    title: string;
    description: string;
...
export function buildCodusConfigCentralizedMutationRequest(
    params: BuildCodusConfigCentralizedMutationRequestParams,
): CentralizedMutationPullRequestRequest {
    ...
    return {
        ...
        files: ({ repositoryFolder }) => {
            ...
            const path = params.centralizedConfigPrService.buildCentralizedPath({
                repositoryFolder,
                relativePath: buildCodusConfigRelativePath(
...
function buildCodusConfigRelativePath(directoryPath?: string): string {
    if (!directoryPath) {
        return 'codus-config.yml';
    }

    return `${directoryPath}/codus-config.yml`;
}
