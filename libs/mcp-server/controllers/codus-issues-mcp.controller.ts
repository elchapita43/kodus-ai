...
export class CodusIssuesMcpController {
    private readonly logger = createLogger(CodusIssuesMcpController.name);

    constructor(
        private readonly mcpServerService: CodusIssuesMcpServerService,
    ) {}

    @Post()
    @ApiOperation({
        summary: 'Handle Codus Issues MCP client request',
        description:
    ...
    async handleClientRequest(@Body() body: any, @Res() res: Response) {
        return handleStatelessMcpPost({
            ...
            errorContext: CodusIssuesMcpController.name,
            errorMessage: 'Error handling Codus Issues MCP request',
            logger: this.logger,
        });
    }
    ...
    @Get()
    @ApiOperation({
        summary: 'GET is not supported for this Codus Issues MCP endpoint',
        description:
    ...
    @Delete()
    @ApiOperation({
        summary: 'DELETE is not supported for this Codus Issues MCP endpoint',
        description:
