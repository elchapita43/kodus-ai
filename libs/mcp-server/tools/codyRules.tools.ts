...
type CodyRuleInput = Required<
    Omit<
        ...
        // the `@cody-sync` pin doesn't apply.
        | 'pinnedSync'
    >
> & {
...
type CodyRuleMemoryInput = Required<
    Omit<
        ICodyRuleMemory,
        | 'uuid'
        | 'createdAt'
...
interface CodyRulesResponse extends BaseResponse {
    data: Partial<ICodyRule>[];
}

interface CreateCodyRuleResponse extends BaseResponse {
    data: Partial<ICodyRule>;
    message?: string;
    prUrl?: string;
    link?: string;
...
interface DeleteCodyRuleResponse extends BaseResponse {
    message?: string;
    prUrl?: string;
}
...
@Injectable()
export class CodyRulesTools {
    private readonly logger = createLogger(CodyRulesTools.name);
    ...
    getCodyRules(): McpToolDefinition {
        ...
        return {
            ...
            description:
                'Get all Cody Rules at organization level. Use this to see organization-wide coding standards, global rules that apply across all repositories, or when you need a complete overview of rules. Returns rules with ACTIVE and PENDING status.',
            inputSchema,
            outputSchema: z.object({
                ...
                data: z.array(
                    z.looseObject({
                        ...
                        status: z.enum(CodyRulesStatus).optional(),
                        ...
                        scope: z.enum(CodyRulesScope).optional(),
                        directoryId: z.string().nullable().optional(),
                    }),
                ),
            }),
            execute: wrapToolHandler(
                async (args: InputType): Promise<CodyRulesResponse> => {
                    const params = {
                    ...
                    const allRules: Partial<ICodyRule>[] = entity.rules || [];

                    const rules: Partial<ICodyRule>[] = allRules.filter(
                        (rule: Partial<ICodyRule>) =>
                            rule.status === CodyRulesStatus.ACTIVE ||
                            rule.status === CodyRulesStatus.PENDING,
                    );
    ...
    getCodyRulesRepository(): McpToolDefinition {
        const inputSchema = z.object({
            organizationId: z
                .string()
                .describe(
        ...
        return {
            ...
            description:
                'Get Cody Rules specific to a particular repository. Use this to see repository-specific coding standards, rules that only apply to one codebase, or when analyzing rules for a specific project. More focused than get_cody_rules. Returns rules with ACTIVE and PENDING status.',
            inputSchema,
            outputSchema: z.object({
                success: z.boolean(),
                count: z.number(),
            ...
            execute: wrapToolHandler(
                async (args: InputType): Promise<CodyRulesResponse> => {
                    ...
                    const repositoryRules: Partial<ICodyRule>[] =
                        allRules.filter(
                            (rule: Partial<ICodyRule>) =>
                                rule.repositoryId &&
    ...
    createCodyRule(): McpToolDefinition {
        const inputSchema = z.object({
            organizationId: z
                .string()
                .describe(
        ...
        return {
            ...
            description:
                'Create a new Cody Rule with custom scope and severity. pull_request scope: analyzes entire PR context for PR-level rules. file scope: analyzes individual files one by one for file-level rules. Rule starts in pending status and must be approved in the UI before it takes effect. After execution, ALWAYS inform the user of: (1) the rule was created and is pending approval, and (2) the provided link to open the pending Cody Rules page to review and approve it. If centralized config is enabled the rule will be published to a pull request pending to be approved instead, and a prUrl is returned.',
            inputSchema,
            outputSchema: z.object({
                success: z.boolean(),
                count: z.number(),
                ...
                link: z
                    ...
                    .optional()
                    .describe(
                        'Link to the pending Cody Rules page where the rule can be reviewed and approved',
                    ),
            }),
            execute: wrapToolHandler(
                async (args: InputType): Promise<CreateCodyRuleResponse> => {
                    const params: {
                        organizationAndTeamData: OrganizationAndTeamData;
                        codyRule: CodyRuleInput;
                    } = {
                        organizationAndTeamData: {
                            organizationId: args.organizationId,
                            ...(args.codyRule.teamId
                                ? { teamId: args.codyRule.teamId }
                                : {}),
                        },
                        codyRule: {
                            title: args.codyRule.title,
                            type: CodyRulesType.STANDARD,
                            rule: args.codyRule.rule,
                            severity: args.codyRule.severity,
                            scope: args.codyRule.scope,
                            examples: (args.codyRule.examples ||
                                []) as ICodyRulesExample[],
                            origin: CodyRulesOrigin.MCP_AGENT,
                            status: CodyRulesStatus.PENDING,
                            repositoryId:
                                args.codyRule.repositoryId || 'global',
                            path:
                                (args.codyRule.scope === CodyRulesScope.FILE
                                    ? args.codyRule.path
                                    : '') || '',
                            directoryId:
                                (args.codyRule.scope === CodyRulesScope.FILE
                                    ? args.codyRule.directoryId
                                    : '') || '',
                            inheritance: {
                                inheritable:
                                    args.codyRule.inheritance?.inheritable ??
                                    true,
                                exclude:
                                    args.codyRule.inheritance?.exclude || [],
                                include:
                                    args.codyRule.inheritance?.include || [],
                            },
                        },
                    };
                    ...
                    const createGroupFolderName =
                        await this.centralizedConfigPrService.resolveDirectoryGroupFolderName(
                            ...
                            params.codyRule.repositoryId,
                            params.codyRule.directoryId,
                        );
                    ...
                    const centralizedPr =
                        await this.centralizedConfigPrService.createMutationPullRequestIfEnabled(
                            buildCodyRuleCentralizedMutationRequest({
                                ...
                                organizationAndTeamData:
                                    params.organizationAndTeamData,
                                repositoryId: params.codyRule.repositoryId,
                                groupFolderName:
                                    createGroupFolderName ?? undefined,
                                ruleContent: params.codyRule,
                    ...
                    if (centralizedPr.mode === 'centralized-pr') {
                        return {
                            ...
                            count: 1,
                            data: {
                                title: params.codyRule.title,
                                rule: params.codyRule.rule,
                                status: CodyRulesStatus.PENDING,
                            },
                    ...
                    const result: Partial<ICodyRule> =
                        await this.codyRulesService.createOrUpdate(
                            params.organizationAndTeamData,
                            params.codyRule,
                            {
                                userId: 'cody-system-tool',
                    ...
                    const awaitingApproval =
                        result?.status === CodyRulesStatus.PENDING;
                    const message = awaitingApproval
                        ? 'Rule created and awaiting approval. Open the Cody Rules page to review and approve it.'
    ...
    updateCodyRule(): McpToolDefinition {
        const inputSchema = z.object({
            organizationId: z
                .string()
                .describe(
        ...
        return {
            ...
            description:
                'Update an existing Cody Rule. Only the fields provided in codyRule will be updated. Use this to modify rule details, change severity, scope, or status of existing rules. After execution, ALWAYS inform the user of the provided link to open the rule in the Cody Rules page. If centralized config is enabled the update will be published to a pull request pending to be approved instead, and a prUrl is returned.',
            inputSchema,
            outputSchema: z.object({
                success: z.boolean(),
                count: z.number(),
                ...
                link: z
                    ...
                    .optional()
                    .describe(
                        'Link to open the updated Cody Rule in the app (or the pull request URL when centralized config is enabled)',
                    ),
            }),
            execute: wrapToolHandler(
                async (args: InputType): Promise<CreateCodyRuleResponse> => {
                    ...
                    const codyRule: CreateCodyRuleDto = {
                        ...
                        ...(args.codyRule.rule && { rule: args.codyRule.rule }),
                        ...(args.codyRule.severity && {
                            severity: args.codyRule.severity,
                        }),
                        ...(args.codyRule.scope && {
                            scope: args.codyRule.scope,
                        }),
                        ...(args.codyRule.repositoryId && {
                            repositoryId: args.codyRule.repositoryId,
                        }),
                        ...(args.codyRule.path && { path: args.codyRule.path }),
                        ...(args.codyRule.examples && {
                            examples: args.codyRule.examples.map((example) => ({
                                snippet: example.snippet || '',
                                isCorrect: example.isCorrect || false,
                            })),
                        }),
                        ...(args.codyRule.directoryId && {
                            directoryId: args.codyRule.directoryId,
                        }),
                        ...(args.codyRule.status && {
                            status: args.codyRule.status,
                        }),
                    };

                    const existingRule = await this.codyRulesService.findById(
                        args.ruleId,
                    );
                    ...
                    const result =
                        await this.codyRulesService.updateRuleWithLogging(
                            organizationAndTeamData,
                            mergedRule,
                            userInfo,
                        );
    ...
    deleteCodyRule(): McpToolDefinition {
        const inputSchema = z.object({
            organizationId: z
                .string()
                .describe(
        ...
        return {
            ...
            description:
                'Delete a Cody Rule permanently from the system. This action cannot be undone. Use this to remove rules that are no longer needed or relevant. If centralized config is enabled the deletion will be published to a pull request pending to be approved.',
            inputSchema,
            outputSchema: z.object({
                success: z.boolean(),
                message: z.string().optional(),
            ...
            execute: wrapToolHandler(
                async (args: InputType): Promise<DeleteCodyRuleResponse> => {
                    const result =
                    ...
                    return {
                        success: result,
                        ...(result
                            ? { message: 'Cody Rule deleted successfully' }
                            : { message: 'Failed to delete Cody Rule' }),
                    };
                },
            ),
        };
                ...
                async (args: InputType): Promise<CreateMemoryRuleResponse> => {
                    const params: {
                        ...
                        codyRule: {
                            ...
                            repositoryId:
                                args.codyRule.repositoryId || 'global',
                            directoryId: args.codyRule.directoryId || null,
                            path: args.codyRule.path || null,
                        },
                    };

                    const result: CreateOrUpdateMemoryResult | null =
                        await this.codyRulesService.createOrUpdateMemory(
                            params.organizationAndTeamData,
                            params.codyRule,
                            {
                                userId: 'cody-memory-mcp-tool',
                                userEmail: 'cody@kodus.io',
                            },
                        );
                    ...
                    const awaitingApproval =
                        resultStatus === CodyRulesStatus.PENDING;
        ...
        return {
            ...
            execute: wrapToolHandler(
                async (args: InputType): Promise<FindMemoriesResponse> => {
                    const memories = await this.codyRulesService.findMemories(
                        {
                            organizationId: args.organizationId,
                            teamId: args.teamId,
                        },
