...
@Injectable()
export class CodusIssuesMcpServerFactory {
    private readonly logger = createLogger(CodusIssuesMcpServerFactory.name);
    private registeredToolsCache?: RegisteredToolDefinition[];

    constructor(private readonly codusIssuesTools: CodusIssuesTools) {}

    async create(): Promise<StatelessMcpRequestHandler> {
        const server = new McpServer(
            {
                name: 'issues-by-codus',
                version: '1.0.0',
            },
            {
                capabilities: {
    ...
    private getRegisteredTools(): RegisteredToolDefinition[] {
        ...
        this.registeredToolsCache = this.codusIssuesTools
            .getAllTools()
            .map((tool) => {
