...
import { ICodyIssuesManagementService } from '@libs/code-review/domain/contracts/CodyIssuesManagement.contract';
import {
...
@Injectable()
export class CodyIssuesManagementService implements ICodyIssuesManagementService {
    private readonly logger = createLogger(CodyIssuesManagementService.name);
    ...
    constructor(
        ...
        @Inject(CODY_ISSUES_ANALYSIS_SERVICE_TOKEN)
        private readonly codyIssuesAnalysisService: CodyIssuesAnalysisService,
        ...
        try {
            ...
            const validationResult =
                await this.permissionValidationService.validateExecutionPermissions(
                    ...
                    userGitId,
                    CodyIssuesManagementService.name,
                );
            ...
            if (shouldAutomaticallyCreateIssues) {
                this.logger.log({
                    ...
                    context: CodyIssuesManagementService.name,
                    metadata: params,
                });
    ...
    async mergeSuggestionsIntoIssues(
        ...
        try {
            ...
            const mergeResult =
                await this.codyIssuesAnalysisService.mergeSuggestionsIntoIssues(
    ...
    async resolveExistingIssues(
        ...
        try {
            ...
            for (const file of files) {
                ...
                const llmResult =
                    await this.codyIssuesAnalysisService.resolveExistingIssues(
                        context,
                        promptData,
                        byokConfig,
                    );
    ...
    private async filterValidSuggestionsFromPrByStatus(
        ...
        const discardedStatuses = [
            ...
            PriorityStatus.DISCARDED_BY_CODY_FINE_TUNING,
                ...
                try {
                    ...
                    if (fullSuggestion) {
                        return {
                            ...
                            brokenCodyRulesIds:
                                fullSuggestion.brokenCodyRulesIds,
    ...
    private applyIssuesFilters(
        ...
        return allSuggestions.filter((suggestion) => {
            ...
            if (
                ...
                suggestion?.label !== 'cody_rules'
            )
                return false;
            if (
                !sourceFilters?.includeCodyRules &&
                suggestion?.label === 'cody_rules'
            )
                return false;
            return true;
