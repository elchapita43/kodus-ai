...
@Injectable()
export class GenerateCodusConfigFileUseCase {
    private readonly logger = createLogger(GenerateCodusConfigFileUseCase.name);
    ...
    async execute(
        ...
        try {
            ...
            let config: CodusConfigFile | undefined;
            ...
            if (repositoryId === 'global') {
                ...
                config = directory?.configs as CodusConfigFile | undefined;
            } else if (repositoryId && repositoryId !== 'global') {
                const repo = codeReviewConfigs.configValue.repositories?.find(
                    (repository) => repository.id === repositoryId,
                );

                config = repo?.configs as CodusConfigFile | undefined;
            }

            return this.getCodyConfigFile(config);
        } catch (error) {
            this.logger.error({
                message: 'Failed to generate Codus config file!',
                context: GenerateCodusConfigFileUseCase.name,
                error: error,
                metadata: {
            ...
            throw new Error(
                `Failed to generate Codus config file for team ${teamId}${repositoryId ? ` and repository ${repositoryId}` : ''}: ${error.message}`,
                { cause: error },
            );
        }
    }

    private getCodyConfigFile(configObject?: CodusConfigFile): {
        yamlString: string;
    } {
        ...
        if (configObject && !this.isEmptyObject(configObject)) {
            ...
            const codusDefaultConfigYMLfile = yaml.load(
                fs.readFileSync('default-codus-config.yml', 'utf8'),
            ) as CodusConfigFile;
            yamlString = yaml.dump(codusDefaultConfigYMLfile);
        }
