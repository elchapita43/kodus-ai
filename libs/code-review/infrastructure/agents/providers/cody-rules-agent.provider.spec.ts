...
// shard call) instead of the old codus-common builder.
...
describe('CodyRulesAgentProvider — rule formatting and applicability', () => {
    let provider: CodyRulesAgentProvider;

    const formatRules = (rules: any[], changedFiles: any[]): string =>
        (provider as any).formatCodyRules(rules, changedFiles);

    const matches = (filePath: string, pattern: string): boolean =>
        (provider as any).matchesPathPattern(filePath, pattern);

    beforeEach(() => {
        provider = new CodyRulesAgentProvider(
    ...
    describe('formatCodyRules — simple rules', () => {
    ...
    describe('formatCodyRules — examples', () => {
    ...
    describe('formatCodyRules — external file reference', () => {
            ...
            const rules = [
                {
                    ...
                    type: CodyRulesType.STANDARD,
                    status: 'active',
                    sourcePath: 'elchapita43/design-system/docs/conventions.md',
                },
            ];
            const out = formatRules(rules, [{ filename: 'a.ts' }]);

            expect(out).toContain(
                '**Reference**: `elchapita43/design-system/docs/conventions.md`',
            );
            expect(out).toContain('use readFile');
            expect(out).toContain('readReference');
        });
    ...
    describe('execute — short-circuits before calling LLM', () => {
        it('returns empty suggestions when no rules are provided', async () => {
            ...
            expect(result.agentName).toBe('codus-rules-review-agent');
...
describe('CodyRulesAgentProvider.execute — sharded end-to-end (#1449)', () => {
    ...
    function makeProvider(responses: any[]) {
        ...
        const provider = new CodyRulesAgentProvider(
    ...
    it('semantic path: judge violations become tagged CodeSuggestions', async () => {
        ...
        expect((out.suggestions[0] as any).brokenCodyRulesIds).toEqual([
            'no-any',
        ]);
    ...
    it('mixed: detector + judge merge into one output (one LLM call)', async () => {
        ...
        const ids = out.suggestions
            .map((s: any) => s.brokenCodyRulesIds?.[0])
            .sort();
    ...
    it('throws when ALL judge shards fail (no silent 0-finding review)', async () => {
        ...
        ).rejects.toThrow(/Cody Rules could not be evaluated: all 1 rule/i);
    });
...
describe('CodyRulesAgentProvider — Context OS reference loading', () => {
    const makeInput = () => ({
        prNumber: 7,
        organizationAndTeamData: {
    ...
    const makeProvider = (loader?: any) => {
        const p = new CodyRulesAgentProvider(
            {} as any,
            {} as any,
            {} as any,
            undefined,
