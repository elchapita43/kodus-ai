...
import { fileMatchesRulePath } from '@libs/common/utils/cody-rules/file-patterns';
...
import {
    ...
    type ShardViolation,
} from '@libs/code-review/infrastructure/agents/collaborators/cody-rules-sharded.judge';
...
import { buildDetectorViolations } from '@libs/code-review/infrastructure/agents/collaborators/cody-rules-detector.compiler';
import {
    ReviewAgentIdentity,
    ReviewAgentInput,
    ReviewAgentOutput,
...
/**
 * Agent that validates code changes against Cody Rules (team-defined rules).
 *
 ...
 */
@Injectable()
export class CodyRulesAgentProvider extends BaseCodeReviewAgentProvider {
    private readonly shardLogger = createLogger('CodyRulesShardedAgent');
     ...
     */
    async execute(
        input: ReviewAgentInput & { codyRules?: Partial<ICodyRule>[] },
    ): Promise<ReviewAgentOutput> {
        const rules = (input.codyRules || []).filter(
            (r) => r.type !== CodyRulesType.MEMORY && r.status === 'active',
        );
        ...
        const formatted = this.formatCodyRules(rules, input.changedFiles);
        ...
        if (semanticRules.length > 0) {
            ...
            // no loop, no codus-common. Main = the org's BYOK model or our managed
            ...
            const runJudge: RunJudge = async ({ system, user, filename }) => {
                const parsed = await runStructuredReviewCall({
                    ...
                    system,
                    user,
                    runName: 'codus-rules-review-agent.shard',
                    organizationId:
            ...
            // every shard span under a `codus-rules-review-agent` trace, tagged
            ...
            // that evaluated none of its semantic cody-rules. Throw so the
            ...
            if (shardsRun > 0 && shardsErrored === shardsRun) {
                throw new Error(
                    `Cody Rules could not be evaluated: all ${shardsRun} rule check(s) failed to run. Your semantic Cody Rules were not applied to this PR.`,
                );
            }
            ...
            // cody-rules were never evaluated, and today that only shows up as
            ...
            if (shardsErrored > 0 && shardsErrored < shardsRun) {
                this.shardLogger.warn({
                    message: `[cody-rules] PARTIAL judge-shard failure for PR#${input.prNumber}: ${shardsErrored}/${shardsRun} shard(s) errored — the surviving ${shardsRun - shardsErrored} shard(s) posted, but the semantic cody-rules on the failed shard(s) were NOT evaluated. Review degraded (not failed). Check the shard warn logs for the cause (wire-schema 400 / provider blip / model unavailability).`,
                    context: this.getIdentity().name,
                    metadata: {
                        prNumber: input.prNumber,
                        shardsRun,
        ...
        // reconciliation, path canonicalization, cody-rule severity) so verify
     ...
     * formatted rule section is derived from `input.codyRules` each call
...
If no violations found, respond with \`{"reasoning": "Checked all rules, no violations found", "suggestions": []}\`.
  ...
  <Rules>
    ...
    private async inlineContextOsReferences(
        ...
        try {
            ...
            if (unresolved.length > 0) {
                this.shardLogger.warn({
                    message: `[cody-rules-shard] ${unresolved.length} rule(s) with a contextReferenceId resolved ZERO references for PR#${input.prNumber}; judging those without their referenced file(s)`,
                    context: this.getIdentity().name,
                    metadata: {
                        organizationAndTeamData: input.organizationAndTeamData,
                        prNumber: input.prNumber,
            ...
            this.shardLogger.warn({
                message: `[cody-rules-shard] Context OS reference load failed for PR#${input.prNumber}; judging without external refs: ${err instanceof Error ? err.message : String(err)}`,
                context: this.getIdentity().name,
                metadata: {
                    organizationAndTeamData: input.organizationAndTeamData,
                    prNumber: input.prNumber,
    ...
    /**
     * Format Cody Rules into a structured prompt section.
     * Filters rules by path applicability to changed files.
     */
    private formatCodyRules(
        rules: Partial<ICodyRule>[],
        ...
        this.shardLogger.log({
            message: `[cody-rules-eval] ${applicableRules.length}/${rules.length} rule(s) selected for the cody-rules agent (${changedFiles.length} changed file(s))`,
            context: CodyRulesAgentProvider.name,
            metadata: {
        ...
        const formatted = applicableRules.map((rule, i) => {
            ...
            if (rule.scope) {
                parts.push(
                    `**Scope**: ${rule.scope === CodyRulesScope.FILE ? 'Per-file' : 'Pull request level'}`,
                );
            }
