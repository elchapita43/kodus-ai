...
// cody-rules execute() override must go through it too, or its Vercel AI SDK
...
describe('CodyRulesAgentProvider.execute — Langfuse trace wrapping (#sharded-no-trace)', () => {
    function makeProvider(responses: any[]) {
        ...
        return new CodyRulesAgentProvider(
    ...
    it('runs the semantic judge INSIDE runAgentWithTrace, tagged with the agent identity + org/team/PR/repo', async () => {
        ...
        expect(meta).toMatchObject({
            traceName: 'codus-rules-review-agent',
            organizationId: 'org-abc',
            teamId: 'team-xyz',
            prNumber: 4242,
            repositoryId: 'repo-123',
