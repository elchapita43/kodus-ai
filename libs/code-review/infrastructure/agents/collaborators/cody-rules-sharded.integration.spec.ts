/**
 * Wiring validation for the sharded cody-rules path (#1449): the NEW integration
 ...
 * evals/cody-rules/sharded-experiment.js.
 */
import {
...
describe('sharded cody-rules — judge → mapAgentFindings wiring (#1449)', () => {
    const rules = [
        { uuid: 'no-console', title: 'no console', rule: 'no console.log', path: '**/*.ts' },
    ];
    const changedFiles = [file('src/a.ts', '5 +console.log(1)')];

    it('maps a shard violation to a CodeSuggestion tagged with brokenCodyRulesIds', async () => {
        ...
        const { violations } = await judgeCodyRulesSharded({
            changedFiles,
            rules,
            runJudge,
        });
        ...
        const mapped = mapAgentFindings(
            { findings: { suggestions: violations } },
            {
                ...
                prNumber: 1,
                isCodyRules: true,
                identityName: 'codus-rules-review-agent',
                labelPolicy: {
                    categoryLabel: 'cody_rules',
                    allowedLabels: ['bug'],
                    supportsMixed: false,
                },
            },
        ...
        expect((s as any).brokenCodyRulesIds).toEqual(['no-console']);
    ...
    it('drops a suggestion with an unknown ruleUuid at the mapper (cody-rules gate)', async () => {
        const mapped = mapAgentFindings(
            {
                findings: {
                    suggestions: [
...
describe('sharded cody-rules — T2 reference-inline (#1449)', () => {
 ...
 */
describe('sharded cody-rules — Context OS references inline into the shard', () => {
    const rule = () => ({
        uuid: 'r-ctx',
