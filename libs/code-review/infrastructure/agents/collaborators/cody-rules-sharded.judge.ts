/**
 * Deterministic sharded judge for cody-rules (issue #1449).
 *
 * The agentic CodyRulesAgentProvider under-covers because it lets the LLM
 ...
 * `ruleId` index, not a UUID. `judgeCodyRulesSharded` resolves it to a real
...
function ruleBlock(rules: Array<Partial<ICodyRule>>): string {
    return rules
        .map((r, i) => {
            const parts = [
                `[${i + 1}] ${r.title}`,
...
function rulesForFile(
    file: FileChange,
    rules: Array<Partial<ICodyRule>>,
): Array<Partial<ICodyRule>> {
    return rules.filter(
        (r) => !r.path || matchesPathPattern(file.filename, r.path),
    );
}

const isPrLevel = (r: Partial<ICodyRule>) =>
    r.scope === CodyRulesScope.PULL_REQUEST;
    ...
    return Promise.all(
        rules.map(async (rule) => {
            ...
            try {
                ...
                logger?.warn({
                    message: `cody-rules reference load failed for ${sourcePath} (rule ${rule.uuid}); judging without it`,
                    // context required or SimpleLogger.shouldSkipLog drops it
                    context: 'cody-rules-sharded',
...
export function inlineLoadedReferences(
    ...
    return rules.map((rule) => {
        ...
        logger?.log?.({
            message: `[cody-rules-shard] inlined ${inlined.length} reference file(s) for rule ${rule.uuid}: ${inlined.join(', ')}`,
            context: 'cody-rules-sharded',
            metadata: {
                ruleUuid: rule.uuid,
 ...
 * brokenCodyRulesIds and reconciles the uuid).
 */
export async function judgeCodyRulesSharded(
    ...
    const perFile = await mapLimit(
        ...
        async ({ file, applicable }) => {
            ...
            try {
                ...
                shardsErrored++;
                logger?.warn({
                    message: `[cody-rules-shard] file shard failed for ${file.filename} (${applicable.length} rule(s)) — degrading to zero findings: ${err instanceof Error ? err.message : String(err)}`,
    ...
    if (prRules.length > 0) {
        ...
        try {
            ...
            shardsErrored++;
            logger?.warn({
                message: `[cody-rules-shard] PR-scope shard failed (${prRules.length} rule(s)) — degrading to zero findings: ${err instanceof Error ? err.message : String(err)}`,
                context: 'cody-rules-sharded',
                metadata: { err },
            });
        }
