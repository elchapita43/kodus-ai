...
describe('judgeCodyRulesSharded — deterministic file×rule sweep (#1449)', () => {
    it('issues one shard per changed file that has applicable rules', async () => {
        const { run, calls } = fakeJudge({});
        const res = await judgeCodyRulesSharded({
            changedFiles: [
    ...
    it('logs WHY a shard failed instead of swallowing the error', async () => {
        ...
        expect(warn.mock.calls[0][0].context).toBe('cody-rules-sharded');
    });
