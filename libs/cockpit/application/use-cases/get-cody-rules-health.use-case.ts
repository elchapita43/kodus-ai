...
/**
 * "Cody Rules — health" table: merges warehouse usage (triggers /
 * implementation rate per rule, from `suggestions_mv.brokenCodyRulesIds`)
 * with rule metadata from Mongo `codyRules` — which is also what surfaces
 * ACTIVE rules that never triggered in the window (`stale`).
 *
 * The state classification lives in `domain/helpers/cody-rules-health.helper`
 * so report SERVICES can reuse it without importing this use-case.
 */

@Injectable()
export class GetCodyRulesHealthUseCase {
    constructor(
    ...
    async execute(q: CockpitRangeQuery): Promise<CodyRuleHealthRow[]> {
        const [usageRows, rulesDoc, warehouseRepoNames, scopeMaps] =
            await Promise.all([
                this.reviewAnalytics.getCodyRulesUsage(q),
                this.codyRulesService.findByOrganizationId(q.organizationId),
        ...
        const rules = (rulesDoc?.rules ?? []).filter(
            (r): r is ICodyRule & { uuid: string } =>
                Boolean(r.uuid) && r.status === CodyRulesStatus.ACTIVE,
        );
        ...
        const out: CodyRuleHealthRow[] = rules.map((rule) => {
