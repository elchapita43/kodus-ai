...
// read-side filter that drops Cody's own past comments stays in sync
// with what every provider emitter actually writes.
import { CODY_IDENTIFIERS } from '@libs/common/utils/cody-identifiers';

import { PlatformResponsePolicyFactory } from './policies/platform-response.policy';

// Constants
const CODY_COMMANDS = {
    BUSINESS_LOGIC_VALIDATION: '@cody -v business-logic',
    CODY_MENTION: '@cody',
    CODUS_MENTION: '@codus',
} as const;
...
const ACKNOWLEDGMENT_MESSAGES = {
    ...
    BUSINESS_LOGIC_INVALID_CONTEXT:
        'The "@cody -v business-logic" command can only be used in the general PR conversation, not in code suggestions or inline comments. Please use it in the main PR discussion thread.',
} as const;

/**
 * Posted instead of running the agent when the org may not use Codus-funded
 * LLM calls (cloud, past trial, no BYOK). A visible pointer beats silence:
 * without it the dev reads the missing reply as Cody being broken.
 */
const CONVERSATION_PLAN_GATE_MESSAGE =
    "I can't reply right now: your organization's trial has ended and no " +
    'LLM API key (BYOK) is configured. Connect your key in the Codus ' +
    'settings to keep chatting with Cody.';
...
class BusinessLogicValidationCommandHandler implements CommandHandler {
    canHandle(userQuestion: string): boolean {
        return userQuestion
            .toLowerCase()
            .trim()
            .startsWith(CODY_COMMANDS.BUSINESS_LOGIC_VALIDATION);
    }
...
class ConversationCommandHandler implements CommandHandler {
    canHandle(userQuestion: string): boolean {
        ...
        const startsWithMention =
            trimmedQuestion.startsWith(CODY_COMMANDS.CODY_MENTION) ||
            trimmedQuestion.startsWith(CODY_COMMANDS.CODUS_MENTION);
...
@Injectable()
export class ChatWithCodyFromGitUseCase {
    private readonly logger = createLogger(ChatWithCodyFromGitUseCase.name);
    ...
    async execute(params: WebhookParams): Promise<void> {
        this.logger.log({
            ...
            context: ChatWithCodyFromGitUseCase.name,
        ...
        try {
            ...
            this.logger.log({
                ...
                context: ChatWithCodyFromGitUseCase.name,
                serviceName: ChatWithCodyFromGitUseCase.name,
                metadata: {
                    platformType: params.platformType,
                    repository: repository.name,
                    pullRequestNumber,
    ...
    private async handleConversationFlow(
        ...
        if (this.shouldIgnoreComment(comment, params.platformType)) {
            this.logger.log({
                message:
                    'Comment made by Cody or does not mention Cody/Codus. Ignoring.',
                context: ChatWithCodyFromGitUseCase.name,
                metadata: {
                    repository: repository.name,
                    pullRequestNumber,
        ...
        // run on the Codus default model; cloud orgs past the trial without
        // BYOK are not funded by Codus — reply with a pointer to connect a
        ...
        const permission =
            await this.permissionValidationService.validateExecutionPermissions(
                organizationAndTeamData,
                undefined,
                ChatWithCodyFromGitUseCase.name,
            );
        ...
        const originalCodyComment = this.getOriginalCodyComment(
            comment,
            normalizedComments,
            params.platformType,
        );
        ...
        const thread = createThreadId(
            {
                ...
                repositoryId: repository.id,
                userId: sender.id,
                suggestionCommentId: originalCodyComment?.id || comment?.id,
            },
            {
                prefix: 'cmc',
            },
        ...
        if (!response) {
            this.logger.warn({
                message: 'No response generated by Cody',
                context: ChatWithCodyFromGitUseCase.name,
                metadata: {
                    repository: repository.name,
                    pullRequestNumber,
    ...
    private getReviewThreadByCommentId(
        ...
        try {
            ...
            this.logger.error({
                ...
                context:
                    'ChatWithCodyFromGitUseCase.getReviewThreadByCommentId',
                error,
    ...
    private shouldIgnoreComment(
        ...
        return (
            this.isCodyComment(comment, platformType) ||
            !this.mentionsCody(comment)
        );
    }

    private getOriginalCodyComment(
        comment: Comment,
        allComments: Comment[],
        platformType: PlatformType,
    ): Comment | undefined {
            ...
            case PlatformType.GITHUB:
                return allComments.filter(
                    (reply) =>
                        ...
                        !this.isCodyComment(reply, platformType),
                );
            case PlatformType.BITBUCKET:
                if (comment.parent?.id) {
                    const originalComment = allComments.find(
    ...
    private prepareContext({
        ...
        return {
            ...
            codeManagementContext: {
                originalComment: {
                    suggestionCommentId: originalCodyComment?.id,
                    suggestionFilePath: comment?.path,
                    suggestionText: originalCodyComment?.body,
                    diffHunk: originalCodyComment?.diff_hunk,
                },
    ...
    private mentionsCody(comment: Comment): boolean {
        const commentBody = comment.body.toLowerCase();
        return [CODY_COMMANDS.CODY_MENTION, CODY_COMMANDS.CODUS_MENTION].some(
            (keyword) => commentBody.startsWith(keyword),
        );
    }

    private isCodyComment(
        comment: Comment,
        platformType: PlatformType,
    ): boolean {
        const login =
        ...
        const bodyWithoutMarkdown =
            platformType !== PlatformType.BITBUCKET
                ? CODY_IDENTIFIERS.MARKDOWN_IDENTIFIERS.DEFAULT
                : CODY_IDENTIFIERS.MARKDOWN_IDENTIFIERS.BITBUCKET;

        return (
            CODY_IDENTIFIERS.LOGIN_KEYWORDS.some((keyword) =>
    ...
    private getAcknowledgmentIds(
        ...
        switch (platformType) {
            case PlatformType.GITHUB:
                ...
                parentId = originalCodyComment?.id;
                break;
            case PlatformType.GITLAB:
                ackResponseId = ackResponse.id;
                parentId = comment?.id;
            ...
            case PlatformType.BITBUCKET:
                ...
                parentId =
                    ackResponse.parent?.id === comment?.id
                        ...
                        : originalCodyComment?.id;
                break;
            case PlatformType.AZURE_REPOS:
                ackResponseId = ackResponse?.id;
                parentId = originalCodyComment?.threadId;
                break;
            default:
                this.logger.warn({
                    message: `Unsupported platform type: ${platformType}`,
    ...
    private extractCustomInstructions(
        ...
        const candidates: unknown[] = [
            ...
            payload?.cody?.customInstructions,
            payload?.cody?.custom_instructions,
