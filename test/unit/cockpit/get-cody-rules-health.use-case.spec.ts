...
import { computeRuleState } from '@libs/cockpit/domain/helpers/cody-rules-health.helper';
...
describe('GetCodyRulesHealthUseCase', () => {
    const baseQuery = {
    ...
    const mkUseCase = (
        ...
        const reviewAnalytics = {
            getCodyRulesUsage: jest.fn().mockResolvedValue(usageRows),
        ...
        return new GetCodyRulesHealthUseCase(
    ...
    it('handles orgs without a codyRules doc', async () => {
    ...
    it('labels scope: global sentinel → null, repo → resolved name, folder → resolved directory name', async () => {
        const useCase = mkUseCase(
            ...
            new Map([['670345891', 'elchapita43/codus-ai']]),
            [
    ...
    it('resolves repo name from the integration config when the warehouse has no PRs for it', async () => {
        const useCase = mkUseCase(
            ...
            [{ id: '670345891', full_name: 'elchapita43/codus-ai' }],
        );
