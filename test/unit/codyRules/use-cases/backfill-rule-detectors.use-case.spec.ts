import { BackfillRuleDetectorsUseCase } from '@libs/codyRules/application/use-cases/backfill-rule-detectors.use-case';
...
function make(rules: any[], compileResult: (rule: any) => any) {
    ...
    return { uc, compileAndSave, codyRulesService };
}
...
describe('BackfillRuleDetectorsUseCase (#1449 T0 activation)', () => {
    it('compiles only eligible rules and tallies the outcome', async () => {
        const rules = [
            ...
            rule({ uuid: 'd', type: CodyRulesType.MEMORY }), // skipped
