...
/**
 * Repo-scope enforcement for `POST /cody-rules/imported/manage`.
 *
 * The endpoint guard only checks Update on cody_rules at the type level;
 ...
 */
describe('ManageImportedCodyRulesUseCase — repo scope', () => {
    ...
    const buildUseCase = async (user: Record<string, unknown> | null) => {
        ...
        return module.get(ManageImportedCodyRulesUseCase);
    };
