...
function createService(
    ...
    const service = new CodyRuleSummaryService(
...
describe('CodyRuleSummaryService', () => {
    beforeEach(() => {
        jest.clearAllMocks();
        ...
        structuredCallMock.mockImplementation(async ({ runName }: any) => {
            if (runName === 'cody-rules.atom-decomposition') {
                return {
                    atoms: [
                        {
    ...
    describe('generate', () => {
        ...
        it('records the LLM call through the usage span (BYOK token accounting)', async () => {
            ...
            expect(
                ...
                expect.objectContaining({
                    runName: 'cody-rules.summary-generation',
    ...
    describe('ensureSummaries', () => {
        it('generates and persists only for long rules lacking a valid summary', async () => {
            ...
            const alreadySummarized: Partial<ICodyRule> = {
                uuid: 'ok',
                rule: LONG_TEXT,
                summary: {
                    content: validSummaryText,
            ...
            const shortRule: Partial<ICodyRule> = { uuid: 's', rule: 'short' };
            const pendingRule: Partial<ICodyRule> = {
                uuid: 'p',
                rule: LONG_TEXT,
            };
        ...
        it('returns rules untouched when generation fails', async () => {
            ...
            const rule: Partial<ICodyRule> = { uuid: 'p', rule: LONG_TEXT };
