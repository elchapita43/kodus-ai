import { CodyRuleDetectorSweepService } from '@libs/codyRules/infrastructure/adapters/services/cody-rule-detector-sweep.service';
...
function make(opts: {
    ...
    const prev = process.env.CODY_RULES_DETECTOR_SWEEP_ENABLED;
    if (opts.enabledEnv === undefined)
        delete process.env.CODY_RULES_DETECTOR_SWEEP_ENABLED;
    else process.env.CODY_RULES_DETECTOR_SWEEP_ENABLED = opts.enabledEnv;

    const release = jest.fn(async () => {});
    const lock = opts.lock === undefined ? { release } : opts.lock;
    const codyRulesService: any = {
    ...
    const svc = new CodyRuleDetectorSweepService(
        codyRulesService,
        backfill,
    ...
    const restore = () => {
        if (prev === undefined)
            delete process.env.CODY_RULES_DETECTOR_SWEEP_ENABLED;
        else process.env.CODY_RULES_DETECTOR_SWEEP_ENABLED = prev;
    };
    return { svc, codyRulesService, backfill, distributedLockService, release, restore };
}

describe('CodyRuleDetectorSweepService (#1449 continuous T0 sweep)', () => {
    ...
    it('skips entirely when the lock is held by another worker', async () => {
        const { svc, codyRulesService, backfill, restore } = make({
            lock: null,
            docs: [{ organizationId: 'o1' }],
        });
        await svc.sweep();
        expect(
            codyRulesService.findOrganizationIdsWithRules,
    ...
    it('stops at the per-run cap and defers remaining orgs', async () => {
        const prev = process.env.CODY_RULES_DETECTOR_SWEEP_MAX_PER_RUN;
        process.env.CODY_RULES_DETECTOR_SWEEP_MAX_PER_RUN = '5';
        ...
        if (prev === undefined)
            delete process.env.CODY_RULES_DETECTOR_SWEEP_MAX_PER_RUN;
        else process.env.CODY_RULES_DETECTOR_SWEEP_MAX_PER_RUN = prev;
        restore();
    });
