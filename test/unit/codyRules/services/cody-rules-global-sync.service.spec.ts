...
/**
 * Builds a CodyRulesSyncService with just the collaborators the global-sync
...
describe('CodyRulesSyncService.purgeGlobalRulesForSourceRepository', () => {
    it('soft-deletes only this source repo\'s global-synced rules, never user or other-source rules', async () => {
        const rules = [
            ...
            globalSyncedRule({ uuid: 'a-del', sourceRepositoryId: 'repo-A', status: CodyRulesStatus.DELETED }),
        ...
        const deletedUuids =
            deleteRuleInOrganizationByIdCodyRulesUseCase.execute.mock.calls.map(
                (c: any[]) => c[0],
            );
...
describe('CodyRulesSyncService.syncRepositoryGlobal', () => {
    ...
    it('short-circuits an unchanged file (same SHA) — no re-convert, no delete', async () => {
        ...
        const { service, codyRulesService, deleteRuleInOrganizationByIdCodyRulesUseCase } =
            createService({
                rules,
    ...
    it('re-imports a changed file (different SHA) tagged with source repo + new hash', async () => {
        ...
        const dto = codyRulesService.createOrUpdate.mock.calls[0][1];
    ...
    it('reactivates a previously REJECTED rule when the changed content is re-imported', async () => {
        ...
        const dto = codyRulesService.createOrUpdate.mock.calls[0][1];
        expect(dto.status).toBe(CodyRulesStatus.ACTIVE);
    });
});

describe('CodyRulesSyncService.syncRepositoryGlobal — plan gating', () => {
    const repository = {
        id: 'repo-A',
        name: 'standards',
    ...
    it('trial plan imports only the first 5 rules found and skips the rest', async () => {
        ...
        expect(codyRulesService.createOrUpdate).toHaveBeenCalledTimes(5);
    });
    ...
    it('paid plan imports every rule found (no cap)', async () => {
        ...
        expect(codyRulesService.createOrUpdate).toHaveBeenCalledTimes(7);
    });
});
