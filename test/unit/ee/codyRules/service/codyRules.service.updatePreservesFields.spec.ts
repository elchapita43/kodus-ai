 ...
 */
describe('CodyRulesService update/sync field preservation', () => {
    ...
    const existingRule: ICodyRule = {
        ...
        centralizedConfig: { path: '.codus/rules.yml' } as any,
        inheritance: {
            inheritable: true,
            exclude: ['repo-x'],
            include: [],
    ...
    const buildService = () => {
        ...
        const service = new CodyRulesService(
            ...
            { validateRulesLimit } as any, // codyRulesValidationService
    ...
    it('preserves every untouched field when only the status changes (pause)', async () => {
        ...
        await service.createOrUpdate(
            ...
            { uuid: 'rule-1', status: CodyRulesStatus.PAUSED } as any,
            userInfo,
        );
        ...
        expect(update.status).toBe(CodyRulesStatus.PAUSED);
    ...
    it('normalizes severity to lower-case while preserving the rest', async () => {
        ...
        expect(update.title).toBe('Original title');
        expect(update.status).toBe(CodyRulesStatus.ACTIVE);
    });
    ...
    it('sync de-pin path: flipping pinnedSync keeps title/severity/inheritance', async () => {
        ...
        // Mirrors codyRulesSync.service.ts de-pin: `{ ...rule, pinnedSync: false }`.
        await service.createOrUpdate(
            organizationAndTeamData,
    ...
    it('sync soft-delete path: flipping status to DELETED keeps the rule body', async () => {
        ...
        // Mirrors codyRulesSync.service.ts soft-delete: `{ ...rule, status: DELETED }`.
        await service.createOrUpdate(
            organizationAndTeamData,
            { ...existingRule, status: CodyRulesStatus.DELETED } as any,
            userInfo,
        );

        const update = capturedUpdate(updateRule);
        expect(update.status).toBe(CodyRulesStatus.DELETED);
