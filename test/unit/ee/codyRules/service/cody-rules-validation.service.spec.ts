...
const createRule = (
    overrides: Partial<ICodyRule> = {},
): Partial<ICodyRule> => ({
    uuid: overrides.uuid || Math.random().toString(36).slice(2),
    title: overrides.title || 'Title',
    rule: overrides.rule || 'Rule',
    type: overrides.type || CodyRulesType.STANDARD,
    status: overrides.status || CodyRulesStatus.ACTIVE,
...
describe('CodyRulesValidationService', () => {
    let service: CodyRulesValidationService;

    beforeEach(() => {
        shouldLimitResourcesMock.mockReset();
        service = new CodyRulesValidationService({
    ...
    describe('validateRulesLimit', () => {
        ...
        it('allows exactly MAX_CODY_RULES (10) at the boundary', async () => {
            ...
            expect(
                await service.validateRulesLimit(
                    { organizationId: 'org-1' } as any,
                    service.MAX_CODY_RULES,
                ),
            ).toBe(true);
            expect(
                await service.validateRulesLimit(
                    { organizationId: 'org-1' } as any,
                    service.MAX_CODY_RULES + 1,
                ),
            ).toBe(false);
        });
    });

    describe('filterCodyRules', () => {
        it('returns standard and memory rules separated in createdAt order', () => {
            ...
            const result = service.filterCodyRules(rules, 'repo-1');
    ...
    describe('Inheritance behavior — proving the "INHERITED: DIRECTORY" leak', () => {
        ...
        const b207a89c = (): Partial<ICodyRule> =>
            createRule({
        ...
        it('passes the inheritance check when viewing from OWN directory (cf5284b4)', () => {
            const result = service.getCodyRulesForFile(
        ...
        it('STOPS the leak when include is set to ONLY the own directoryId', () => {
            ...
            const pinned: Partial<ICodyRule> = {
                ...b207a89c(),
                inheritance: {
                    inheritable: true,
            ...
            const resultInOwnDir = service.getCodyRulesForFile(
                'qantilever/src/foo.kt',
                [pinned],
                {
                    repositoryId: '769144833',
            ...
            const resultInOtherDir = service.getCodyRulesForFile(
                'applications/backoffice-bff/src/foo.java',
                [pinned],
                {
                    repositoryId: '769144833',
    ...
    describe('inheritance matrix — global / repo / dir × inheritable / include / exclude', () => {
        ...
        const ruleAt = (
            ...
            inheritance: Partial<ICodyRule['inheritance']> = {},
        ): Partial<ICodyRule> =>
            createRule({
        ...
        it('global rule applies in repo view', () => {
            ...
            const out = service.getCodyRulesForFile(fileInDirA, [r], {
                repositoryId: REPO,
            });
        ...
        it('repo-level rule applies in any dir view of that repo (inherited)', () => {
            const r = ruleAt({ repositoryId: REPO });
            const out = service.getCodyRulesForFile(fileInDirB, [r], {
                repositoryId: REPO,
                directoryId: DIR_B,
            });
            expect(out).toHaveLength(1);
    ...
    describe('Bug 1 regression — rule from .cursorrules in subdirectory does not leak to unrelated paths', () => {
        ...
        const javaSpringArchRule = (pathValue: string): Partial<ICodyRule> =>
            createRule({
                ...
                // see libs/codyRules/infrastructure/adapters/services/codyRulesSync.service.ts
        ...
        it('DEMONSTRATES THE BUG: rule with raw "**/*" path still matches files outside the source subdirectory', () => {
            ...
            const result = service.getCodyRulesForFile(salesFlowFile, rules, {
                repositoryId: '769144833',
            });
        ...
        it('PROVES THE FIX: rule scoped to the source subdirectory does NOT match sales-flow files', () => {
            // Post-fix shape — scopePathToSourceDirectory in codyRulesSync.service.ts
        ...
        it('PROVES THE FIX: scoped rule still matches files inside its own subdirectory', () => {
            ...
            const result = service.getCodyRulesForFile(backofficeBffFile, rules, {
                repositoryId: '769144833',
            });
    ...
    describe('comma-joined multi-glob rule paths (#1483)', () => {
        ...
        it('keeps character classes intact (comma inside [] is not a separator)', () => {
            ...
            expect(
                service
                    .getCodyRulesForFile('src/a/main.ts', [rule], filters)
                    .map((r) => r.uuid),
            ).toEqual(['bracketed']);
            expect(
                service
                    .getCodyRulesForFile('docs/readme.md', [rule], filters)
                    .map((r) => r.uuid),
            ).toEqual(['bracketed']);
        });
        ...
        it('still matches single-glob paths as before', () => {
            ...
            expect(
                service
                    .getCodyRulesForFile('app/jobs/sync_job.rb', [rule], filters)
                    .map((r) => r.uuid),
            ).toEqual(['single']);
            expect(
                service.getCodyRulesForFile('app/models/user.rb', [rule], filters),
            ).toEqual([]);
        });
        ...
        it('matches folders against ANY of the comma-joined globs', () => {
            ...
            for (const folder of ['app/services', 'lib/tasks']) {
                const result = service.getCodyRulesForFolder(
                    folder,
                    [rule],
                    filters,
                );
            ...
            expect(
                service.getCodyRulesForFolder('app/models', [rule], filters),
            ).toEqual([]);
        });
    });
});
