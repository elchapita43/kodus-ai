 ...
 */
describe('CodyRulesService create gate (free-plan limit counts ACTIVE only)', () => {
    const organizationAndTeamData: OrganizationAndTeamData = {
        organizationId: 'org-1',
        teamId: 'team-1',
    };

    const buildService = (existingRules: Array<{ status: CodyRulesStatus }>) => {
    ...
    it('skips the quota check entirely when the new rule is created PAUSED (P4)', async () => {
        ...
        await service.createOrUpdate(
            organizationAndTeamData,
            { ...newRule, status: CodyRulesStatus.PAUSED },
 ...
 */
describe('CodyRulesService create gate enforces the 10-rule ceiling', () => {
    const organizationAndTeamData: OrganizationAndTeamData = {
        organizationId: 'org-1',
        teamId: 'team-1',
    };
    ...
    // A validator mirroring CodyRulesValidationService with limiting ON:
    // allowed only while the post-operation total stays within MAX.
    const buildEnforcingService = (
        existingRules: Array<{ status: CodyRulesStatus }>,
    ) => {
        const repositoryMock = {
            findByOrganizationId: jest
                .fn()
    ...
    const activeRules = (n: number) =>
        Array.from({ length: n }, () => ({ status: CodyRulesStatus.ACTIVE }));
    ...
    it('creates the 11th rule PAUSED+lockedByPlan instead of rejecting it', async () => {
        ...
        expect(persistedRule.status).toBe(CodyRulesStatus.PAUSED);
    ...
    it('never throws across a batch of creates past the cap — proves IDE-sync/import loops no longer abort mid-batch', async () => {
        ...
        for (const call of repositoryMock.addRule.mock.calls) {
            expect(call[1].status).toBe(CodyRulesStatus.PAUSED);
            expect(call[1].lockedByPlan).toBe(true);
        }
    });
});

describe('CodyRulesService.createOrUpdate severity normalization on update (P3)', () => {
    const organizationAndTeamData: OrganizationAndTeamData = {
        organizationId: 'org-1',
        teamId: 'team-1',
    };
