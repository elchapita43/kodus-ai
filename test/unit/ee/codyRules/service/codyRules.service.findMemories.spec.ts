...
describe('CodyRulesService.findMemories', () => {
    const organizationAndTeamData: OrganizationAndTeamData = {
        organizationId: 'org-1',
        teamId: 'team-1',
    };

    const createMemoryRule = (overrides: Partial<ICodyRule>): ICodyRule => ({
    ...
    const setup = (rules: ICodyRule[]) => {
        const repositoryMock = {
            findByOrganizationId: jest
                .fn()
                .mockResolvedValue({ rules } as unknown as ICodyRules),
        };
        ...
        const service = new CodyRulesService(
            ...
            { registerCodyRulesLog: jest.fn() } as any,
            {} as any,
            {} as any,
            validationService,
            {} as any,
