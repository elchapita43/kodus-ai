import { CodyRulesLogHandler } from '@libs/ee/codeReviewSettingsLog/infrastructure/adapters/services/codyRulesLog.handler';
import {
    ActionType,
    ConfigLevel,
...
describe('CodyRulesLogHandler', () => {
    let handler: CodyRulesLogHandler;
    let mockUnified: ReturnType<typeof createMockUnifiedLogHandler>;

    beforeEach(() => {
        mockUnified = createMockUnifiedLogHandler();
        handler = new CodyRulesLogHandler(mockUnified as any);
    });
    ...
    describe('config level determination', () => {
        it('no repo → GLOBAL', async () => {
            await handler.logCodyRuleAction({
