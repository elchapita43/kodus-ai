...
describe('parseCodyRuleFile', () => {
    it('parses the documented template verbatim', () => {
        const parsed = parseCodyRuleFile(TEMPLATE);
    ...
    it('accepts a plain-string path and `severity` as an alias', () => {
        const parsed = parseCodyRuleFile(
            [
                '---',
                'title: T',
                'path: "src/**"',
    ...
    it.each([
        ...
        expect(parseCodyRuleFile(content)).toBeNull();
    });
});
...
describe('isCodyRuleTemplateFile', () => {
    it.each([
        ['.cody/rules/security.md', true],
        ['.cody/rules/architecture/layering.md', true],
        ['apps/api/.cody/rules/naming.md', true],
        ...
        ['.cody/other/file.md', false],
        ['CLAUDE.md', false],
        ['docs/coding-standards/style.md', false],
        [null, false],
    ])('%s → %s', (filePath, expected) => {
        expect(isCodyRuleTemplateFile(filePath as any)).toBe(expected);
    });
});
