...
describe('IDE_RULE_DIR_MARKERS', () => {
    ...
    it('does NOT contain bare ".cursor" or ".cody" — those have no isolated pattern', () => {
        // We only ship `.cursor/rules/**/*.mdc` and `.cody/rules/**`, not
        // bare `.cursor/...` or `.cody/...`. If somebody later adds a
        ...
        expect(IDE_RULE_DIR_MARKERS).not.toContain('.cody');
    });
});
...
describe('extractRepoSubdirFromIdeSource', () => {
    const cases: Array<{
        ...
        {
            name: '.cody/rules at repo root',
            source: '.cody/rules/security.md',
            expected: null,
        },
        {
        ...
        {
            name: '.cody/rules under a subdir',
            source: 'apps/api/.cody/rules/security.md',
            expected: 'apps/api',
        },
        {
...
describe('RULE_FILE_DISCOVERY_PATTERNS (#1484/#1485)', () => {
    ...
    it('discovers nested guidance files (monorepo convention)', () => {
        ...
        expect(discovers('packages/ui/.cody/rules/naming.md')).toBe(true);
    });
