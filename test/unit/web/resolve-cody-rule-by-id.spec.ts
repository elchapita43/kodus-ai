import { resolveCodyRuleById } from "../../../apps/web/src/core/utils/cody-rules/resolve-rule";

describe("resolveCodyRuleById", () => {
    ...
    it("finds the rule via the repository-scoped lookup when available", async () => {
        ...
        const result = await resolveCodyRuleById(
            targetId,
    ...
    it("falls back to the org-wide listing when teamId is missing (broken deep-link)", async () => {
        ...
        // Cody-generated comment links omit teamId/directoryId, so the
