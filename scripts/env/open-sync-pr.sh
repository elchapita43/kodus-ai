...
git config user.name  "codus-env-sync[bot]"
git config user.email "codus-env-sync@users.noreply.github.com"
...
git add -- "${files[@]}"
git commit -m "chore(env): sync from codus-ai@${TAG}

Auto-generated from codus-ai/.env.schema at ${SHA}.
Source: https://github.com/elchapita43/codus-ai/tree/${SHA}/.env.schema"
...
if [ -n "$existing_pr" ]; then
    ...
    gh pr edit "$existing_pr" \
        --body "Auto-generated from \`codus-ai/.env.schema\` at tag \`${TAG}\` ([\`${SHA:0:7}\`](https://github.com/elchapita43/codus-ai/tree/${SHA}/.env.schema)).

Review the diff — added/removed vars and changed defaults reflect schema edits in codus-ai.

Approve to keep \`${repo}\` in sync with this release cut."
else
    gh pr create \
        --title "chore(env): sync from codus-ai@${TAG}" \
