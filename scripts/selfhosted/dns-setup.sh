...
# existing codus-tunnel.service (webhooks) so the two tunnels are managed
# independently.
#
...
#   1. Installs /etc/systemd/system/codus-web-tunnel.service (→ :3000)
...
#   4. Restarts codus-{web,api,webhooks,worker} so they pick up new envs
#
# Usage:
...
# ---------- 1. Install + start codus-web-tunnel.service ----------
# Separate systemd unit from codus-tunnel.service (the webhooks one) so
...
log "Installing codus-web-tunnel.service on '$NAME' ($SERVER_IP)..."
...
cat > /etc/systemd/system/codus-web-tunnel.service <<'UNIT'
[Unit]
Description=cloudflared quick tunnel for Codus web (:3000)
...
systemctl enable --now codus-web-tunnel.service
REMOTE
ok "codus-web-tunnel.service installed + started"
...
[ -n "$WEB_TUNNEL_URL" ] || {
    ...
    err "Check on the droplet: journalctl -u codus-web-tunnel.service --no-pager | tail -30"
    exit 1
}
...
# `codus-web` (not `web`), the rest are bare.
