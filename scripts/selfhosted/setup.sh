...
#
# Prompts for the values provision.sh needs, saves them to ~/.codus-dev/config
...
existing_installer_path="${CODUS_INSTALLER_PATH:-}"
...
if command -v op >/dev/null 2>&1; then
    OP_AVAILABLE=1
    OP_HINT=" — or paste a 1Password ref like op://Engineering/codus-dev/<field>"
fi
...
if [ "$mode" = "show" ]; then
    ...
    echo -e "  ${BLUE}CODUS_INSTALLER_PATH${NC}        ${existing_installer_path:-${GRAY}(not set)${NC}}"
    echo ""
    exit 0
fi
...
$(echo -e "${GREEN}╭─ Codus self-hosted dev setup ─╮${NC}")
...
new_openai_key=$(prompt_secret "OpenAI-compatible API key" "$existing_openai_key" \
    "Required for Codus to review PRs. Team default uses Kimi/Moonshot — get a key at https://platform.moonshot.ai. For native OpenAI, use https://platform.openai.com/api-keys.${OP_HINT}")
...
new_llm_model=$(prompt_plain "LLM model" "$default_llm_model" \
    "Team default: $DEFAULT_LLM_MODEL (Moonshot Kimi K2.6). For OpenAI, use 'gpt-4o' or 'auto' to let Codus router pick.")
...
default_installer_path="${existing_installer_path:-$REPO_ROOT/../codus-installer}"
new_installer_path=$(prompt_plain "Path to codus-installer checkout" "$default_installer_path" \
...
cat > "$tmp_file" <<EOF
# Codus self-hosted dev config — generated by scripts/selfhosted/setup.sh
# Source via 'set -a; . ~/.codus-dev/config; set +a' or via direnv .envrc.
...
CODUS_INSTALLER_PATH=$new_installer_path
EOF
chmod 600 "$tmp_file"
...
if command -v direnv >/dev/null 2>&1; then
    if [ -f "$ENVRC" ]; then
        if grep -q "codus-dev/config" "$ENVRC" 2>/dev/null; then
            ok "$ENVRC already loads the config (direnv ready)"
        else
            warn "$ENVRC exists but does not load ~/.codus-dev/config — add it manually if you want auto-load."
        fi
    else
        echo ""
        ...
        if [[ "$reply" =~ ^[Yy]$ ]]; then
            cat > "$ENVRC" <<EOF
# Auto-loads Codus dev config when you cd into this repo.
# Managed by scripts/selfhosted/setup.sh — run it to regenerate.
if [ -f "\$HOME/.codus-dev/config" ]; then
    dotenv "\$HOME/.codus-dev/config"
fi
EOF
            ok "$ENVRC created"
            ...
            dim "Skipping direnv. You can run 'set -a; . ~/.codus-dev/config; set +a' manually, or create .envrc later."
        fi
    fi
else
    dim ""
    dim "direnv not detected. Install with 'brew install direnv' for auto-load (optional)."
    dim "Without direnv: scripts/selfhosted/*.sh reads ~/.codus-dev/config automatically."
fi
