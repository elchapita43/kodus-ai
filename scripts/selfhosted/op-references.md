...
This file is the source of truth for **internal Codus engineers**. External contributors can ignore it — the `setup` flow works fine with plain values.

## Why use 1Password refs

When you set a field's value to `op://Vault/Item/field` in `~/.codus-dev/config`, the scripts resolve it at runtime via the `op` CLI. Benefits:
...
3. Verify: `op read "op://Engineering/codus-self-hosted-dev/do-token"` should print a token

## Standard paths

> All in vault **Engineering**, item **codus-self-hosted-dev**.

| Env | Reference path | Notes |
|---|---|---|
| `DIGITALOCEAN_TOKEN` | `op://Engineering/codus-self-hosted-dev/do-token` | Required |
| `SH_LICENSE_KEY` | `op://Engineering/codus-self-hosted-dev/license-paid` | Optional |
| `SH_LICENSE_KEY_PATH` | path to a `seats=1` JWT on disk (default `~/.codus-dev/license-seats1.jwt`) | Optional; needed only by the `per-seat-license-toggle` matrix scenario |
| `GH_DEV_TOKEN` | `op://Engineering/codus-self-hosted-dev/gh-bot-token` | Optional |
| `API_OPEN_AI_API_KEY` | `op://Engineering/codus-self-hosted-dev/openai-key` | **Required.** Stores the Moonshot API key (team default uses Kimi K2.6 — see below). For native OpenAI, replace with a `sk-...` from platform.openai.com. |
...
| `API_LLM_PROVIDER_MODEL` | `kimi-k2.6` | Using native OpenAI, set to `gpt-4o` (or `auto` to let Codus router pick) |
...
4. After setup, your `~/.codus-dev/config` looks like:

   ```bash
   DIGITALOCEAN_TOKEN=op://Engineering/codus-self-hosted-dev/do-token
   SH_LICENSE_KEY=op://Engineering/codus-self-hosted-dev/license-paid
   GH_DEV_TOKEN=op://Engineering/codus-self-hosted-dev/gh-bot-token
   API_OPEN_AI_API_KEY=op://Engineering/codus-self-hosted-dev/openai-key
   CODUS_INSTALLER_PATH=/Users/you/dev/codus-installer
   ```
...
1. Open the **codus-self-hosted-dev** item in the **Engineering** vault
...
```
ERROR: DIGITALOCEAN_TOKEN is a 1Password reference (op://Engineering/codus-self-hosted-dev/do-token),
...
3. **Check the path** — `op read "op://Engineering/codus-self-hosted-dev/do-token"` directly, fix if needed
