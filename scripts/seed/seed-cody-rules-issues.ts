...
const pickRulesForOrg = async (
    ...
    const codyRulesCollection = db.collection('codyRules');
    const orgRules = await codyRulesCollection.findOne({
        organizationId,
    });

    if (!orgRules || !Array.isArray(orgRules.rules)) {
        throw new Error(
            `No Cody Rules found for organization ${organizationId}.`,
        );
    }
    ...
    if (!activeRules.length) {
        throw new Error(
            `Organization ${organizationId} does not have active Cody Rules.`,
        );
    }
...
const buildIssueDoc = (
    ...
    return {
        title: `[Cody Rule] ${rule.title}`,
        description:
            (rule.rule as string)?.slice(0, 400) ||
            'Automatically seeded issue to test Cody Rules screens.',
...
async function main() {
    ...
    try {
        ...
        Object.values(result.insertedIds).forEach((id, index) => {
            console.log(
                `   • Issue #${index + 1}: MongoId=${id.toString()} | Rule=${issues[index].codyRule?.title}`,
            );
        });
    } catch (error) {
        console.error(
            '❌ Failed to seed Cody Rules issues:',
