#!/usr/bin/env bash
# Sign up a Codus tenant on the SSO E2E droplet, then configure SSO with
...
#   ADMIN_EMAIL     default: sso-user@codus-test.com (matches SAML user)
...
ADMIN_EMAIL="${ADMIN_EMAIL:-sso-user@codus-test.com}"
...
#    bootstrap-codus.sh which also no-ops on conflict.
...
body = {
    ...
    # active=false matches bootstrap-codus.sh: the API rejects
