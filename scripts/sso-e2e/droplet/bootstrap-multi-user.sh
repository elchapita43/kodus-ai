#!/usr/bin/env bash
# Seed extra Keycloak users and activate the existing Codus SSO config so
...
#     (default "sso-e2e") and the seeded admin (sso-user@codus-test.com)
#     posted /sso-config with active=false.
#
# What this script does on the droplet (via SSH):
#   1. Read KC admin password from /opt/codus-installer/.env.
#   2. Create 2 extra Keycloak users via the admin REST API:
#         newbie-sso@codus-test.com   (signup-via-SSO target)
#         removed-sso@codus-test.com  (removed-user target)
...
NEWBIE_EMAIL="newbie-sso@codus-test.com"
REMOVED_EMAIL="removed-sso@codus-test.com"
...
# public sslip.io URL, which the codus-web container can't reach via
...
env_set() {
    local k=$1 v=$2
    if grep -qE "^${k}=" /opt/codus-installer/.env; then
        sed -i "s|^${k}=.*|${k}=${v}|" /opt/codus-installer/.env
    else
        echo "${k}=${v}" >> /opt/codus-installer/.env
    fi
}
CURR_HOSTNAME=$(grep -E '^WEB_HOSTNAME_API=' /opt/codus-installer/.env | head -1 | cut -d= -f2-)
if [ "${CURR_HOSTNAME}" != "api" ]; then
    ...
    cd /opt/codus-installer && docker compose -p codus-installer -f docker-compose.yml up -d --force-recreate codus-web
fi
REMOTE
...
KC_ADMIN_PASSWORD=$(grep -E '^SSO_E2E_KC_ADMIN_PASSWORD=' /opt/codus-installer/.env | head -1 | cut -d= -f2-)
if [ -z "${KC_ADMIN_PASSWORD:-}" ]; then
    echo "error: SSO_E2E_KC_ADMIN_PASSWORD not present in /opt/codus-installer/.env" >&2
    exit 1
fi
...
# rather than hardcoding. The codus-installer's compose names it
# `db_codus_postgres` today but that's an implementation detail of
# the installer chart, not a contract.
PG_CONTAINER=$(docker ps --format '{{.Names}}' | grep -E '_postgres$|codus[_-]postgres' | head -1)
PG_USER=$(grep -E '^API_PG_DB_USERNAME=' /opt/codus-installer/.env | head -1 | cut -d= -f2-)
PG_DB=$(grep -E '^API_PG_DB_DATABASE=' /opt/codus-installer/.env | head -1 | cut -d= -f2-)
