...
#   1. Realm `codus-sso-e2e`
#   2. SAML client `codus` (recipient: API callback)
#   3. Test user `sso-user@codus-test.com` (password: TestSso!2026)
#
...
# Codus SSO config expects: { entryPoint, idpIssuer, cert }.

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
KC="${KC_URL:-http://localhost:8080}"
REALM="${SSO_E2E_REALM:-codus-sso-e2e}"
# Must match the SP issuer Codus puts in the SAMLRequest.
# saml-auth.strategy.ts:        ssoConfig.providerConfig.issuer || 'codus-orchestrator'
# We don't set `issuer` on the providerConfig, so the default applies.
CLIENT_ID="${SSO_E2E_CLIENT_ID:-codus-orchestrator}"
USER_EMAIL="${SSO_E2E_USER_EMAIL:-sso-user@codus-test.com}"
...
CODUS_ORG_ID_FILE="${REPO_ROOT}/.tmp/sso-e2e-org-id.txt"
...
# The orgId is created by bootstrap-codus.sh and persisted in the file
# below. If that file is missing, fall back to a wildcard so the realm
# still boots — bootstrap-codus.sh re-runs this script after creating
# the org.
ORG_ID="*"
if [ -f "${CODUS_ORG_ID_FILE}" ]; then
    ORG_ID="$(cat "${CODUS_ORG_ID_FILE}")"
fi
SSO_E2E_DOMAIN="${SSO_E2E_DOMAIN:-codus.lvh.me}"
...
# 5. Extract IdP descriptor fields the Codus SSO config expects.
