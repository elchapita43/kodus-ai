/**
 * Standalone test: codus-graph inside E2B sandbox
 *
 * Usage:
 *   E2B_API_KEY=... node scripts/test-codus-graph-e2b.mjs [repo-url] [branch] [file1 file2 ...]
 *
 * Example:
 *   E2B_API_KEY=... node scripts/test-codus-graph-e2b.mjs \
...
const CODUS_GRAPH_VERSION = '0.2.1';
...
async function main() {
    console.log('=== codus-graph E2B Test ===');
    ...
    try {
        ...
        // Step 3: Install bun + codus-graph
        await run(sandbox, [
        ...
        ].join(' && '), `Install codus-graph@${CODUS_GRAPH_VERSION}`, 120_000);

        // Step 4: Verify codus-graph is available
        await run(sandbox, [
            'export PATH="$HOME/.bun/bin:$PATH"',
            'codus-graph --version 2>&1 || echo "codus-graph not found in PATH"',
        ].join(' && '), 'Verify codus-graph');
        ...
        const parseResult = await run(sandbox, [
            ...
            `cd ${REPO_DIR}`,
            `mkdir -p ${GRAPH_DIR}`,
            `codus-graph parse --all --repo-dir . --out ${GRAPH_PATH}${includePatterns.map(p => ` --include "${p}"`).join('')}${excludePatterns.map(p => ` --exclude "${p}"`).join('')} 2>&1`,
        ...
        await run(sandbox, [
            'export PATH="$HOME/.bun/bin:$PATH"',
            `cd ${REPO_DIR}`,
            `codus-graph context --files ${files.join(' ')} --graph ${GRAPH_PATH} --repo-dir . --format json 2>&1 | head -100`,
        ...
        const promptResult = await run(sandbox, [
            'export PATH="$HOME/.bun/bin:$PATH"',
            `cd ${REPO_DIR}`,
            `codus-graph context --files ${files.join(' ')} --graph ${GRAPH_PATH} --repo-dir . --format prompt --out ${PROMPT_PATH} 2>&1`,
