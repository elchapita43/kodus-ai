...
const log = logger('cody-rules-file-sync');
...
// `.cody/rules/**` template files were (a) mangled by the LLM importer
...
//   merged PR with a @cody-sync template file
...
const RULE_FILE_PATH = '.cody/rules/e2e-file-sync.md';
...
export const codyRulesFileSync: Scenario = {
    id: 'cody-rules-file-sync',
    title: 'A .cody/rules template file syncs verbatim and its multi-glob path is enforced',
    priority: 'P0',
    appliesTo: {
    ...
    async run(ctx: RunContext) {
        ...
        const rulePr = await ctx.provider.openPR({
            branch: `e2e/cody-rules-file-sync-rule-${runTag}`,
            title: `[e2e] sync rule file ${runTag}`,
            body: `Automated by Codus E2E run ${ctx.runId}: merges a .cody/rules template (@cody-sync) so the repo-file importer creates the rule.`,
        ...
        const violationBranch = `e2e/cody-rules-file-sync-violation-${runTag}`;
        let violationPr:
            | Awaited<ReturnType<typeof ctx.provider.openPR>>
            | undefined;
        try {
            const synced = await pollUntil<FoundRule>(
                async () => {
                    ...
                    // One merge must import ONE rule: the cross-process sync
                    // race produced two identical rules from a single merge
                    // (caught by live manual validation, invisible to the
                    // find()-based assert this replaced).
                    ctx.assert(
                        matches.length <= 1,
                        `Duplicate import: ${matches.length} rules share sourcePath=${RULE_FILE_PATH} title="${ruleTitle}" — cross-process sync dedupe regression`,
                    );
            ...
            ctx.assert(
                synced,
                `Merged PR #${rulePr.number} with ${RULE_FILE_PATH} (@cody-sync) but no rule with sourcePath=${RULE_FILE_PATH} and title="${ruleTitle}" appeared within 7min — repo-file sync did not import the template`,
            );
            ...
            violationPr = await ctx.provider.openPR({
                ...
                body: `Automated by Codus E2E run ${ctx.runId}: plants ${MARKER} in a file matching the rule's SECOND glob.`,
                fixtureFiles: {
                    ...
                    (await pollUntil(
                        async () => {
                            const resp = await http<{ data?: unknown[] }>(
                                `${ctx.target.apiBaseUrl}/cody-rules/suggestions?ruleId=${encodeURIComponent(syncedRuleId!)}`,
                                {
                                    headers: {
                                        Authorization: `Bearer ${session.accessToken}`,
                                    },
            ...
            // Same race-vs-regression discipline as cody-rules-create-and-apply:
            ...
            if (syncedRuleId) {
                try {
                    await http(
                        `${ctx.target.apiBaseUrl}/cody-rules/delete-rule-in-organization-by-id?ruleId=${encodeURIComponent(syncedRuleId)}&teamId=${encodeURIComponent(session.teamId)}`,
                        {
                            method: 'DELETE',
                            headers: {
                                Authorization: `Bearer ${session.accessToken}`,
...
export default codyRulesFileSync;
