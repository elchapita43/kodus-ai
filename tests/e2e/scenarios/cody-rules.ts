...
> = {
    github: {
        // Persistent fixture branch in codus-e2e/tiny-url. Adds
        ...
        head: "fixture/cody-rule-todo-remove-me",
        base: "main",
    },
    gitlab: {
        // Same fixture mirrored into gitlab.com/codus-e2e/tiny-url. The
    ...
    "azure-devops": {
        ...
        // dev.azure.com/elchapita43/codus-e2e/_git/codus-e2e.
        head: "fixture/cody-rule-todo-remove-me",
        base: "main",
    },
    bitbucket: {
    ...
    // App-installed clone of tiny-url (codus-e2e/tiny-url-app) carries
    // the same fixture/cody-rule-todo-remove-me branch via the
...
export const codyRulesCreateAndApply: Scenario = {
    id: "cody-rules-create-and-apply",
    title: "A Cody Rule is created and applied to a fresh PR",
    priority: "P0",
    appliesTo: {
    ...
    async run(ctx: RunContext) {
        ...
        ctx.assert(
            fixture,
            `No fixture branch pair configured for provider ${ctx.provider.name} in cody-rules.ts`,
        );
        ...
        if (swept > 0) {
            log.info(`[cody-rules] swept ${swept} stale rule(s) (e2e-rule-* + auto-generated past_reviews) before creating a fresh one`);
        }
        ...
        const ruleResp = await http<{
            ...
            // POST /cody-rules/create-or-update (CreateCodyRuleDto). `type`,
            // `title`, `rule`, `severity` are required by the DTO; severity is
            // lowercase. `origin` is optional (defaults to 'manual') and must be
            // one of the CodyRulesOrigin values: manual, library, past_reviews,
        ...
        ensureOk(ruleResp, "cody-rules:create");
        ...
        // before the rule propagates and then run the cody-rules agent with no
        ...
        const opened = await ctx.provider.openPRFromBranches({
            head: fixture!.head,
            base: fixture!.base,
            title: `[e2e] cody-rules ${ruleName}`,
            body: `Automated PR opened by Codus E2E run ${ctx.runId} to validate rule ${ruleName}. Auto-closed by the scenario; branches are persistent fixtures and are not deleted.`,
        });
        ...
        try {
            ...
            // returns as soon as Cody posts the completion comment on the
            // provider, but the pipeline persists
            // `files.suggestions[].brokenCodyRulesIds` asynchronously after
                ...
                const count =
                    (await pollUntil(async () => {
                        const resp = await http<{ data?: unknown[] }>(
                            `${ctx.target.apiBaseUrl}/cody-rules/suggestions?ruleId=${encodeURIComponent(ruleId!)}`,
                            {
                                headers: {
                                    Authorization: `Bearer ${session.accessToken}`,
                                },
                        ...
                        ensureOk(resp, "cody-rules:findSuggestionsByRule");
            ...
            // but that masked the class of bug where Cody posts the comment
            ...
            if (ruleId) {
                try {
                    await http(
                        // DELETE /cody-rules/delete-rule-in-organization-by-id
                        // ?ruleId=…&teamId=… (query params, not path).
                        `${ctx.target.apiBaseUrl}/cody-rules/delete-rule-in-organization-by-id?ruleId=${encodeURIComponent(ruleId)}&teamId=${encodeURIComponent(session.teamId)}`,
                        {
                            method: "DELETE",
                            headers: {
                                Authorization: `Bearer ${session.accessToken}`,
...
// /cody-rules/find-by-organization-id response. Shape is roughly
...
//     background 3-month backfill added in the cody-rules-generation hotfix).
    ...
    for (const r of stale) {
        try {
            const del = await http(
                `${ctx.target.apiBaseUrl}/cody-rules/delete-rule-in-organization-by-id?ruleId=${encodeURIComponent(r.uuid)}&teamId=${encodeURIComponent(session.teamId)}`,
                {
                    method: "DELETE",
                    headers: { Authorization: `Bearer ${session.accessToken}` },
                    timeoutMs: 20_000,
...
export default codyRulesCreateAndApply;
