...
const log = logger("cody-rules-coverage");

// ── #1449 coverage regression guard ──────────────────────────────────────────
//
// The bug this scenario protects against: the OLD agentic cody-rules agent let
...
//
// `cody-rules-create-and-apply` proves a rule fires AT ALL (≥1 suggestion).
...
// multi-file coverage test, enrich `fixture/cody-rule-todo-remove-me` in the
...
const COVERAGE_MIN = Number(process.env.CODY_RULES_COVERAGE_MIN ?? 2);

export const codyRulesCoverage: Scenario = {
    id: "cody-rules-coverage",
    title: "A Cody Rule is flagged at every site in a PR (sharding coverage, #1449)",
    priority: "P1",
    appliesTo: {
    ...
    async run(ctx: RunContext) {
        ...
        // it — cody-rules-create-and-apply already proves the rule fires. Skip
        // by default; enrich fixture/cody-rule-todo-remove-me to N≥6 lines
        // across ≥2 files, then set CODY_RULES_COVERAGE_ENABLED=1 (and
        // COVERAGE_MIN) to turn this into a real multi-site gate.
        if (process.env.CODY_RULES_COVERAGE_ENABLED !== "1") {
            ctx.skip(
                "cody-rules-coverage pending an enriched multi-site fixture " +
                    "(set CODY_RULES_COVERAGE_ENABLED=1 once fixture/cody-rule-todo-remove-me carries N≥6 marker sites)",
            );
        }
        ...
        ensureOk(ruleResp, "cody-rules-coverage:create");
        ...
        const opened = await ctx.provider.openPRFromBranches({
            head: fixture!.head,
            base: fixture!.base,
            title: `[e2e] cody-rules coverage ${ruleName}`,
        ...
        try {
            ...
            const suggestionsCount =
                (await pollUntil<number>(async () => {
                    ...
                    ensureOk(resp, "cody-rules-coverage:suggestions");
...
export default codyRulesCoverage;
