...
const log = logger('cody-rules-lifecycle');
...
//  * '@cody-sync' itself was reported as a missing file, and the LLM
//    detector contaminated real reference names with a fabricated
//    'cody-sync/' repo prefix → clean-error-content asserts;
...
const RULE_FILE_PATH = '.cody/rules/e2e-lifecycle.md';
...
export const codyRulesLifecycle: Scenario = {
    id: 'cody-rules-lifecycle',
    ...
    async run(ctx: RunContext) {
        ...
        const mergeRuleFile = async (
            ...
            const pr = await ctx.provider.openPR({
                ...
                body: `Automated by Codus E2E run ${ctx.runId} (lifecycle phase: ${phase}).`,
        ...
        try {
            ...
            ctx.assert(
                !errorText.toLowerCase().includes('cody-sync'),
                `Sync errors must never mention the @cody-sync control marker (spurious-marker regression). errors=${errorText.slice(0, 400)}`,
            );
            log.info(
...
export default codyRulesLifecycle;
