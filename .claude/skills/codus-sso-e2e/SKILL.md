...
docker image inspect codus-api:sso-e2e >/dev/null 2>&1 \
    || echo "ERROR: build with 'API_TAGS=codus-api:sso-e2e API_CLOUD_MODE=false docker buildx bake -f docker-bake.hcl api'"
docker image inspect codus-web:sso-e2e >/dev/null 2>&1 \
    || echo "ERROR: build with 'WEB_TAGS=codus-web:sso-e2e RELEASE_VERSION=sso-e2e docker buildx bake -f docker-bake.hcl web'"
...
═══════════════════════════════════════════════════════════
  ...
  Layer 2: codus-web:test image               built
...
1. `browser_navigate https://api.${SSO_E2E_DOMAIN:-codus.lvh.me}/auth/sso/login/$(cat .tmp/sso-e2e-org-id.txt)`
2. Fill Keycloak login form: `sso-user@codus-test.com` / `TestSso!2026`
3. After Keycloak posts the SAML assertion back, the browser should
   land on `https://app.${SSO_E2E_DOMAIN:-codus.lvh.me}/setup` (or
...
- **Method A (recommended, no code changes)**: read API logs after
  the SAML round-trip:
  ```sh
  docker logs codus-sso-e2e-api 2>&1 | grep '\[SSO_E2E\]' | tail -1
  ```
...
The default shape is `codus.lvh.me` (3-label common parent — analog
...
# Demo-only images
docker rmi codus-web:nofix-selfhosted codus-web:nofix-cloud 2>/dev/null
...
- **"Network error while requesting…" on `/setup`** → Web container
  ...
  (compose `codus-web.volumes` should include the mkcert CAROOT).
