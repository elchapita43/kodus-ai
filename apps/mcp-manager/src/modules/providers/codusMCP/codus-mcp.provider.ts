...
import { CodusMCPClient } from '../../../clients/codusMCP';
import {
...
@Injectable()
export class CodusMCPProvider extends BaseProvider {
    private readonly client: CodusMCPClient;
    ...
    private readonly logger: Logger = new Logger(CodusMCPProvider.name);
    constructor(
        ...
        this.client = new CodusMCPClient();
    ...
    private resolveManagedBaseUrl(baseUrl: string): string {
        ...
        // served by the codus-api MCP server, NOT the mcp-manager. Resolve
        // against the codus-api origin so the URL ends up addressable.
        //
        // `API_CODUS_MCP_SERVER_URL` (already in every .env) carries the
        // canonical address — e.g. `http://codus_api:3001/mcp`. We extract
        ...
        const mcpServerUrl = process.env.API_CODUS_MCP_SERVER_URL;
        if (!mcpServerUrl) {
            throw new Error(
                'Cannot resolve relative MCP base URL: set API_CODUS_MCP_SERVER_URL or API_CODUS_MCP_MANAGER_BACKEND_URL',
            );
        }
    ...
    private transformManagedIntegration(
        ...
        return {
            ...
            providerType: MCPProviderType.CODUSMCP,
            ...managed.auth,
    ...
    async getIntegration(
        ...
        try {
            ...
            if (integration.id !== integrationId) {
                throw new Error(
                    `Integration ${integrationId} não suportada pela Codus`,
                );
            }
        ...
        try {
            if (this.managedIntegrations.has(integrationId)) {
                return {
                    success: true,
                    message:
                        'Selected tools updated for managed Codus MCP integration.',
                    selectedTools,
                };
            }
    ...
    async initiateConnection(
        ...
        try {
            ...
            throw new Error(
                `Integration ${config.integrationId} não suportada para conexão Codus`,
            );
        } catch (error) {
    ...
    private async safeGetTools(client: CustomClient): Promise<MCPTool[]> {
        ...
        try {
            ...
            console.error('Failed to fetch managed Codus MCP tools:', error);
            return [];
        }
    }
}
