...
/**
 * How long a `generating_*` `codyLearningStatus` may persist before the
 * CodyLearning cron treats it as a dead run and regenerates.
 *
 ...
 */
export function isCodyLearningStatusStale(
    status: CodyLearningStatus | undefined,
    updatedAt: Date | undefined,
    now: number = Date.now(),
): boolean {
    const isGenerating =
        status === CodyLearningStatus.GENERATING_CONFIG ||
        status === CodyLearningStatus.GENERATING_RULES;
...
/**
 * How many consecutive hard-crashed rule-generation runs the CodyLearning
 ...
 * cron tick. `GenerateCodyRulesUseCase` bumps the count when it enters
