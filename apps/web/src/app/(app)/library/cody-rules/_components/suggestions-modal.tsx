...
import { getCodyRuleSuggestions } from "@services/codyRules/fetch";
import type { CodyRuleSuggestion } from "@services/codyRules/types";
...
export const SuggestionsModal = ({
    ...
    const {
        ...
        isError,
        refetch,
    } = useQuery<CodyRuleSuggestion[]>({
        queryKey: ["cody-rule-suggestions", ruleId],
        queryFn: () => getCodyRuleSuggestions(ruleId),
        enabled: open,
    });

    const groupedByPR = safeArray<CodyRuleSuggestion>(suggestions).reduce(
        (acc, suggestion) => {
        ...
        {} as Record<
            string,
            {
                ...
                prUrl: string;
                repositoryFullName: string;
                suggestions: CodyRuleSuggestion[];
            }
        >,
    );
