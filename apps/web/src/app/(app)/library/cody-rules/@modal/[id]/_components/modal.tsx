...
import { addCodyRuleToRepositories } from "@services/codyRules/fetch";
import {
    CodyRulesOrigin,
    CodyRulesStatus,
...
export const CodyRuleLibraryItemModal = ({
    rule,
    repositoryId,
    directoryId,
    repositories,
    ...
    const [addToRepositories, { loading: isAddingToRepositories }] =
        useAsyncAction(async () => {
            const newRule: CodyRule = {
                title: rule.title,
                rule: rule.rule,
                severity: resolveCodyRuleDisplaySeverity(
                    rule,
                ) as CodyRule["severity"],
                path: "",
            ...
            const addedCodyRules = await addCodyRuleToRepositories({
                rule: newRule,
                repositoriesIds,
                directoriesIds,
                teamId,
            ...
            if (isCentralizedPrResponse(addedCodyRules)) {
                toast(
            ...
            return router.push(`/library/cody-rules`);
        });
    ...
    const { mutate: sendFeedback, isPending: isFeedbackActionInProgress } =
        useMutation<any, Error, FeedbackType>({
            ...
            onSuccess: (data, feedback) => {
                revalidateServerSidePath("/library/cody-rules");
                const isRemovingFeedback = userFeedback === feedback;
        ...
        <Dialog
            ...
            onOpenChange={() => {
                if (window.history.length === 1) {
                    router.push("/library/cody-rules");
                } else {
                    router.back();
                }
            }}>
