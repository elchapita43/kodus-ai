...
import {
    applyPendingCodyRules,
    convertPendingUpdatesToNew,
    discardPendingCodyRules,
...
export const PendingSection = ({
    ...
    const approve = (r: CodyRule) =>
        run(
            () => applyPendingCodyRules(teamId, [r.uuid!]),
            "Change proposed through centralized pull request.",
        );

    const discard = (r: CodyRule) =>
        run(
            () => discardPendingCodyRules(teamId, [r.uuid!]),
            "Discard proposed through centralized pull request.",
        );

    const createInstead = (r: CodyRule) =>
        run(
    ...
    const bulkApprove = () =>
        runBulk(
            () => applyPendingCodyRules(teamId, selectedIds),
            "Changes proposed through centralized pull request.",
        );

    const bulkDiscard = () =>
        runBulk(
            () => discardPendingCodyRules(teamId, selectedIds),
    ...
    const cardSelection = (r: CodyRule) =>
        canEdit && r.uuid
            ? {
