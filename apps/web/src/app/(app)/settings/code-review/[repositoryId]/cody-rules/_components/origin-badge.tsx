...
} from "src/core/utils/cody-rules/infer-origin";
...
const ORIGIN_TOOLTIPS: Record<Exclude<InferredRuleOrigin, "manual">, string> = {
    ...
    "Cody-generated":
        "Suggested by the Cody rule generator from past reviews",
    Library: "Added from the Cody rule library",
    "MCP/Agent": "Created by an MCP / agent",
    CLI: "Created via the Cody CLI",
};
...
//   Cody-generated  → tertiary  (pink)   — Cody / LLM brand
...
type OriginBadgeProps = {
     ...
     * (`@cody-sync` vs `Orphan`) is only meaningful — and only rendered —
...
// Cody-generated). Intentionally avoids Radix Tooltip because nesting a
...
//      Onboarding / Cody-generated / Library (manual hides). Purely identity.
//
...
//        • file still tagged `@cody-sync` (`pinnedSync`) → kept in sync →
//          discreet neutral `@cody-sync` chip (healthy, low emphasis).
        ...
        <span className="inline-flex flex-wrap items-center gap-1.5">
            ...
            {showMaintenance && isPinned && (
                <Badge
                    ...
                    size="xs"
                    title={
                        "Kept in sync via @cody-sync even with auto-sync off" +
                        sourceSuffix
                    }
            ...
            {showMaintenance && !isPinned && (
                <Badge
                    ...
                    size="xs"
                    title={
                        "Auto-sync is off and this file has no @cody-sync marker — no longer maintained" +
                        sourceSuffix
                    }
