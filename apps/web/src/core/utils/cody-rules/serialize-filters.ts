/**
 * Pure helpers for serializing/parsing the Cody Rules list filters into
...
export function parseFiltersFromParams(
    ...
    const codySync = params.get(FILTER_PARAM_KEYS.codySync) === "1";
    ...
    return {
        query,
        listFilters: { origins, severities, codySync, withSyncErrors, pausedOnly },
        onlyOrphans,
    };
}
...
export function applyFiltersToParams(
    ...
    if (filters.listFilters.codySync) {
        params.set(FILTER_PARAM_KEYS.codySync, "1");
    } else {
        params.delete(FILTER_PARAM_KEYS.codySync);
    }
