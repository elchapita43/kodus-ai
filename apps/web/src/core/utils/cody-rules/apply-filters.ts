/**
 * Pure helpers for the Cody Rules list filters.
 *
...
export type ListFilters = {
    ...
    /** Toggle: when true, only Auto-sync rules pinned via `@cody-sync` pass. */
    codySync: boolean;
 ...
 *
 * Excludes only:
 *   - pinnedSync rules — their source file carries `@cody-sync`, so the
...
/**
 * @cody-sync filter: only Auto-sync rules still pinned via `@cody-sync`
 * (`pinnedSync`) pass. Everything else is excluded while it's on.
 */
export function matchesCodySyncFilter(
    rule: {
        sourcePath?: string | null;
        origin?: string | null;
