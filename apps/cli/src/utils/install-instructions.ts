...
const UNIX_INSTALLER_URL =
    'https://raw.githubusercontent.com/elchapita43/cli/main/install.sh';
const WINDOWS_INSTALLER_URL =
    'https://raw.githubusercontent.com/elchapita43/cli/main/install.ps1';
...
export function resolveRemoteInstallInstructions(
    ...
    if (platform === 'win32') {
        return {
            primary: `powershell -NoProfile -ExecutionPolicy Bypass -Command "$tmp = Join-Path $env:TEMP 'codus-install.ps1'; Invoke-WebRequest ${WINDOWS_INSTALLER_URL} -OutFile $tmp; & $tmp"`,
    ...
    return {
        ...
        fallback: [
            `curl -fsSL ${UNIX_INSTALLER_URL} -o /tmp/codus-install.sh`,
