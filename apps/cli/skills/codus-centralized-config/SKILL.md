---
name: codus-centralized-config
description: Use when the user wants to manage centralized configuration via `codus config centralized` commands (status, init, sync, disable, and download).
---
...
Centralized configuration allows teams to manage their Codus configuration files in a single repository, providing a single source of truth for their settings. This skill enables users to manage their centralized configuration through Codus CLI commands, including initializing centralized config, syncing configuration, disabling centralized config, and downloading generated config files.
...
If the setting is enabled, then any pull request merged to the selected repository will trigger a sync to update the configuration in Codus. This allows teams to easily manage and update their configuration as needed, without having to manually apply changes in Codus.
...
Manage centralized configuration through Codus CLI commands only.
...
- Requests to run: `codus config centralized status|init|sync|disable|download`.
- Requests to enable or disable centralized config from terminal.

## Workflow

Unless specifically stated, YOU must run the Codus CLI commands on behalf of the user. Do not provide instructions to run commands without also running them yourself.
...
ALWAYS run the command `codus config remote list` first to get the list of repositories the user has access to and their current selection. You must provide the repo in the shape `owner/repo` for example `organization/repository-name`. If you cannot find the repository in the list, then tell the user about it.

- Defaults and behavior:
- `--sync-option` defaults to `pr`. Defines the behaviour of the initial sync after enabling centralized config.
    - `pr` creates a pull request with their current config. A link to the PR is provided in the command output. Merging the PR triggers a sync to update the configuration in Codus.
...
A sync will pull the latest configuration from the repository and override the user's current configuration defined in the Codus database with the config from the repository. Syncs happen automatically when a PR is merged regardless of the initial sync option selected, but users can also choose to run a manual sync at any time to pull the latest changes from the repository.
...
```bash
codus config centralized download --out <path/to/centralized-config.zip>
```
...
- If repository selection fails, verify the repository is already selected in Codus (`codus config remote list`).
