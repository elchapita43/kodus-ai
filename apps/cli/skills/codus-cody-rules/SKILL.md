...
description: Use when the user wants to create, update or view Cody Rules via `codus rules` command.
---
...
Cody Rules are a set of guidelines that Cody follows when generating code. They help ensure that the generated code is consistent, high-quality, and aligned with the user's preferences and project requirements.

## Goal

Manage Cody Rules through Codus CLI only. Do not suggest creating rule files manually.

ALWAYS Use `codus rules` subcommands for all create, update, and view operations. All these rules are ALWAYS managed by the `codus rules` command from the CLI. Do NOT suggest creating files or storing rules in any other way. When the user wants to create, update or view Cody Rules, utilize the `codus rules` command with the appropriate subcommands and options as outlined in the instructions files.

## Centralized Config Convention

When centralized config is enabled for the selected team/repository scope, `codus rules create` and `codus rules update` may return centralized PR metadata instead of directly created/updated rule records.
...
- [instructions/create-cody-rule.md](instructions/create-cody-rule.md): Guidelines for creating new Cody Rules.
- [instructions/update-cody-rule.md](instructions/update-cody-rule.md): Guidelines for updating existing Cody Rules.
- [instructions/view-cody-rules.md](instructions/view-cody-rules.md): Guidelines for viewing and retrieving Cody Rules.

You MUST always load at least one of these instructions files to handle the specific user request related to Cody Rules. Each file contains detailed steps and examples for the corresponding action (create, update, view). Always ensure that you are following the instructions in these files when managing Cody Rules through the `codus rules` command.

Should the user request an action that is not covered by these instructions, you should first clarify the user's intent and then determine if it falls under create, update, or view operations. If it does, proceed to load the corresponding instructions file to ensure that you are following the correct workflow for managing Cody Rules through the `codus rules` command.

Should the user request a new action related to Cody Rules that differs from the initial action, you should load the appropriate instructions file for that new action to ensure that you are following the correct workflow for managing Cody Rules through the `codus rules` command.

## Structure of a Cody Rule

A Cody Rule typically consists of the following components:
