---
name: codus-pr-suggestions-resolver
description: Use when the user wants to fetch, triage, or implement Codus suggestions for an existing remote pull request via `codus pr suggestions`, `--pr-url`, or `--pr-number`.
---
...
Fetch PR suggestions via Codus CLI, triage each suggestion against the PR goal, apply safe fixes, then validate with build/tests and report results.
...
### 2) Run Codus suggestions

Use:

```
codus pr suggestions --pr-url <url>
```

Or when the URL is not available:

```
codus pr suggestions --pr-number <number> --repo-id <id>
```
