/**
 * Pure matching/ordering logic for the Codus hunk sidebar.
 *
 * Split out of `index.tsx` so it can be unit-tested without a React/OpenTUI
 * runtime — the sidebar's real risk isn't the rendering, it's mapping a Codus
...
export type CodusSeverity = 'critical' | 'error' | 'warning' | 'info';

export interface CodusFinding {
    id: string;
    file: string;
    line: number;
    endLine: number;
    severity: CodusSeverity;
    title: string;
    category?: string;
    ruleId?: string;
}

export interface CodusFindings {
    version: 1;
    summary?: string;
    findings: CodusFinding[];
}
...
export const SEVERITY_ORDER: CodusSeverity[] = [
    'critical',
    'error',
    'warning',
    'info',
...
export const SEVERITY_GLYPH: Record<CodusSeverity, string> = {
    critical: '‼',
    error: '✖',
    warning: '⚠',
    info: 'ℹ',
 ...
 * glyph.
 */
export function coerceSeverity(value: unknown): CodusSeverity {
...
export function parseFindings(raw: unknown): CodusFindings {
    const empty: CodusFindings = { version: 1, findings: [] };
    if (!raw || typeof raw !== 'object') {
        return empty;
    }
    const candidate = raw as Partial<CodusFindings>;
    ...
    return {
        ...
        findings: candidate.findings
            .filter(
                (finding): finding is CodusFinding =>
                    Boolean(finding) &&
 ...
 */
export function orderFindings(findings: CodusFinding[]): CodusFinding[] {
...
export function findHunkIndex(
    file: DiffFileLike,
    finding: Pick<CodusFinding, 'line'>,
): number | null {
...
export function countBySeverity(
    findings: readonly CodusFinding[],
): Partial<Record<CodusSeverity, number>> {
    const tally: Partial<Record<CodusSeverity, number>> = {};
