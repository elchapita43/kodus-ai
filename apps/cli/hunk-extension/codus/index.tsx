/**
 * Codus findings sidebar for Hunk.
 *
 * Loaded by `codus review` with `--extension <this folder>` whenever it hands a
 * review off to the hunk TUI. Findings arrive out-of-band through the
 * `CODUS_HUNK_FINDINGS` env var — a JSON sidecar written by the CLI — because
 ...
 */
function loadFindings(): CodusFindings {
    const sidecarPath = process.env.CODUS_HUNK_FINDINGS;
    if (!sidecarPath) {
...
function CodusFindingsSidebar({
    ...
    const headline =
        ordered.length === 0
            ? ' Codus · no findings'
            : ` Codus · ${ordered.length} finding${ordered.length === 1 ? '' : 's'}`;
...
export default function registerCodusFindings(hunk: HunkExtensionAPI): void {
    ...
    hunk.registerSidebarView({
        ...
        component: CodusFindingsSidebar,
    });
    ...
    hunk.on('startup', (_event, ctx) => {
        ...
        const breakdown = SEVERITY_ORDER.filter((severity) => counts[severity])
            ...
            .join(' ');
        ctx.notify(
            `Codus · ${ordered.length} finding${ordered.length === 1 ? '' : 's'} ${breakdown} · y panel · n/p next`,
        );
    });
    ...
    hunk.registerCommand(
        { id: 'toggle', title: 'Toggle Codus findings', key: 'y' },
        (ctx) => {
            if (!revealed) {
                reveal(ctx);
                return;
    ...
    const step = (delta: number) => (ctx: ExtensionCommandContext) => {
        if (ordered.length === 0) {
            ctx.notify('No Codus findings in this review');
            return;
        }
    ...
    hunk.registerCommand(
        { id: 'next', title: 'Next Codus finding', key: 'n' },
        step(1),
    );
    hunk.registerCommand(
        { id: 'previous', title: 'Previous Codus finding', key: 'p' },
        step(-1),
    );
}
